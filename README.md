...
> 本文件是仓库唯一根目录 README，也是产品与协作入口。文档分类入口见 [`docs/README.md`](./docs/README.md)；**产品 / 消费者决策 / UI / 前端 / 跨端 / API / 数据可信 / 后台 / 隐私 / 测试验收完整规范集见 [`docs/product-blueprint/`](./docs/product-blueprint/README.md)**；新机器部署见 [`docs/deployment.md`](./docs/deployment.md)；数据库设计见 [`docs/database.md`](./docs/database.md)；Agent 强制规范见 [`AGENTS.md`](./AGENTS.md)；任务清单见 [`CODEX_TASKS.md`](./CODEX_TASKS.md)；进度计划见 [`PROJECT_PLAN.md`](./PROJECT_PLAN.md)；命令见 [`COMMANDS.md`](./COMMANDS.md)；数据来源与口径见 [`DATA_SOURCES.md`](./DATA_SOURCES.md)。
...
尽量一次完成一个独立功能模块或明确优化后再统一验证和提交，避免围绕同一模块反复碎片化提交；每次任务完成后必须输出当前项目百分比进度。

当用户明确说明当前周期 token/额度较充足时，优先安排大批量数据扩充与审核、OCR/AI 分析协议、报告生成、跨端重构等高 token 消耗模块；低 token 的零散优化可后置。执行高 token 任务时仍要集中完成关键功能闭环，避免多轮重复消耗上下文。
...
后续编码 Agent 执行任务时，必须按“用户流程闭环”推进，而不是只完成单个组件、单个函数或单个文件后停止。

核心规则：

1. 当用户要求实现某个阶段、模块或功能闭环时，不要只完成一个小组件就停止。
2. 每轮任务应尽量完成一个可验证的用户流程，例如首页闭环、搜索闭环、详情闭环、成分表分析闭环、收藏历史闭环。
3. 一个有效的功能闭环通常应包含：数据模型、业务逻辑、页面 / 组件、路由跳转、状态处理、移动端样式、错误 / 空状态、文档更新和验证命令。
4. 可以修改多个相关文件，但禁止修改无关功能、无关模块或进行大范围无关重构。
5. 如果任务过大，可以在内部拆成 2～4 个子步骤，但应在同一轮中继续推进，直到完成当前范围内可验证的闭环。
6. 不要因为遇到非阻塞问题就直接停止；可以继续完成其他可完成部分，并在 `AI_REVIEW.md` 中说明阻塞点、影响范围和后续处理建议。
7. 本轮任务完成前，必须同步检查并更新 `COMMANDS.md`、`PROJECT_PLAN.md`、`AI_REVIEW.md`；如果某个文档确认无需改动，也必须在最终回复中说明原因。
8. 最后必须执行当前项目可用的验证命令，例如 build、lint、test；如果某条命令无法执行，必须说明原因。纯文档修改至少执行文档或差异检查。
9. 如果只完成了局部组件，但用户流程无法验证，则视为任务未完成。
10. 页面和交互必须符合现代移动端 App 审美，不能只满足功能可用；移动端布局、状态反馈、错误 / 空状态和触控体验都应纳入闭环验收。

---

## 3. 项目定位

**CompLens / 成分镜（项目代号 CompCheck）** 面向普通用户，是一个**食品配料表拍照识别与成分分析 App**。
...
1. **real 模式**：后端配置了 `OCR_API_KEY` 且前端已登录，调用 `POST /api/ocr`
2. **manual 模式**：未配置 Key 或未登录，用户手动输入/粘贴配料表文字
3. **fallback 模式**：API 调用失败，降级到 manual 模式，提示"OCR 识别失败，请手动输入文字"

**关键规则**：
- 未配置 OCR Key 时，App 不得崩溃，必须显示 manual 入口
- 不允许在没有真实 OCR Key 的情况下返回伪造的识别结果
- `isMock: true` 的结果必须明确标注，不得混淆为真实 OCR 结果

### 3.0.2 禁止伪造规则

以下内容**绝对禁止**：

1. **禁止伪造 OCR 结果**：没有 API Key 时，不能返回假装识别成功的文字
2. **禁止 AI 编造数据来源**：AI 不能生成或推断 `sourceName`、`sourceVersion`、`rawSourceText`、`adi`、`usageLimits` 等需要官方来源的字段
3. **禁止 AI 生成权威安全结论**：AI 不能给出"这个成分绝对安全/有害"的表述
4. **禁止把 seed 样本当完整数据集**：100 条 unverified seed 不能标注为"完整食品添加剂库"
5. **禁止生成医疗诊断**：所有成分分析结果不构成医疗建议
...
环境变量名不加 `VITE_` 前缀（Vite 仅用于前端构建和开发服务器，密钥不注入前端）。服务端密钥只在后端 Node.js 进程中读取，不在前端暴露。
...
- 任何打包工具（webpack、Parcel — Vite 内置 Rollup 除外）
- TypeScript 编译器
...
| 技术栈违规 | 引入 React / Vue / TypeScript / webpack / Parcel 等禁止依赖 |
