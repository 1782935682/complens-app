...
export const foodAdditiveRules: AdditiveRule[] = [
  ...
  {
    name: '单双甘油脂肪酸酯',
    aliases: ['单，双甘油脂肪酸酯', '单、双甘油脂肪酸酯', '单双甘油脂肪酸酯'],
    category: '乳化剂',
    effect: '帮助改善组织状态和口感。',
    reminder: '常见于烘焙、冰淇淋和饮料类食品。',
    keywords: ['单，双甘油脂肪酸酯', '单、双甘油脂肪酸酯', '单双甘油脂肪酸酯'],
    displayLevel: 'normal'
  },
  {
    name: '蔗糖脂肪酸酯',
...
export function recognizeAdditives(input: RecognitionInput): AdditiveRecognition[] {
  ...
  foodAdditiveRules.forEach((rule) => {
    const matchedTerms = unique(rule.keywords.filter((term) => combinedText.includes(normalizeText(term))));
...
function buildRecognitionFromRule(
  ...
  const targetNotes = Object.entries(rule.targetReminders || {})
    .filter(([key]) => Boolean(attention?.goals.includes(key)))
    .map(([, value]) => value);
  return {
...
function findRuleForText(value: string): AdditiveRule | undefined {
  ...
  return foodAdditiveRules.find((rule) => rule.aliases.some((alias) => text.includes(normalizeText(alias))));
}
...
function inferCategory(value: string): AdditiveCategory {
  ...
  if (/胶|果胶|卡拉胶|黄原胶|稳定剂|增稠/.test(text)) return '增稠剂/稳定剂';
  if (/乳化|磷脂|脂肪酸酯|吐温|司盘/.test(text)) return '乳化剂';
  if (/酸度|柠檬酸|乳酸|苹果酸|碳酸钠|磷酸/.test(text)) return '酸度调节剂';
  if (/抗氧化|抗坏血酸|维生素C|茶多酚|BHA|BHT|TBHQ/.test(text)) return '抗氧化剂';
  if (/香精|香料|香兰素|麦芽酚|柠檬醛|薄荷脑/.test(text)) return '香精香料';
  if (/膨松|碳酸氢|泡打粉|磷酸氢钙/.test(text)) return '膨松剂';
  return '其他食品添加剂';
}
...
function fallbackEffect(category: AdditiveCategory): string {
  ...
  if (category === '增稠剂/稳定剂') return '帮助改善稠度或保持口感稳定。';
  if (category === '乳化剂') return '帮助油和水混合，改善口感。';
  if (category === '酸度调节剂') return '调节酸味或口感。';
  if (category === '抗氧化剂') return '帮助减少氧化，保持食品状态。';
  if (category === '香精香料') return '用于调味或增强风味。';
  if (category === '膨松剂') return '帮助烘焙食品形成蓬松口感。';
