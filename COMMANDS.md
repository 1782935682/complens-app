...
微信小程序构建可额外设置 `WEIXIN_MP_APPID` 临时注入真实 AppID，设置 `WEIXIN_MP_URL_CHECK=true|false` 控制生成产物的开发者工具合法域名校验配置。详细导入、公众平台域名和真机验收见 [`docs/platform/wechat-mini-program.md`](./docs/platform/wechat-mini-program.md)。
也可以把本机调试配置写入 `user-uniapp/.env.local`；该文件已被 gitignore，仅用于本地微信开发者工具调试，不能提交真实账号、域名或密钥。
微信小程序专项命令（小程序构建、后端 3010 启动、Postgres、Caddy、systemd、公网 API 验证）集中维护在 [`docs/platform/wechat-mini-program.md`](./docs/platform/wechat-mini-program.md)。
Windows Git Bash 运行小程序时，优先看 `docs/platform/wechat-mini-program.md` 的“Windows / Git Bash 快速运行”小节；仓库根目录使用 `npm run user:build:mp-weixin`，`user-uniapp/` 子目录内使用 `npm run build:mp-weixin`。小程序脚本会直接通过当前 Node 执行 uni 的 JS 入口，避免 Windows `.cmd` shim / shell 路径问题。

ChatGPT UI 评审入口已提供，但必须由运行环境提供 `OPENAI_API_KEY`，不得写入前端、文档或仓库。先运行 `npm --prefix user-uniapp run build:h5 && npm --prefix user-uniapp run visual:smoke` 生成截图，再运行 `OPENAI_API_KEY=... npm --prefix user-uniapp run review:chatgpt`；输出写入 `/tmp/complens-chatgpt-ui-review.json`。未配置 Key 时脚本会返回 `missing_openai_api_key`，不会伪造评审结果。

OCR 成熟度评测中的 ChatGPT Review 有额外降级规则：如果当前运行环境没有 `OPENAI_API_KEY`，必须使用当前 Codex 会话的子 agent 作为 `chatgpt-subagent` 替代审查方，审查 `reports/ocr/ocr-evaluation-report.json` 和 `reports/ocr_reviews/ocr-review-summary.md`，并把 JSON 结果写入 `reports/ocr_reviews/chatgpt-subagent-review.json`。随后用 `OCR_EVAL_INCLUDE_CHATGPT_SUBAGENT=1 npm run ocr:evaluate` 把该 Review 纳入汇总；未执行子 agent Review 时不得把 ChatGPT Review 视为通过。

OCR 500 样本扩容必须先缓存样本图片：执行 `OCR_EVAL_TARGET=500 npm run ocr:cache-samples`，报告输出到 `reports/ocr/sample-cache-report.md` / `.json`，失败 URL 输出到 `datasets/ocr_samples/labels/sample-cache-failures.json`。只有本地缓存成功样本数达到目标后，才继续执行 `npm run ocr:evaluate`；如果不足 500，需要继续搜索公开样本替换不可达 URL，不能用下载失败样本凑数。

DeepSeek 综合评审入口已提供，但必须由后端/本机运行环境提供 `DEEPSEEK_API_KEY`，不得写入前端、文档或仓库。先运行 `npm --prefix user-uniapp run build:h5 && npm --prefix user-uniapp run visual:smoke` 生成截图，再运行 `npm run user:review:deepseek`；脚本会读取截图清单、当前页面观察、功能路径、产品/设计/文档摘要，输出 `/tmp/complens-deepseek-comprehensive-review.json`。未配置 Key 时脚本会返回 `missing_deepseek_api_key`，不会伪造评审结果。

OCR 成熟度评测必须同时保留 DeepSeek 深度 Review：配置 `DEEPSEEK_API_KEY` 后，`npm run ocr:evaluate` 会通过统一 AI Review provider 对样本的 OCR 原文、`ingredientsText`、`nutritionText`、`otherText`、报告摘要和数据来源做深度审查。DeepSeek Review 平均分低于 85、或 Review 指出配料/营养/来源标注高风险时，不得进入下一阶段。

OCR Provider 抽象已支持 `manual` / `mock` / `aliyun` / `paddleocr` / `rapidocr`。`OCR_PROVIDER=mock` 会返回明确标注为 `provider: "mock"` 的固定测试结果；`OCR_PROVIDER=rapidocr` 会调用本机 `/home/downloads/tools/complens-ocr` FastAPI 服务，不需要 `OCR_API_KEY`。后端现在优先读取 `OCR_LOCAL_URL`，兼容旧变量 `OCR_SERVICE_URL`；推荐本机目标为 `OCR_LOCAL_URL=http://127.0.0.1:18080/ocr`。`OCR_PROVIDER=aliyun` 已接入阿里云通用文字识别，后端需要 `ALIYUN_ACCESS_KEY_ID`/`ALIYUN_ACCESS_KEY_SECRET`（兼容 `OCR_API_KEY`/`OCR_API_SECRET`）；缺少 Key/Secret 时 `POST /api/ocr` 返回 `503 ocr_not_configured`。`paddleocr` 仍是占位 provider，已配置但未适配时返回 `501 ocr_provider_pending`。前端会进入 manual/fallback 确认页，不会伪造 OCR 识别文本。
...
| 正式用户端产品输出审查 | `cd user-uniapp && npm run audit:product-output` / `npm run user:audit:product-output` | ✅ 已实现 |
| 正式用户端 OCR/报告回归 | `cd user-uniapp && npm run regression:ocr-report` / `npm run user:regression:ocr-report` | ✅ 已实现 |
| 正式用户端 H5 视觉烟测 | `cd user-uniapp && npm run visual:smoke` / `npm run user:visual:smoke` | ✅ 已实现（基于已构建 H5 产物） |
| 正式用户端 ChatGPT UI 评审 | `cd user-uniapp && npm run review:chatgpt` / `npm run user:review:chatgpt` | ✅ 已实现（需 `OPENAI_API_KEY`，缺 Key 不伪造结果） |
| 正式用户端 DeepSeek 综合评审 | `cd user-uniapp && npm run review:deepseek` / `npm run user:review:deepseek` | ✅ 已实现（需 `DEEPSEEK_API_KEY`，缺 Key 不伪造结果） |
| OCR 报告级 AI 深度评审 | `npm run ocr:review-report` | ✅ 已实现（需 `DEEPSEEK_API_KEY`；读取最新 500 样本评测、DeepSeek 样本评审和 Codex 子 agent JSON） |
| 商品码报告级 DeepSeek 评审 | `npm run product-code:review-report` | ✅ 已实现（需 `DEEPSEEK_API_KEY`；读取条码 / 二维码 / 商品数字码各 500 用例评测报告） |
| 商品码合成图片级评测 | `npm run product-code:image-evaluate` | ✅ 已实现（条码 / 二维码 PNG 各 500，并链接商品数字码解析 500；不等同于微信真机扫码） |
| 商品码图片级 DeepSeek 评审 | `npm run product-code:image-review-report` | ✅ 已实现（需 `DEEPSEEK_API_KEY`；读取合成图片级评测和 Codex 子 agent JSON） |
...
| 官方材料盘点 | `npm run ingredient:inventory` | ✅ 已实现，扫描 `docs/source-materials/` 并生成 `data/official_sources/manifests/` |
| 官方材料下载/缺失清单 | `npm run ingredient:fetch` | ✅ 已实现，自动下载目录固定为 `docs/source-materials/downloaded/`，缺失项写 `docs/manual-download-required.md` |
| 官方材料抽取/staging | `npm run ingredient:extract` | ✅ 已实现，生成 `data/official_sources/extracted/` 与 `data/official_sources/staging/` |
| source-materials staging 入库 | `npm run ingredient:import-staging` / `npm run ingredient:promote` | ✅ 已实现，只写本地开发 DB，新增数据保持 `pending_review` |
| 食品成分知识库导入 | `npm run ingredient:import` | ✅ 已实现，导入 GB2760 知识库 + source-materials staging，生产需人工确认 |
| pending-review 受控正式化 | `npm run ingredient:promote-reviewed` | ✅ 已实现，只提升已通过报告 review 且满足 S0/证据完整/校验来源条件的数据；官方目录专用表仅允许非 OCR official_catalog 来源 |
| 普通配料 S2 种子导入 | `npm run ingredient:ordinary-seed` | ✅ 已实现，导入 479 条 OCR 高频普通配料候选，全部保持 `pending_review` |
| 食品成分知识库校验 | `npm run ingredient:validate` | ✅ 已实现，默认读取本地 DB；静态校验用 `-- --no-db` |
| 食品成分覆盖报告 | `npm run ingredient:report` / `npm run ingredient:coverage` | ✅ 已实现，生成 `docs/official-food-data-coverage.md` 和旧兼容报告 |
| 食品成分知识库全量更新 | `npm run ingredient:update-all` | ✅ 已实现，串联 inventory/fetch/extract/import/ordinary-seed/validate/coverage |
| 本地数据审核页 | `HOST=0.0.0.0 PORT=8000 node scripts/review-server.mjs` | ✅ 已实现，打开 `http://<本机IP>:8000/docs/review-index.html`，批量审核写入 `docs/review-results.json` / `docs/review-results.md` |
| 本机 OCR 服务启动 | `cd /home/downloads/tools/complens-ocr && .venv/bin/python -m uvicorn app:app --host 127.0.0.1 --port 18080` | ✅ 已实现（本机 RapidOCR） |
| 统一架构目标 OCR 服务启动 | `cd /home/downloads/tools/complens-ocr && .venv/bin/python -m uvicorn app:app --host 127.0.0.1 --port 18080` | ✅ 配置名迁移已完成，后端优先读 `OCR_LOCAL_URL` |
...
构建完成后用微信开发者工具导入 `user-uniapp/dist/build/mp-weixin` 进行平台调试。小程序账号、后端域名和真机验收清单见 `docs/platform/wechat-mini-program.md`。

正式用户端产品输出审查：

```bash
cd user-uniapp
npm run audit:product-output
```

该命令扫描 `user-uniapp/src`，并检查搜索结果、报告描述、OCR 目标字段提取等用户可见输出是否保留“为什么看 / 标签写法 / 数据来自 / 怎么核对 / 待核对线索”等必要结构，同时拦截购买/医疗/强结论、过度安抚和偏专业表达；还会做函数级结构检查，防止报告决策、添加剂识别、关注项命中或过敏提醒直接扫描完整 OCR 原文。根目录可用 `npm run user:audit:product-output` 转发执行。

正式用户端 OCR/报告回归：

```bash
cd user-uniapp
npm run regression:ocr-report
```

该命令通过 Vite 直接加载 `labelTextExtractor` 和 `reportBuilder`，运行非目标包装字段、营养表-only、真实致敏原、待核对添加剂等样本，确认产品名/规格/厂家/裸“含有”声明不进入目标字段，营养表-only 不触发添加剂和过敏提醒，`pending_review` 等待核对项不进入正常添加剂解释。根目录可用 `npm run user:regression:ocr-report` 转发执行。
...
# ChatGPT / OpenAI API 评审，需要本机环境变量 OPENAI_API_KEY
npm run review:chatgpt

# DeepSeek 综合评审，需要本机环境变量 DEEPSEEK_API_KEY
npm run review:deepseek
```

ChatGPT 评审输出 `/tmp/complens-chatgpt-ui-review.json`，DeepSeek 综合评审输出 `/tmp/complens-deepseek-comprehensive-review.json`。两个脚本都只读取运行环境变量，不接受前端传 Key；未配置 Key 时返回 `missing_*_api_key`。
...
当前本机真实 OCR 使用 `/home/downloads/tools/complens-ocr`，生产环境可配置后端阿里云 OCR 凭证后切换为 `OCR_PROVIDER=aliyun`。目录和替换说明见 `INTEGRATIONS.md`。OCR 服务只能由后端调用，不对公网暴露，前端/小程序/App 不直连。
...
上述 `OCR_LOCAL_URL` 已是推荐配置名；当前代码优先读取 `OCR_LOCAL_URL`，未配置时兼容旧的 `OCR_SERVICE_URL`。

直接验证 OCR 服务：

```bash
curl -X POST "http://127.0.0.1:18080/ocr" \
...
GB 2760-2024 官方 PDF 全文转换（需要本机已安装 `poppler-utils` / `pdftotext`，默认读取 `docs/source-materials/GB_2760-2024_食品安全国家标准　食品添加剂使用标准.pdf`）：
...
`ingredient:inventory` 只扫描 `docs/source-materials/`，生成 `data/official_sources/manifests/` 和 checksum；`ingredient:fetch` 的自动下载目录固定为 `docs/source-materials/downloaded/`，不能把官方材料下载到 `docs/` 根目录或 `data/official_sources/`；无法自动获取的官方资料写入 `docs/manual-download-required.md`。`ingredient:extract` 使用 PDF 文本层生成 `data/official_sources/extracted/` 和 `data/official_sources/staging/ingredient-official-staging.json`。`ingredient:import` 先导入现有 GB2760 A.1 / 参考表 / 旧 seed 派生知识库，再导入 source-materials staging；只允许本地数据库 host（`localhost` / `127.0.0.1` / `postgres`），生产导入必须人工确认 `ALLOW_NON_LOCAL_INGREDIENT_IMPORT=1`。这些表是新知识库旁路层，不改变现有 `/api/ingredients` 返回结构，也不破坏当前 OCR 添加剂匹配。

`ingredient:promote-reviewed` 必须在 `docs/review-results.json` 显示本轮报告全部通过后运行。该命令只连接本地开发 DB，筛选 `S0`、`government_public_document`、`verified_by_local_content_and_checksum`、证据字段完整且非 OCR 辅助的数据，把符合条件的 `ingredient_master` 提升为 `source_status=official` / `regulatory_status=verified_regulation`，并同步 `ingredient_source_relations.status=current`、`ingredient_regulatory_rules.status=current`、`nutrition_fortifier_rules.status=current`。官方目录专用表只处理 `source_type=official_catalog` 且非 OCR 的新食品原料、食药物质和通用食品菌种，分别同步 `novel_food_ingredient_rules.status=current`、`food_medicine_rules.status=current`、`microorganism_strains.status=current`。S2 普通配料、OCR 辅助菌种、GB 2760 C.3 非目录菌种引用、数字标签独立公告本地原文未保存项、营养声称阈值、数字标签规则和过敏原规则继续保持 pending。执行后生成 `docs/pending-review-promote-report.md`。
