...
- 正式用户端目标为 `user-uniapp/`（uni-app + Vue3），后台目标为 `admin-web/`（Vue3 + TDesign Web）。这两个目录和命令目前仍是**计划项，待 Codex 实现**，不要当成已存在工程调用。
...
| 正式用户端 H5 启动 | `cd user-uniapp && npm run dev:h5` | 计划命令，待 Codex 实现 |
| 正式用户端微信小程序启动 | `cd user-uniapp && npm run dev:mp-weixin` | 计划命令，待 Codex 实现 |
...
当前命令启动的是历史 Web/PWA 原型（`src/` + Vite）。正式 `user-uniapp/` 用户端尚未创建，对应命令见上方计划项。
