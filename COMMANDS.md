...
| pending-review 受控正式化 | `npm run ingredient:promote-reviewed` | ✅ 已实现，只提升已通过报告 review 且满足 S0/证据完整/校验来源条件的数据 |
...
`ingredient:promote-reviewed` 必须在 `docs/review-results.json` 显示本轮报告全部通过后运行。该命令只连接本地开发 DB，筛选 `S0`、`government_public_document`、`verified_by_local_content_and_checksum`、证据字段完整且非 OCR 辅助的数据，把符合条件的 `ingredient_master` 提升为 `source_status=official` / `regulatory_status=verified_regulation`，并同步 `ingredient_source_relations.status=current`、`ingredient_regulatory_rules.status=current`、`nutrition_fortifier_rules.status=current`。S2 普通配料、OCR 辅助菌种、官方页面 URL 待补齐目录、营养声称阈值、数字标签和过敏原规则继续保持 pending。执行后生成 `docs/pending-review-promote-report.md`。
