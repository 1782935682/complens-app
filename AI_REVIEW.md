...
- 修复 PR 审查反馈：导入本机数据时先丢弃无法解析的 stale 对比项，再按类别最多保留 4 个项目，避免通过 JSON 导入绕过页面上限或让隐藏无效项占满名额。
...
- 补充成分对比路由、渲染、存储、导出导入、导入上限截断、无效导入项过滤和 service worker 缓存清单测试。
