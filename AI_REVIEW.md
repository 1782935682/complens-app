...
11. GB 2760 官方 PDF 表 A.1 已完成第 8-148 页 staging 转换：`src/data/gb2760OfficialStaging.js` 合并人工校对行与 `src/data/gb2760OfficialGeneratedA1Staging.js` 的自动抽取行，保存 2515 行表 A.1 行级抽取记录，后端 `gb2760_official_records` 表和 seed 通路可将这些记录入库；自动抽取行仍需人工审核。
12. staging 数据中 13 行与首批 5 条 `verified_regulation` 的正式 `usageLimits` 对齐，2502 行为 `needs_review`，其中 554 行关联 91 个现有食品添加剂 ID，1961 行尚未匹配本地 ingredient；100 条 seed 中 91 条在官方 PDF 表 A.1 找到可匹配证据并已覆盖，9 条未找到可结构化 A.1 证据；`needs_review` 只代表官方 PDF 原文和页码已抽取，不自动升级正式成分详情或 `isVerified`。
...
| GB 2760 官方 PDF staging | 2515 行可入库 | 覆盖表 A.1 的 PDF 第 8-148 页；13 行已与正式 verified 记录对齐；2502 行为 `needs_review`，覆盖 91/91 条有 A.1 证据的 seed，另有 1961 行未匹配本地 ingredient；自动抽取行仍需人工审核 |
...
| 阻塞项 | 需要确认 |
|---|---|
| GB 2760 条款级核验 | 2502 行 `needs_review` staging 需要人工确认、去重/归并和 ingredient 匹配后才能进入正式 `ingredients.usageLimits` |
...
本轮 GB 2760-2024 官方来源、首批 PDF 条款级数据、2515 行 staging 数据、100 条 seed 的 A.1 覆盖审计和 264 页 PDF 全文转换已通过完整验证链；表 A.1 第 8-148 页已转换到 staging：
...
cd backend && npm run db:seed -- --version gb2760-staging-coverage-20260613 --reviewed-by codex --change-note "GB 2760 official staging seed coverage expansion"
