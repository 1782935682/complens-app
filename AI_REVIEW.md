...
- 修复 PR 审查反馈：读取和导入本机数据时先丢弃无法解析的 stale 对比项，再按类别最多保留 4 个项目，避免通过 JSON 导入绕过页面上限或让隐藏无效项占满名额。
...
- 补充成分对比路由、渲染、存储、导出导入、导入上限截断、无效导入项过滤、旧版脏 localStorage 过滤和 service worker 缓存清单测试。
...
- 新增对比服务和对比页。
- 扩展本机数据快照和设置页摘要。
- 修复导入快照绕过对比数量上限、无效对比项占用名额、旧版脏 localStorage 阻塞新增的问题。
- 更新 PWA 预缓存与测试覆盖。
