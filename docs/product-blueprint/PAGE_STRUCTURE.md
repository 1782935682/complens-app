...
> 2026-06-19 MVP 收缩口径：微信小程序用户端品牌暂定「成分镜」，首屏以单一“拍标签”入口为核心；P0 只承诺食品标签 OCR、文本清洗、有效标签提取、文本确认、本地规则识别和可读解读结果。OCR 成功后仍必须进入用户文本确认，不因识别清楚而跳过。商品正面识别、条码识别、商品库优先匹配、商品对比和日化识别均后置到 P1 / P2，不作为当前主流程入口。首页 Demo 仅使用内置食品标签文本体验本地规则解读，不代表商品库能力。
...
| 拍食品标签 | 合并拍照、相册、OCR、OCR adapter、文本清洗、配料/营养/致敏原提取、手动输入、文本确认、阶段式 loading 和结果生成；OCR 后文本确认必经，只处理食品标签，不走商品库命中或条码识别 | `user-uniapp/src/pages/capture/index.vue` / `/pages/capture/index` | 核心注册页 |
| 食品标签解读 | 第一屏展示“结果摘要”和大众化重点提醒；有清晰配料/营养时展示添加剂大白话、配料表、未确认线索、识别文字、收藏、分享和历史入口；信息不足时只展示补充提示和识别文字 | `user-uniapp/src/pages/report/index.vue` / `/pages/report/index` | 核心注册页 |
...
- **核心组件**：一句话结论、我的关注项、重点提醒、配料表解读、营养成分解读、包装卖点提示、食品添加剂分组、过敏 / 忌口提示、暂未识别 / 暂未收录、数据来源和查看依据。
...
| --- | --- | --- | --- |
| 扫描记录 | 查看扫描会话 | Beta | 计划 |
| OCR 记录 | OCR 识别文字、provider、耗时、失败原因 | MVP | 计划 |
...
| 页面 | 页面目标 | 阶段 | 当前状态 |
| --- | --- | --- | --- |
| 功能开关 | OCR、AI、对比、卖点提示、订阅入口 | MVP / 产品化 | 计划 |
