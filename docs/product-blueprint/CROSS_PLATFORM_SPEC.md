...
- **正式用户端目标栈**：`user-uniapp`，uni-app + Vue3，目标覆盖 H5/PWA、微信小程序、Android、iOS。
...
- **微信小程序尚未开始**（待办 / 待确认），后续应由 `user-uniapp` 承载，不在旧 `src/` 上补小程序适配。
...
| --- | --- | --- | --- | --- | --- |
| 拍照 | 支持（正式 user-uniapp H5；历史原型 file input） | 后续（user-uniapp） | 后续（user-uniapp）/ 历史 Capacitor 已有原型 | 后续（user-uniapp）/ 历史 Capacitor 已有原型 | 不支持（无需） |
| 相册上传 | 支持（正式 user-uniapp H5；历史原型 file input） | 后续（user-uniapp） | 后续（user-uniapp） | 后续（user-uniapp） | 不支持（无需） |
| 图片压缩 | 支持（历史原型已有） | 后续 | 后续 | 后续 | 不支持（无需） |
| OCR | 支持（经后端） | 后续（经后端） | 后续（经后端） | 后续（经后端） | 部分支持（审核查看，非主流程） |
| 本地缓存 | 支持（历史原型 IndexedDB；正式端待 uni-app 适配） | 后续（平台文件） | 后续（平台文件） | 后续（平台文件） | 后续 |
| 历史记录 | 支持（本地/登录后同步） | 后续 | 后续 | 后续 | 部分支持（管理视角） |
| 分享 | 支持（历史原型 Web Share / 复制；正式端待适配） | 后续（原生转发） | 后续 | 后续 | 不支持（无需） |
