...
- **Web / PWA 为主**，已具备 PWA 安装 / 离线基础。
- **移动端**通过 **Capacitor 7.x** 接入原生能力：
...
1. 当前优先把 **Web / PWA 主流程跑通**。
2. 后续以**统一设计系统 + 统一 API** 迁移到微信小程序 / Android / iOS。
3. 管理后台**单独建设**，不复用消费端的拍照主流程。
...
6. 前端不能暴露 OCR Key / AI Key；OCR 服务由后端代理调用，不直接暴露公网给用户端。
7. 小程序 / App 代码不得假设存在 `window` / `document` / `navigator`；所有平台能力必须通过适配层隔离。
...
### 1.1 Web / PWA
- 拍照 / 选图：使用浏览器 `file` / `camera` input（`<input type="file" accept="image/*" capture>`）。
...
### 1.3 Android App（Capacitor）
- 拍照 / 选图：`@capacitor/camera` 提供原生相机 / 相册。
...
> 注意：当前抽象覆盖 **Web 降级 + Capacitor 原生**。**微信小程序适配尚未纳入** `nativeBridgeService`（待补 / 待确认）。
...
- **图片上传后由后端调用 OCR**，前端不直接做最终 OCR 决策。
...
| --- | --- | --- | --- | --- | --- |
| 拍照 | 支持（file input camera） | 后续（待确认） | 支持（@capacitor/camera） | 支持（@capacitor/camera） | 不支持（无需） |
| 相册上传 | 支持（file input） | 后续（待确认） | 支持 | 支持 | 不支持（无需） |
| 图片压缩 | 支持 | 后续 | 支持 | 支持 | 不支持（无需） |
| OCR | 支持（经后端） | 后续 | 支持（经后端） | 支持（经后端） | 部分支持（审核查看，非主流程） |
| 本地缓存 | 支持（IndexedDB） | 后续（平台文件） | 部分支持（Filesystem，统一策略待确认） | 部分支持（Filesystem，统一策略待确认） | 后续 |
| 历史记录 | 支持 | 后续 | 支持 | 支持 | 部分支持（管理视角） |
| 分享 | 支持（Web Share / 复制） | 后续（原生转发） | 支持（@capacitor/share） | 支持（@capacitor/share） | 不支持（无需） |
...
> 说明：标注为“后续 / 待确认”的项均为尚未建设，不得在实现前当作“支持”。微信小程序整体处于“后续 / 待确认”阶段；支付为“后续 / 后置”；推送为“后续”。
