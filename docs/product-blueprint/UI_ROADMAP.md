...
> - [`ARCHITECTURE_SPEC.md`](./ARCHITECTURE_SPEC.md)：统一技术栈、旧前端迁移、后台和 OCR 架构
...
- **正式用户端路线已调整**：新复杂用户端功能按 `user-uniapp`（uni-app + Vue3）规划；当前 `src/` Vite 前端作为历史 Web/PWA 原型和迁移来源。旧 `src/` 可修 blocker、数据可信和 OCR 降级问题，但不继续堆复杂新业务。
- **后台路线已调整**：后台管理端为独立 `admin-web`（Vue3 + TDesign Web），覆盖产品运营、数据治理、业务监控、系统配置、权限与审计；不复用消费端页面。
- **后端唯一入口**：所有端通过现有 `backend/` API 访问 OCR/AI/数据库；前端、小程序、App、后台不直连本地 OCR 服务或数据库。
...
**建议顺序**：0.统一跨端技术栈规划与迁移边界 → 1.移动端首页 → 2.拍照/上传页 → 3.标签类型识别 → 4.OCR 文本确认页 → 5.配料拆分页 → 6.营养成分表解析 → 7.我的关注项 → 8.食品标签解读报告页 → 9.历史记录页 → 10.成分详情页 → 11.数据说明页 → 12.包装卖点提示（后续） → 13.两款商品对比（后续） → 14.后台管理端（分期） → 15.微信小程序适配 → 16.Android/iOS App 适配。
...
- **目标**：确认 `backend/`、`src/`、`ios/`、`android/` 的真实现状，输出迁移边界。
- **涉及页面 / 目录**：`src/`、`backend/`、`ios/`、`android/`、计划 `user-uniapp/`、计划 `admin-web/`。
- **验收标准**：不删除旧前端；不重复创建后端；不把计划目录写成已存在。
- **是否需要人工**：否。
- **是否阻塞后续**：是，阻塞正式用户端和后台新工程初始化。
- **Codex 可执行任务**：执行 `STACK-A`，只做结构盘点和迁移方案，不重构业务代码。

### S2：用户端 uni-app 初始化规划

- **目标**：规划 `user-uniapp`，面向 H5/PWA、微信小程序、Android、iOS。
- **涉及页面 / 组件**：首页、拍照/上传、标签类型选择、文本确认、关注项、报告、历史。
- **验收标准**：不假设 `window/document/navigator`；图片上传前压缩；所有 API 走后端。
- **是否需要人工**：需要用户确认启动时机。
- **是否阻塞后续**：阻塞正式跨端实现，但不阻塞旧原型修 blocker。
- **Codex 可执行任务**：执行 `STACK-B`，先规划工程结构和迁移清单。

### S3：后台 admin-web 分期规划

- **目标**：规划独立后台，按 MVP / Beta / 产品化 / 上架商业化分期。
- **涉及页面**：Dashboard、用户与会员、内容运营、食品标签业务、数据治理、OCR/AI/Provider、系统配置、权限与审计。
- **验收标准**：MVP 不塞满所有后台模块；会员/订阅/支付账号依赖标记人工阻塞。
- **是否需要人工**：后台启动时机、支付/商店账号、法务材料需要人工。
- **是否阻塞后续**：不阻塞消费者主路径。
- **Codex 可执行任务**：执行 `ADMIN-A` 到 `ADMIN-H`，先做文档和 API 契约，再按阶段落地代码。

ADMIN-A 收口（2026-06-15）：

1. 后台一级导航固定为 Dashboard、数据治理、食品标签业务、用户与会员、内容运营、OCR/AI/Provider、系统配置、权限与审计。
2. MVP 只强制数据治理、OCR/反馈回溯、基础配置和管理员登录预留；用户 CRM、内容运营、会员订阅、订单支付、AI 成本、RBAC 和上架合规均按 Beta / 产品化 / 上架商业化推进。
3. `admin-web/` 仍为计划目录，ADMIN-A 不创建工程、不新增路由实现、不把任何后台页面写成已落地。
4. 后续 ADMIN-B 优先细化数据治理后台 MVP 的页面/API 复用，尤其是 `/api/gb2760/*`、`/api/ingredients*` 与 `pending_review` 展示边界。
...
- **验收标准**：字段可编辑；解析失败可手动修正；信息不足时提示请补充标签文字后查看结果。
- **是否需要人工**：否。
...
- **验收标准**：报告名为“食品标签解读”；第一屏展示一句话结论和重点提醒；专业依据默认折叠。
- **是否需要人工**：需要一次产品文案确认。
- **是否阻塞后续**：是，消费者主路径核心。
...
- **验收标准**：
  1. 报告按“一句话结论 / 我的关注项 / 重点提醒 / 配料表解读 / 营养成分解读 / 数据来源”组织；
