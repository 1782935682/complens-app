...
- OCR 服务：Python FastAPI + RapidOCR，本地地址目标为 `http://127.0.0.1:18080/ocr`，后端通过 `OCR_LOCAL_URL` 调用，并兼容旧的 `OCR_SERVICE_URL`。
...
7. **消费者默认视角**：页面默认展示食品标签解读和重点提醒，专业法规/数据库字段默认折叠到“数据来源和查看依据”。
...
user-uniapp/
  src/
    pages/       已存在  首页、拍照/上传、报告、关注项、搜索；历史/收藏本期不注册
...
| 历史 | 后续能力，本期不注册 | `#/history` / `#/food/history` |
...
| 收藏 | 后续能力，本期不注册 | `#/favorites` / `#/food/favorites` |
...
| `front_claims` | 包装正面卖点 | 信息不足报告内保留为包装声明线索，可由用户主动补充 |
| `barcode_or_product` | 条码 / 产品名 | 后续能力，MVP 不强制 |
| `unknown_label` | 未知标签 | 展示类型选择页，允许重新拍 |

数字标签二维码是后续扫描对象类型，不属于当前已实现 `labelType`；落地前需先完成共享 API / 前端 / 后端 schema 迁移，或通过独立 `scanObjectType` 表达。

系统无法判断时，前端直接生成信息不足报告，并在报告内提供补拍配料表、补拍营养成分表和手动补充入口；不得在识别结束后默认进入确认页。

```
idle
  └─选图 / 拍照─> selectingImage
                    └─选中─> recognizing
                                   ├─清晰可生成─> generatingReport ─> reportReady
                                   ├─商品身份线索─> aiPublicLabelLookup ─> generatingReport ─> reportReady
                                   ├─低置信 / 信息不足─> generatingInsufficientReport ─> reportReady
                                   └─失败─> generatingInsufficientReport ─> reportReady
reportReady
  ├─补拍配料 / 营养表─> selectingImage
  ├─手动粘贴文字─> manualInput
  └─返回首页
manualInput（仅用户主动触发）
  ├─补充配料 / 营养 / 包装声明─> generatingReport ─> reportReady
  └─重新拍─> selectingImage
failed
  ├─重试─> 回到失败发生的环节
  ├─手动粘贴文字─> manualInput
...
| `generatingInsufficientReport` | 低置信、失败或信息不足时自动生成信息不足报告，展示可补拍 / 手动补充动作 |
| `manualInput` | **手动补充页**：仅用户主动点击手动输入或报告内补充时展示可编辑文本 |
...
强制约束：
- **OCR 失败不中断流程**：失败后直接生成信息不足报告，并保留补拍 / 手动粘贴文字入口。
- **识别结束直接生成报告**：清晰 OCR / 商品身份识别 / AI 公开标签线索直接进入报告；低置信、失败或信息不足也生成信息不足报告。只有用户主动手动输入或从报告点击补充时才进入 `manualInput`。
...
### 9.1 禁止文案（强制）
任何文案、状态、报告中**禁止**出现：可以买、不能买、健康、不健康、安全、有害、致癌、治疗、诊断、一定过敏、绝对安全、绝对有害、一定致敏、一定不能吃、有毒。改用相对、可追溯、带来源与不确定性的表述，例如“建议关注”“更适合重点查看”“部分人群可能需要留意”“信息不足，请补充标签文字后查看结果”。
...
4. **历史报告**：本期不外露历史 / 收藏入口；后续重启时再开放本地历史与云同步迁移。
