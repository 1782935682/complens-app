...
- 主路径（不可偏离）：首页 → 拍食品标签 → 标签类型识别 → OCR / 商品身份识别 → 自动生成食品标签解读报告（信息足够时输出解读，低置信 / 失败 / 信息不足时输出信息不足报告）；用户主动手动输入或从报告补充时进入文本补充。本期不验收历史 / 收藏入口。承载页面见 `PAGE_STRUCTURE.md`。
...
- 移动端通用约束：尊重安全区（顶部刘海 / 底部 Home 条）；禁止整页横向滚动；可点击控件命中区域 ≥ 44px；当前微信小程序 P0 底部主导航为「首页 / 我的」，成分查询作为弱辅助入口，历史和设置本期不外露。
...
- [ ] loading：OCR 识别中有进度态。
- [ ] empty：未选择图片时有引导态。
- [ ] error：相机权限被拒时给出"重试"与"改为手动输入"两条出路；OCR 失败时不崩溃，直接生成信息不足报告并提供补拍 / 手动补充动作。
- [ ] 可信：OCR / AI 输出不得在扫描页或报告页展示为权威结论；AI 搜索只能标注为公开标签线索，不能伪装成包装实拍 OCR。
- [ ] 拍照识别结束不进入文本确认页：清晰 OCR 直接自动报告；只有商品名、品牌、商品码或二维码时先 AI 搜索公开标签线索，搜到配料 / 营养则生成带 AI 来源标注的参考报告，搜不到、低置信、OCR 失败或未知图片时生成信息不足报告；只有用户主动手动输入 / 报告补充时进入文本补充。
- [ ] OCR 文本提取只把配料表、营养成分表、致敏原提示和明确包装声明作为目标字段；产品名、净含量、规格、厂家、地址、电话、生产日期等非目标字段只可保留在识别详情，不得进入配料/营养判断。
- [ ] 条码、二维码和数字编码可作为商品身份信息保存和历史复用；不得伪装成配料表、营养成分表或官方法规依据。
...
- [ ] 数字标签二维码当前作为 `barcode_or_product` 商品身份线索和 `recognition.detectedType=qrcode` 展示；正式数字标签结构化解析需通过后续独立扫描对象类型或 schema 迁移后再验收。
- [ ] 低置信或未知标签时直接生成信息不足报告，并在报告中提供补拍配料表 / 营养成分表或手动补充动作。
- [ ] 标签类型判断不展示为权威结论。
- [ ] 用户从报告主动补充时进入对应文本补充；重新拍保持单一拍包装入口。
...
- [ ] 成分查询只作为辅助入口，不替代拍食品标签 → OCR / 商品身份识别 → 自动报告（足够信息或信息不足）→ 报告主路径；文本补充只由用户主动触发。
...
- [ ] 本期微信小程序不注册历史页和收藏入口；后续重启该能力时再恢复历史 / 档案验收。
- [ ] 历史 / 产品档案能力重新开放后，报告保存后应可检索回看（历史支持 `?q=` 与 `?filter=`，过滤值经 `normalizeHistoryFilter`）。
- [ ] 历史能力重新开放后，再验收全部 / 收藏筛选、报告页和历史卡片收藏切换。
- [ ] loading：列表加载占位。
- [ ] empty：无历史 / 无档案时引导首拍。
- [ ] error：读取失败可重试。
...
- [ ] 从首页可一路走到食品标签解读报告或信息不足报告，无死路；本期不要求保存历史。
- [ ] 拍照识别结束均自动生成报告：清晰 OCR 或 AI 公开标签线索生成参考解读；低置信、失败、生成失败或信息不足生成信息不足报告；手动输入或报告内补充才进入文本补充。
...
- [ ] 拍照识别结束未进入确认页；低置信 OCR、失败或信息不足场景以信息不足报告呈现，手动输入和报告内补充才进入文本补充；AI 公开标签线索必须明确来源标注。
...
| --- | --- | --- | --- | --- | --- |
| 主路径走通（首页→报告 / 信息不足报告） | 必查（`user-uniapp` H5） | 真机待验 | 真机待验 | 开发者工具/真机待验 | 不适用 |
...
- [ ] OCR 失败必须降级为信息不足报告并保留手动补充入口；低置信或信息不足不进入确认页；清晰可生成的 OCR 成功结果自动生成报告。
...
- [ ] OCR 成熟度 AI Review：执行 OCR 成熟度评测时，若没有 `OPENAI_API_KEY`，必须使用当前 Codex 会话子 agent 输出 `reports/ocr_reviews/chatgpt-subagent-review.json` 并用 `OCR_EVAL_INCLUDE_CHATGPT_SUBAGENT=1` 纳入汇总；同时必须执行 DeepSeek 深度 Review（需要 `DEEPSEEK_API_KEY`），Review 平均分低于 85 或存在高风险问题时不得进入下一阶段。
- [ ] H5 视觉烟测：涉及核心页面、样式、路由或报告结构时，先 `cd user-uniapp && npm run build:h5`，再执行 `npm run visual:smoke`，确认首页、拍食品标签、搜索、我的、报告不白屏、不横向溢出且关键文案存在。
...
| `npm run ocr:cache-samples` | OCR 公开样本图片预缓存与可达性校验 | 100/500 样本扩容、网络样本替换 | 本地缓存成功样本数达到目标；失败 URL 单独报告并替换，不能进入 OCR 指标 |
| `npm run ocr:evaluate` | OCR 成熟度评测（样本 OCR、分区、报告、自动评测、AI Review 汇总） | OCR 分区能力、样本扩容、500 样本验收 | 本地指标达标；无 `OPENAI_API_KEY` 时必须用 Codex 子 agent 作为 `chatgpt-subagent` 并设置 `OCR_EVAL_INCLUDE_CHATGPT_SUBAGENT=1` 纳入汇总；DeepSeek 深度 Review 平均分需 >=85；未达标时报告必须写明不能声明完成 |
...
- OCR 拍照入口、信息不足报告、文本补充页、manual / fallback 降级与响应格式校验；AI / OCR 未配置时的降级返回。
