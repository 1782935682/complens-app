...
- 主路径（不可偏离）：首页 → 拍食品标签 → 标签类型识别 → OCR / 商品身份识别 → 清晰可生成时自动报告；低置信、失败、手动输入或信息不足时文本确认 / 补充 → 配料拆分 / 营养字段解析 → 我的关注项 → 食品标签解读报告 → 保存历史 / 收藏。承载页面见 `PAGE_STRUCTURE.md`。
...
- [ ] 清晰可生成的 OCR / 商品身份识别结果自动进入报告；只有商品名、品牌、商品码或二维码时可先 AI 搜索公开标签线索，搜到配料 / 营养则生成带 AI 来源标注的参考报告，搜不到或手动输入 / 生成失败时进入信息不足 / 文本确认 / 补充页。
...
- [ ] 成分查询只作为辅助入口，不替代拍食品标签 → OCR / 商品身份识别 → 自动报告或文本确认 / 补充 → 报告主路径。
...
- [ ] 从首页可一路走到保存历史，无死路。
- [ ] 清晰可生成的 OCR 或 AI 公开标签线索可自动生成报告；低置信、失败、手动输入、生成失败或信息不足必须进入文本确认 / 补充。
...
- [ ] 手动输入、低置信 OCR、失败或信息不足场景未跳过文本确认 / 补充；自动报告只适用于清晰可生成的 OCR 或已标注来源的 AI 公开标签线索。
...
- [ ] OCR 失败必须降级到手动输入；低置信或信息不足必须进入文本确认 / 补充页；清晰可生成的 OCR 成功结果允许自动生成报告。
