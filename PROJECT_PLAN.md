...
> 整体产品进度：**32%**（口径：iOS/Android 可上线 + 可付费订阅）

| 里程碑 | 名称 | 状态 | 完成度 |
|---|---|---|---|
| M0 | 口径校准与开发规则 | ✅ 已完成 | 100% |
| M1 | Web/PWA 可用 MVP + 食品数据基础 | 🔄 进行中 | ~62% |
| M2 | 移动端 App 基座（Capacitor/原生工程） | 🔄 进行中 | ~23% |
| M3 | 后端、账号与云同步 | 🔄 进行中 | ~40% |
...
**当前瓶颈（P0）：** 移动端技术基座 → 订阅支付 → 合规材料。前端 Web 原型仍在 M1 迭代，M2 已完成 Vite、Capacitor 基础脚手架、原生 Camera/Share 适配层和 iOS 权限说明文档，M3 已完成 Hono API 骨架、食品添加剂数据库 schema、迁移、seed、只读成分 API、基础账号鉴权和用户数据同步 API，但前端登录 UI、订阅权益、支付和 M4–M7 仍未开始，决定整体进度的是 P0 基础设施，不是前端页面数量。

**当前最早未完成任务**：见 [`CODEX_TASKS.md`](./CODEX_TASKS.md) → `Batch 2-D：iOS 工程配置 [人工]`；下一个 Codex 批次为 `Batch 4-A：套餐设计与权益服务`
...
当前阶段：第二阶段 / 移动端 App 基座已开始，后端基础设施起步，付费能力未开始

当前判断：项目已经有可继续迭代的 Web 原型、部分食品添加剂数据结构和 Vite 前端工程化基座，但距离 iOS/Android 上线和付费产品还有明显差距。现在不能按“可上线 App”口径评估为接近完成。

真实情况：

- 已有 `backend/` Hono/TypeScript API 骨架、健康检查、CORS、请求日志、全局 JSON 错误处理、Docker、Drizzle/PostgreSQL、只读成分 API、基础账号鉴权 API 和 Vitest 测试。
- 已有食品添加剂数据库 schema、迁移和本机 seed，并新增用户收藏、历史、过敏原和报告同步表；仍没有订阅权益、OCR/AI 任务数据和生产部署配置。
- 账号系统已完成注册、登录、退出、`me`、账号删除和 Bearer 鉴权下的用户数据同步后端接口，但没有前端登录页、刷新 token 或匿名升级。
- 没有 iOS/Android 工程、证书、原生移动端构建和商店元数据。
- 没有 Apple IAP、Google Play Billing、订阅恢复和服务端权益校验。
- 没有真实 OCR。
- 没有真实 AI 分析。
- 没有真实成分数据来源和审核流程。
- 食品添加剂本地库已有 100 条草稿种子数据，但仍缺逐食品类别限量和来源条款审核；化妆品仍是原型示例数据。
...
2026-06-12 按新目标重新校准：主线仍是食品添加剂，但产品范围扩大为 iOS/Android 上线和付费订阅。前端基础框架（路由、搜索、收藏、存储）可复用；食品添加剂 schema、过敏原枚举、过敏原设置页、首次启动引导、本机数据导出/导入/清空、搜索历史隐私开关、数据校验脚本、100 条食品添加剂草稿种子数据、食品数据审核摘要展示、数据来源与审核状态页、食品详情字段基础展示、详情页相关成分推荐、详情页功能分类导航、详情页和数据页纠错反馈预填入口、首页分类筛选入口、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、搜索拼音/首字母/常见误写/近似纠错辅助、搜索无结果热门分类入口、食品分析类别化 summary、分析页空输入防御、真实包装成分表括号复配解析、分析置信度和解析质量面板、基础扫描页、扫描图片类型/大小校验、图片预览旋转/移除、扫描草稿本地保存/恢复、OCR 未接入手动输入提示、Vite 前端构建和 dev server、Capacitor 基础配置与同步命令、Capacitor Camera/Filesystem/Share 插件、扫描页原生相机/相册入口与 Web 文件选择降级、原生分享/Web Share/复制分层降级、viewport-fit 与安全区底部适配、iOS 权限描述文档、PWA public 静态资源、service worker 离线 app shell、基础 404、详情缺失成分友好页、页面标题、SPA 焦点管理、搜索筛选器、类别感知顶部导航、最近查询单条删除、收藏空态搜索入口、本地分析报告保存、报告历史检索、报告 v2 解读区块、报告来源引用和 Markdown/JSON 导出、详情/报告/成分对比原生分享 / Web Share / 复制 fallback、会员中心前端信息架构和订阅操作未接入提示、支持中心和本机反馈记录、隐私与条款合规材料入口、成分对比本机列表和横向对比页、Hono/TypeScript 后端骨架、健康检查、CORS、请求日志、全局 JSON 错误处理、后端 Docker/Compose、Drizzle 成分库 schema、迁移、100 条食品添加剂 seed、只读成分 API、基础账号鉴权 API、JWT middleware、收藏/历史/过敏原/报告同步 API、前端 storage 登录态后台同步和 Vitest 后端测试已完成。新增口径下，iOS/Android 工程人工配置、前端登录 UI、订阅购买、服务端权益校验、商店审核、支付沙盒、真机测试和正式上线合规均未开始，因此总进度仍处于早期。
...
|---|---:|---:|---:|---|---|
| 食品添加剂数据与内容治理 | 18% | 30% | 5.40% | 已定义 `FoodAdditive` schema、数据校验脚本、来源聚合、版本聚合、数据来源与审核状态页和 100 条草稿种子数据，达到 MVP 目标数量并覆盖防腐剂、甜味剂、着色剂、增稠剂、乳化剂、膨松剂、抗氧化剂、香料类、磷酸盐类、糖醇类等常见类别 | GB 2760 逐食品类别限量、来源条款逐条审核、ADI 原文核验、内容版本管理 |
| 核心查询与分析体验 | 14% | 64% | 8.96% | 已有搜索、搜索建议、搜索结果分页、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、搜索拼音/首字母/常见误写/近似纠错辅助、搜索无结果热门分类入口、成分对比本机列表、搜索/详情/收藏加入对比入口、横向对比页、详情/报告/成分对比原生分享 / Web Share / 复制 fallback、首页/搜索/详情食品数据审核状态提示、详情页和数据页纠错反馈预填入口、详情缺失成分友好页、首次启动引导、会员中心前端闭环、支持中心本机反馈闭环、数据来源与审核状态页、详情、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、食品/化妆品类别化文本分析 summary、分析页空输入防御、真实包装成分表前缀/复配括号/剂量后缀解析、分析置信度和解析质量面板、扫描页图片预览、旋转预览、移除图片、校正入口和草稿恢复、OCR 未接入手动输入提示、分析页 AI 协议状态与本地 fallback 展示、本地报告保存、报告列表/详情、报告历史检索、报告解读区块、报告来源引用、Markdown/JSON 导出、本机数据导出/导入/清空、搜索历史隐私开关、收藏、收藏空态搜索入口、历史、筛选、类别路由、移动端底部主导航、PWA 安装元数据、service worker 离线 app shell 和基础渲染测试 | 移动端主流程重设计、大规模搜索索引、异常边界 |
...
| 移动端 App 工程与跨端体验 | 12% | 23% | 2.76% | 已有 Vite 前端工程化基座、Capacitor 依赖与 `capacitor.config.json`、Camera/Filesystem/Share 插件、原生相机/相册适配层、原生分享适配层、Web 降级、viewport-fit、安全区底部适配、iOS 权限描述文档、Web 小屏底部主导航、类别感知链接、独立扫描路由、首次启动引导页、成分对比页移动端布局、PWA manifest 和 service worker 离线 app shell | iOS/Android 工程人工配置、深链、推送、真机适配、商店内测 |
| 账号、订阅与付费权益 | 10% | 15% | 1.50% | 已有前端会员中心信息架构、Free/Pro 套餐说明、本机用量展示、购买/恢复/管理订阅未接入提示；后端已完成 `users` / `sessions`、注册、登录、退出、`me`、账号删除和登录态用户数据同步基础接口 | 前端登录、刷新 token、匿名升级、Apple IAP、Google Play Billing、真实恢复购买、服务端权益校验 |
| 后端 API、同步与安全基础设施 | 12% | 40% | 4.80% | 已有 `backend/` Node.js 20 + TypeScript + Hono 项目骨架、`GET /health`、CORS、请求日志、全局 JSON 错误处理、`.env.example`、Dockerfile、PostgreSQL 15 compose、Drizzle `ingredients` / `ingredient_sources` / `users` / `sessions` / `user_favorites` / `user_history` / `user_allergens` / `user_reports` schema、迁移、100 条食品添加剂 seed、`/api/ingredients` 列表/分类/详情 API、`/api/auth/*` 鉴权 API、`/api/user/*` 同步 API 和 Vitest 测试 | 对象存储、队列、限流、审计日志、密钥管理、生产部署 |
| OCR 与 AI 分析能力 | 8% | 12% | 0.96% | 有扫描页 Web 文件选择、原生相机/相册入口、失败降级、类型/大小校验、预览、旋转预览、移除图片、OCR 请求/响应协议、OCR 响应校验、OCR 本地 fallback、OCR 协议状态展示和未接入手动输入提示、可编辑校正文本、草稿保存/恢复、本地文本分析、AI 分析请求/响应协议、AI 响应校验、本地 fallback 和分析页 fallback 展示 | 真实 OCR、图片预处理、服务端模型代理、真实 AI 调用、结构化报告、成本控制 |
| 测试、CI、质量与可观测性 | 8% | 62% | 4.96% | 有本地 lint/test/Vite build、数据校验、CI Node 20.19 依赖安装与 Vite/public/backend 触发路径、首次启动引导测试、会员中心路由/渲染/服务测试、支持中心路由/渲染/本机反馈/预填反馈测试、隐私与条款路由/渲染/入口测试、成分对比路由/渲染/本机存储/分享测试、移动端导航链接测试、PWA service worker 注册和缓存清单测试、Capacitor 7 插件依赖断言、Camera/Share 非 native 降级测试、viewport-fit/安全区/iOS 权限文档断言、数据来源页路由/渲染/来源聚合/纠错入口测试、本机数据摘要/导出/导入/清空测试、搜索历史隐私开关测试、storage 未登录本地回退和登录态后台同步测试、扫描页路由与渲染测试、分析页空输入、详情缺失成分、收藏空态和搜索无结果热门分类测试、搜索建议、拼音/首字母/常见误写/近似纠错辅助、分页、排序、风险分布、分类分布、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、成分表复配括号/剂量后缀解析、分析置信度和解析质量面板测试、数据审核摘要渲染、本地报告保存/导出/导入/检索/解读区块/来源引用/分享测试、旧版报告兼容测试、OCR/AI 协议/响应校验/fallback/页面展示测试、后端 health/CORS/404/500、ingredients API、auth API、user sync API 与 JWT middleware Vitest 测试、后端脚手架静态断言和 GitHub Actions 工作流 | E2E、移动端真机测试、支付沙盒测试、崩溃监控、覆盖率 |
...
| 成分搜索 | 原型完成（逻辑可复用） | 47% | 中文名、英文名、别名、拼音、首字母、常见误写、近似纠错、分类、描述和食品 E-number/INS 编码本地搜索，支持按类别切换数据集、搜索建议、搜索结果分页、搜索结果排序、风险分布筛选、分类分布筛选、首页分类筛选入口、搜索无结果热门分类入口、搜索结果加入对比，并支持风险等级和成分分类组合筛选；食品搜索页显示草稿数据审核状态和“可能相关”候选 | 大规模索引、真实搜索日志召回评估、更多食品数据搜索样例、移动端筛选交互 | 算法可复用，食品数据仍是草稿库 |
| 成分详情 | 原型完成（食品字段基础接入） | 31% | 展示名称、英文名、别名、功能、风险说明、收藏按钮、加入对比按钮和分享按钮；食品详情页已展示 E-number、INS、ADI、适用食品类别、GB 状态、草稿状态、来源链接、草稿审核提示和相关成分推荐，功能分类芯片可跳转到搜索筛选或关键词搜索，并可一键带入数据纠错反馈草稿；缺失成分 ID 会进入暂未收录友好页并带搜索和数据纠错预填入口 | 精确使用限量、字段解释、来源可信度标识、相关产品、来源可信度说明 | 食品字段、基础关联推荐、详情页横向浏览入口、分享入口和纠错入口已有展示，但还不是完整法规详情页 |
| **食品添加剂数据** | **草稿数量达到 MVP 目标，治理入口基础完成，内容审核未完成** | **30%** | 已定义 `FoodAdditive` 类型、`src/data/foodAdditives.js` 含 100 条草稿种子数据、`npm run validate:data` 校验脚本、来源/版本聚合和数据来源与审核状态页 | GB 2760 逐食品类别限量、E-number 对照审核、ADI 原文核验、真实过敏原标注审核 | **主线核心已有 100 条草稿规模和透明状态入口，但还不是可正式上线的数据集** |
| **过敏原用户档案** | **基础完成，登录态同步起步** | **30%** | 已定义 EU 14 类标准过敏原枚举 `src/data/allergens.js`，提供 `getUserAllergens()` / `setUserAllergens(ids)`，并新增首次启动引导和设置页选择、保存、清空过敏原、本机数据导出/导入/清空；后端已提供 `/api/user/allergens`，前端 storageService 在登录态可后台同步 | 设置页说明增强、更多地区标准映射、真实跨设备验收 | 单机可用，并已有登录态云同步基础；仍缺前端登录 UI 和真机/跨设备验收 |
| **过敏原全局警告** | **基础逻辑完成，真实数据待审核** | **4%** | 搜索结果、详情页、分析结果已接入过敏原匹配和警告展示逻辑；100 条食品种子数据中已草稿标注大豆/蛋/亚硫酸盐相关项 | 食品数据 `allergenTypes` 逐条审核、视觉验收 | 逻辑已接入，但标注仍是草稿级别 |
| 成分表文本分析 | 基础完成（真实包装格式起步） | 33% | 按分隔符拆分，支持 `配料：` 等标签前缀、复配添加剂括号展开、剂量/百分比后缀清理、单/双甘油脂肪酸酯保护，匹配本地库并展示匹配原因、置信度、覆盖率、低置信和暂未收录核对提示；分析页空输入提交会提示且不跳转；食品/化妆品 summary 使用类别化语境，支持保存本地报告快照、导出 Markdown/JSON 和分享，并有真实食品样例测试 | 更多真实包装格式、复合原料归类、置信度校准、OCR 结果噪声纠错 | 已能验证更接近真实包装的文本分析闭环，但仍不是生产级解析器 |
| 图片 OCR | 协议和基础扫描闭环，真实识别未接入 | 12% | 独立扫描页支持 Capacitor Camera 系统相机/相册入口、Web 文件选择降级、常见图片类型和 8 MB 大小校验、图片预览、旋转预览、移除图片、OCR 协议状态展示、未接入手动输入提示、OCR 请求/响应 contract、响应校验、本地 fallback、手动校正文本、本地草稿保存/恢复并跳转分析 | OCR 服务端代理、图片压缩、方向修正、真实识别、裁剪、失败重试、成本控制 | 已有可用入口、校正闭环和可接服务端的结构化协议，但不具备真实图片识别能力 |
...
| 收藏 | 基础完成，登录态同步起步 | 43% | localStorage 收藏、取消收藏、详情页按钮、收藏页展示、收藏页空态搜索入口、收藏页内联取消、收藏页加入对比、类别感知收藏记录；后端已提供 `/api/user/favorites`，前端 storageService 在登录态读写会同步服务端并保持未登录本地行为 | 批量管理、离线同步队列、真实跨设备验收 | 单机流程可用，登录态云同步基础已接入，但还缺前端账号入口和跨设备验收 |
...
| 搜索历史 | 基础完成，登录态同步起步 | 42% | localStorage 保存最近 8 条、首页展示、清空、单条删除；设置页可关闭自动记录，关闭后首页展示隐私提示，导出/导入会保留该偏好；后端已提供 `/api/user/history`，前端 storageService 在登录态可后台同步 | 隐私开关更细分、离线同步队列、真实跨设备验收 | 单机基础能力已做，并已有登录态云同步接口 |
| 移动端 App 基座 | Web Shell + Capacitor 原生适配起步 | 27% | 已新增 Vite 前端工程化基座、Capacitor 依赖、`capacitor.config.json`、`cap:sync` / `cap:open:*` 命令、Camera/Filesystem/Share 插件、原生相机/相册入口、原生分享入口、Web 降级、viewport-fit、safe-area bottom、iOS 权限描述文档、Web 小屏底部主导航、类别感知移动链接、扫描页 active 态、首次启动引导页、成分对比移动端布局、会员中心移动端单列样式、PWA manifest / 图标和 service worker 离线 app shell | iOS/Android 工程人工签名配置、原生启动屏、权限恢复页、深链、推送、真机验收 | 已完成 Capacitor 可同步脚手架、基础原生能力适配和移动 Web Shell，但还不是可安装验收的原生 App |
| 账号系统 | 后端基础完成，前端未接入 | 18% | 后端已完成邮箱注册、登录、JWT Bearer 鉴权、退出、`me`、账号删除、服务端 session 失效，并为收藏/历史/过敏原/报告同步接口复用鉴权 | 前端登录页、token 刷新、匿名升级、第三方登录、账号隐私导出 | 后端账号底座可支撑同步和后续权益，但用户还不能在前端完成登录 |
...
| 报告与同步 | 本地报告闭环，登录态同步起步 | 32% | localStorage 保存分析报告快照，支持报告列表、详情、删除、清空、关键词检索、重新分析、类别隔离、风险/过敏原/数据边界/下一步建议解读区块、来源引用与审核状态、复制 Markdown、下载 Markdown、下载 JSON、原生分享 / Web Share / 复制 fallback，并可随本机数据快照整体导出、导入或清空；后端已提供 `/api/user/reports`，前端 storageService 在登录态可后台同步 | 离线同步队列、PDF/图片导出、原生文件持久化、真实跨设备验收 | 已完成单机闭环和登录态同步基础，但还不是 Pro 级报告系统 |
...
| 安全 | 基础起步 | 16% | 没有前端写死 API Key，HTML 输出有基础转义；后端已支持 JWT Bearer 鉴权、服务端 session 失效、账号删除和登录态用户数据接口鉴权 | CSP、依赖审计、输入限制、限流、审计日志、刷新 token、细粒度权限边界 | 已有基础鉴权和用户数据边界，但距离生产安全仍很早 |
| 测试 | 很早期 | 55% | 函数断言覆盖搜索、搜索建议、搜索分页、搜索排序、搜索无结果热门分类、风险分布筛选、分类分布筛选、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、分析页空输入提示、详情缺失成分页、收藏空态入口、复配括号/剂量后缀解析、分析置信度、解析质量面板、数据审核摘要渲染、来源/版本聚合、数据来源页渲染和纠错入口、相关成分推荐、文本拆分、食品样例、过敏原警告、首次启动引导、会员中心路由/渲染/服务、支持中心路由/渲染/本机反馈记录/预填草稿、隐私与条款路由/渲染/入口、成分对比路由/渲染/本机存储、路由解析、扫描页渲染、404、页面标题、详情页标题、历史删除、历史记录开关、storage 未登录本地回退和登录态后台同步、移动端导航链接、PWA service worker 注册和缓存清单、Capacitor 插件版本、Camera/Share 非 native 降级、viewport-fit、安全区和 iOS 权限文档、本地报告保存/导出/导入/检索/解读区块/来源引用、旧版报告兼容、OCR/AI 协议/响应校验/fallback 和页面展示；后端 Vitest 覆盖 health、CORS、404、500 JSON、ingredients API、auth API、user sync API 和 JWT middleware；已有 CI 工作流 | E2E、浏览器验收、移动端真机、支付沙盒、覆盖率 | 后端业务 API 已有基础覆盖，但仍不能支撑移动端和付费上线 |
| 后端 | 成分 API、基础鉴权和用户同步完成 | 40% | `backend/` 已有 Node.js 20 + TypeScript + Hono 工程、`GET /health`、CORS、请求日志、全局 JSON 错误处理、`.env.example`、Dockerfile、PostgreSQL 15 compose、Drizzle `ingredients` / `ingredient_sources` / `users` / `sessions` / `user_favorites` / `user_history` / `user_allergens` / `user_reports` schema、迁移、100 条 seed、`/api/ingredients` 列表/分类/详情 API、`/api/auth/register`、`/api/auth/login`、`/api/auth/logout`、`/api/auth/me`、`DELETE /api/auth/account`、`/api/user/*` 收藏/历史/过敏原/报告同步 API、Vitest 测试和 CI 后端校验 | 任务队列、缓存、日志结构化、限流、支付回调、OCR/AI 代理、生产部署 | 正式产品已有只读成分 API、账号底座和用户同步 API，但付费、OCR/AI 代理和生产安全仍未完成 |
...
| 收藏页 | 基础实现 | 展示已收藏成分，空状态引导去当前类别搜索页，支持内联取消收藏，并可从收藏卡片加入对比；底层 storageService 登录态可同步 `/api/user/favorites` | 排序、批量管理、离线同步队列、跨设备验收 | `src/pages/favoritesPage.js`、`src/services/storageService.js`、`backend/src/routes/user.ts` |
...
| 移动端首页/底部导航 | 基础实现 | Web 小屏底部导航包含首页、扫描、搜索、收藏、我的五个主入口，并复用当前类别路由和 active 态；扫描入口已指向独立扫描页；已补 `viewport-fit=cover` 和 safe-area bottom 兜底 | 真机验收、图标、手势返回、原生安全区细化 | `src/index.html`、`src/router/router.js`、`src/main.js`、`src/styles.css`、`scripts/test.mjs` |
| PWA/移动安装元数据 | 基础实现 | 已添加 manifest、theme color、iOS Web App meta、viewport-fit、SVG 应用图标、扫描快捷方式、service worker 注册、Vite public 静态资源、运行时缓存和离线首页回退 | 真机安装验收、PNG 多尺寸图标、商店素材、离线浏览器验收 | `src/index.html`、`public/manifest.webmanifest`、`public/app-icon.svg`、`src/main.js`、`public/sw.js`、`scripts/test.mjs` |
...
| 扫描页 | 基础完成（真实 OCR 未接入） | 独立 `/scan` 页面支持 Capacitor Camera 系统相机/相册入口、Web 文件选择降级、图片类型和大小校验、图片预览、旋转预览、移除图片、OCR 协议状态展示、OCR 未接入手动输入提示、OCR 降级反馈、手动录入/校正文本、本地草稿自动保存/恢复，并可跳转分析页 | 真实 OCR、图片裁剪、识别进度、失败重试、权限恢复、真机验收 | `src/pages/scanPage.js`、`src/services/nativeBridgeService.js`、`src/router/router.js`、`src/main.js`、`src/store/userStore.js`、`src/utils/imageFile.js`、`src/services/ocrService.js`、`scripts/test.mjs` |
| 分析报告页 | 基础实现，登录态同步起步 | 支持本地报告列表、详情、关键词检索、删除、清空、重新分析、类别隔离、风险/过敏原/数据边界/下一步建议解读区块、来源引用与草稿审核状态、Markdown 复制、Markdown 下载、JSON 下载和原生分享 / Web Share / 复制 fallback；底层 storageService 登录态可同步 `/api/user/reports` | PDF/图片导出、原生文件导出、离线同步队列、跨设备验收 | `src/pages/reportsPage.js`、`src/store/userStore.js`、`src/services/storageService.js`、`src/services/reportExportService.js`、`src/services/shareService.js`、`src/services/nativeBridgeService.js`、`backend/src/routes/user.ts` |
...
| 数据来源追溯 | 基础展示完成 | 100 条食品种子数据含 GB 2760、Codex INS、JECFA、EU 数据库来源链接和检索日期；数据页聚合展示来源、地区、检索日期和引用记录数 | 每条数据可追溯到具体标准条目、发布日期、适用地区 | 目前有来源级链接和聚合页，缺逐条条款编号 |
| 数据维护工具 | 部分完成 | `npm run validate:data` 可检查食品添加剂必填字段、重复 id、枚举值、来源 URL、来源检索日期和 gbStatus；服务层可生成审核摘要、来源聚合和版本聚合 | 导入脚本、审核流程、变更记录、真实数据质量报告 | 已有校验和透明展示入口，但还没有数据导入和审核流程 |

#### 食品添加剂专用字段（新增，部分完成）

| 字段 | 说明 | 来源 | 状态 |
|---|---|---|---|
| `eNumber` | EU E-number（如 E330） | EU 法规 | schema 已完成，100 条种子数据已按“如有”口径草稿填充 |
| `gbCode` | GB/INS 编号（如 INS 330） | GB 2760 / Codex INS | schema 已完成，100 条种子数据已草稿填充 |
| `gbStatus` | GB 2760 状态：`permitted` / `restricted` / `prohibited` | GB 2760 | schema 已完成，100 条种子数据已草稿填充 |
| `adi` | 每日允许摄入量（如 “0–40 mg/kg bw”） | JECFA / Codex | schema 已完成，100 条种子数据已草稿填充，需原文核验 |
| `usageLimits` | 使用限量和适用食品类别（如”饮料：0.2g/kg”） | GB 2760 | schema 已完成，100 条种子数据暂不填占位限量，避免将未审核限量误展示为真实数据 |
| `foodCategories` | 允许使用的食品类别列表 | GB 2760 | schema 已完成，100 条种子数据已草稿填充 |
| `allergenTypes` | 过敏原类别标注 | EU 14 类标准 | schema 已完成，100 条种子数据已草稿标注部分相关项 |
| `cautionFor` / `cautionGroups` | 特殊人群（孕妇/婴幼儿/糖尿病）禁忌说明 | 多来源 | schema 已完成，100 条种子数据已草稿填充 |
...
| 后端 API | 成分只读 API、基础鉴权和用户同步完成 | 已有 Hono 应用、`GET /health`、CORS、请求日志、全局 JSON 错误处理、配置读取、`GET /api/ingredients`、`GET /api/ingredients/categories`、`GET /api/ingredients/:id`、分页和非法参数 400；已完成 `/api/auth/*` 注册/登录/退出/me/账号删除和 `/api/user/*` 收藏、历史、过敏原、报告同步接口 | analyze、ocr、entitlements、支付回调、profile 扩展等接口 | 已能从 PostgreSQL 查询食品添加剂并保存登录用户基础数据，但 Pro 能力、OCR/AI 和生产治理 API 仍未开始 |
| 数据库 | 成分库、账号和用户同步表基础完成 | Drizzle `ingredients`、`ingredient_sources`、`users`、`sessions`、`user_favorites`、`user_history`、`user_allergens`、`user_reports` 表、迁移、100 条食品添加剂 seed 和幂等 upsert | 订阅权益、支付事件、OCR/AI 任务、审计日志、更多 profile 字段 | 成分、账号和用户同步数据底座已起步，但权益和任务数据仍未开始 |
...
| 单元测试 | 基础覆盖扩展中 | 函数断言覆盖搜索、搜索建议、分类摘要、搜索排序、风险分布筛选、分类分布筛选、相关成分推荐、解析、过敏原、食品样例、首次启动引导、会员中心、支持中心、隐私与条款、成分对比、路由、404、页面标题、本机数据导出/导入/清空、storage 未登录本地回退和登录态后台同步、本地报告保存、报告导出、Capacitor 插件版本、非 native Camera/Share 降级、viewport-fit、安全区、iOS 权限文档、AI 协议校验和降级服务；后端 Vitest 覆盖 health、CORS、404、500 JSON、ingredients API、auth API、user sync API 和 JWT middleware | 更多边界、浏览器行为测试、移动端 E2E、支付沙盒测试 | `scripts/test.mjs`、`backend/tests/` |
...
| 功能项 | 状态 | 需要实现 | 验收标准 |
|---|---|---|---|
| 账号体系 | 后端基础完成，前端未接入 | 后端已完成邮箱注册、登录、JWT Bearer 鉴权、退出、`me`、账号删除和登录态用户数据同步 API；仍需前端登录页、刷新 token、匿名用户升级和隐私导出 | 用户可在前端登录，并跨设备恢复收藏、过敏原、报告和订阅 |
...
| P0 | **后端、账号和云同步闭环** | 订阅、OCR、AI、跨端恢复都依赖后端 | 无法做付费权益、跨设备恢复、数据同步验收和密钥保护 | 补前端登录 UI、刷新 token、离线同步队列、生产安全、订阅权益和 OCR/AI 代理 |
...
| 扩充食品添加剂 MVP 数据集（50–100 条） | 已完成 | 已有 100 条草稿种子数据，达到 MVP 目标数量，覆盖防腐剂、甜味剂、着色剂、增稠剂、乳化剂、膨松剂、抗氧化剂、香料类、磷酸盐类、糖醇类等常见品类；仍需补 GB 2760 逐条限量来源 |
...
状态：后端项目初始化、成分数据库 schema、seed、只读成分 API、基础账号鉴权和登录态用户数据同步已完成，订阅权益、生产安全和 OCR/AI 代理未开始。

| 任务 | 状态 | 验收标准 |
|---|---|---|
| 后端项目初始化 | 已完成 | API 服务可本地启动，有健康检查、CORS、配置管理、请求日志、全局 JSON 错误处理、Docker/Compose 和 Vitest 测试 |
| 数据库 schema | 部分完成 | `ingredients`、`ingredient_sources`、`users`、`sessions`、`user_favorites`、`user_history`、`user_allergens`、`user_reports` 表已完成迁移，食品添加剂已有 100 条 seed；entitlements、payment events、OCR/AI task 表仍待后续批次 |
| 鉴权 | 基础完成 | 支持邮箱注册、登录、JWT Bearer 鉴权、退出、账号删除；刷新 token 和匿名升级待后续批次 |
| 云同步 | 基础完成 | 收藏、历史、过敏原档案、报告已有 JWT 鉴权 API 和前端 storageService 登录态后台同步；仍需前端登录 UI、离线队列和真实跨设备验收 |
