...
- 当前食品基础库为 112 条：32 条 `verified_jecfa` 安全评价匹配、12 条 `common_ingredient` 普通配料词库、68 条 `unverified` seed 参考；`verified_regulation` 仍为 0，全部 `isVerified: false`，不能视为完整成分库或 GB 2760 法规库。
...
**当前瓶颈（P0）**：
1. 数据源准确性（32 条 JECFA 安全评价匹配，12 条普通配料词库，68 条仍未验证；GB 2760 逐条法规条款和限量待人工确认）
...
- 人工：继续确认 GB 2760 官方文件版本、条款、适用食品类别和限量；无法结构化时先保留 `rawSourceText`
- Codex 可立即执行：继续做官方数据导入、JECFA 映射、常见配料词库、OCR 未匹配收集、人工校验队列和数据可信等级展示；`Batch A-A：AI 解释层集成` 仍需要 AI API Key 人工前置
...
- 当前食品基础库中 32 条按 WHO JECFA 官方数据库标记为 `verified_jecfa`，12 条为 `common_ingredient`，68 条仍为 `unverified`。全部记录仍为 `isVerified: false`，因为 GB 2760 逐食品类别限量、条款编号和中国适用条件仍未完成可信审核。
...
|---|---:|---:|---:|---|---|
| 数据源准确性 | 22% | 34% | 7.48% | 已建立 `verified_regulation` / `verified_jecfa` / `mapped_candidate` / `common_ingredient` / `unverified` / `unknown_from_ocr` 分层；32 条 JECFA 安全评价匹配，68 条仍未验证；不把 AI 输出当权威来源 | GB 2760 官方文件逐条核验、版本号、生效日期、条款编号、原文片段 |
| 数据完整度 | 18% | 25% | 4.50% | 当前覆盖 100 条食品添加剂 seed 和 12 条常见普通配料词库；不追求一次性补齐全部食品配料，按官方数据导入 + 人工校验队列持续扩充 | GB 2760 基础权威库、逐食品类别限量、ADI 原文、过敏原和特殊人群审核 |
...
| **食品添加剂数据** | **基础权威库已起步，内容审核未完成** | **42%** | 已定义食品数据分层字段，`src/data/foodAdditives.js` + `src/data/commonFoodIngredients.js` 合计 112 条；32 条为 `verified_jecfa`，12 条为 `common_ingredient`，68 条为 `unverified`；`npm run validate:data` 输出质量报告；来源/版本聚合和数据来源与审核状态页已同步 | GB 2760 逐食品类别限量、E-number 对照审核、ADI 原文核验、真实过敏原标注审核、人工校验队列 | **主线核心已建立基础权威库结构，但还不是可正式上线的完整法规数据集** |
...
| 数据来源追溯 | 基础展示完成 | 食品基础库含 GB 2760 seed reference、Codex INS、JECFA、EU 数据库来源链接和检索日期；32 条记录已指向 JECFA 具体条目 URL；12 条普通配料标记为内部样例词库；数据页聚合展示来源、地区、检索日期、引用记录数和数据状态 | 每条法规数据可追溯到具体 GB 2760 标准条目、发布日期、适用地区 | 目前已有 JECFA 单条 URL 和来源聚合页，仍缺 GB 2760 逐条条款编号 |
| 数据维护工具 | 部分完成 | `npm run validate:data` 可检查食品数据必填字段、重复 id、枚举值、来源 URL、来源检索日期、数据状态和 JECFA/GB 2760 状态边界，并输出 status 分布、缺来源字段、缺限量和来源版本分布质量报告；服务层可生成审核摘要、来源聚合和版本聚合 | 导入脚本、审核流程、变更记录、GB 2760 条款级质量报告、人工校验队列 | 已有校验、质量报告和透明展示入口，但还没有完整审核后台 |
...
| `usageLimits` | 使用限量和适用食品类别（如”饮料：0.2g/kg”） | GB 2760 | schema 已完成，食品添加剂 seed 暂不填占位限量，避免将未审核限量误展示为真实数据 |
...
| `dataStatus` | verified_regulation / verified_jecfa / mapped_candidate / common_ingredient / unverified / unknown_from_ocr | 来源可信分层 | schema 已完成，当前为 0 / 32 / 0 / 12 / 68 / 0 |
...
| 建立基础权威数据底座 | 进行中 | 已有 100 条食品添加剂 seed、32 条 JECFA 安全评价匹配、12 条普通配料词库和分层状态字段；仍需补 GB 2760 逐条限量来源和人工校验队列 |
