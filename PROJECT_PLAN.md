...
> 整体产品进度：**30%**（口径：iOS/Android 可上线 + 可付费订阅）
...
**当前瓶颈（P0）：** 移动端技术基座 → 后端账号/同步 → 订阅支付 → 合规材料。前端 Web 原型仍在 M1 迭代，M2 已完成 Vite、Capacitor 基础脚手架、原生 Camera/Share 适配层和 iOS 权限说明文档，M3 已完成 Hono API 骨架、食品添加剂数据库 schema、迁移、seed、只读成分 API 和基础账号鉴权，但云同步仍未开始，M4–M7 完全未开始，决定整体进度的是 P0 基础设施，不是前端页面数量。

**当前最早未完成任务**：见 [`CODEX_TASKS.md`](./CODEX_TASKS.md) → `Batch 2-D：iOS 工程配置 [人工]`；下一个 Codex 批次为 `Batch 3-D：收藏与历史云同步`
...
| 后端 | Node/TypeScript API + 数据库 + 对象存储 + 队列 | Hono/TypeScript API 骨架、食品添加剂 schema/seed、只读成分 API 和基础账号鉴权已完成 | 云同步、订阅校验、OCR、AI 都依赖后端 |
...
- 已有食品添加剂数据库 schema、迁移和本机 seed，但没有用户数据同步、订阅权益、OCR/AI 任务数据和生产部署配置。
- 账号系统已完成注册、登录、退出、`me` 和账号删除的后端基础接口，但没有前端登录页、刷新 token、匿名升级或云同步。
...
| 账号、订阅与付费权益 | 10% | 12% | 1.20% | 已有前端会员中心信息架构、Free/Pro 套餐说明、本机用量展示、购买/恢复/管理订阅未接入提示；后端已完成 `users` / `sessions`、注册、登录、退出、`me` 和账号删除基础接口 | 前端登录、刷新 token、匿名升级、Apple IAP、Google Play Billing、真实恢复购买、服务端权益校验 |
| 后端 API、同步与安全基础设施 | 12% | 28% | 3.36% | 已有 `backend/` Node.js 20 + TypeScript + Hono 项目骨架、`GET /health`、CORS、请求日志、全局 JSON 错误处理、`.env.example`、Dockerfile、PostgreSQL 15 compose、Drizzle `ingredients` / `ingredient_sources` / `users` / `sessions` schema、迁移、100 条食品添加剂 seed、`/api/ingredients` 列表/分类/详情 API、`/api/auth/*` 鉴权 API 和 Vitest 测试 | 用户数据同步 API、对象存储、队列、限流、审计日志、密钥管理 |
...
| 测试、CI、质量与可观测性 | 8% | 60% | 4.80% | 有本地 lint/test/Vite build、数据校验、CI Node 20.19 依赖安装与 Vite/public/backend 触发路径、首次启动引导测试、会员中心路由/渲染/服务测试、支持中心路由/渲染/本机反馈/预填反馈测试、隐私与条款路由/渲染/入口测试、成分对比路由/渲染/本机存储/分享测试、移动端导航链接测试、PWA service worker 注册和缓存清单测试、Capacitor 7 插件依赖断言、Camera/Share 非 native 降级测试、viewport-fit/安全区/iOS 权限文档断言、数据来源页路由/渲染/来源聚合/纠错入口测试、本机数据摘要/导出/导入/清空测试、搜索历史隐私开关测试、扫描页路由与渲染测试、分析页空输入、详情缺失成分、收藏空态和搜索无结果热门分类测试、搜索建议、拼音/首字母/常见误写/近似纠错辅助、分页、排序、风险分布、分类分布、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、成分表复配括号/剂量后缀解析、分析置信度和解析质量面板测试、数据审核摘要渲染、本地报告保存/导出/导入/检索/解读区块/来源引用/分享测试、旧版报告兼容测试、OCR/AI 协议/响应校验/fallback/页面展示测试、后端 health/CORS/404/500、ingredients API、auth API 与 JWT middleware Vitest 测试、后端脚手架静态断言和 GitHub Actions 工作流 | E2E、移动端真机测试、后端同步/API 集成测试、支付沙盒测试、崩溃监控、覆盖率 |
...
折算结果约 30.64%，当前按 **30%** 记录。

## 5. 已实现与待实现总表

| 模块 | 当前状态 | 模块完成度 | 已实现 | 需要实现 | 真实评价 |
|---|---|---:|---|---|---|
| 项目结构 | 部分完成 | 36% | `src/components`、`src/pages`、`src/router`、`src/services`、`src/store`、`src/data`、`src/utils` 已建立，Vite 配置和 `public/` 静态资源目录已接入；`backend/` 已初始化 Hono/TypeScript API、Drizzle schema、migration、seed 和 route/service 工程 | API client、账号/同步表、移动端平台工程、平台能力封装 | 能继续验证业务并已有标准前端构建、后端骨架和成分 API，但还不是移动端可上线工程 |
| 开发脚本 | 基础完成 | 67% | `npm run dev`、`npm run build`、`npm run preview`、`npm run lint`、`npm run test`、`npm run validate:data`、`npm run cap:sync`、`npm run cap:open:ios`、`npm run cap:open:android` 已配置；`backend/` 已有 `dev`、`typecheck`、`test`、`build`、`db:generate`、`db:migrate`、`db:seed` 和 Docker Compose 命令，CI 会执行前端和后端基础校验 | 覆盖率、发布流水线、移动端真机脚本 | 本地、PR、Capacitor 同步、后端 API 和数据库基础校验命令可控，但还不覆盖真机、鉴权/同步 API 和发布 |
...
| 收藏 | 基础完成 | 37% | localStorage 收藏、取消收藏、详情页按钮、收藏页展示、收藏页空态搜索入口、收藏页内联取消、收藏页加入对比、类别感知收藏记录 | 云同步、批量管理 | 单机可用，已避免食品/化妆品收藏串类，并可从收藏继续进入对比流程 |
...
| 报告与同步 | 本地报告起步 | 26% | localStorage 保存分析报告快照，支持报告列表、详情、删除、清空、关键词检索、重新分析、类别隔离、风险/过敏原/数据边界/下一步建议解读区块、来源引用与审核状态、复制 Markdown、下载 Markdown、下载 JSON、原生分享 / Web Share / 复制 fallback，并可随本机数据快照整体导出、导入或清空 | 云同步、离线队列、PDF/图片导出、原生文件持久化 | 已完成单机闭环、基础导出/导入、历史检索、来源引用、本地报告解读骨架和移动端分享入口，但还不是 Pro 级报告系统 |
...
| 安全 | 基础起步 | 12% | 没有前端写死 API Key，HTML 输出有基础转义；后端已支持 JWT Bearer 鉴权、服务端 session 失效和账号删除 | CSP、依赖审计、输入限制、限流、审计日志、刷新 token、权限边界 | 已有基础鉴权，但距离生产安全仍很早 |
| 测试 | 很早期 | 50% | 函数断言覆盖搜索、搜索建议、搜索分页、搜索排序、搜索无结果热门分类、风险分布筛选、分类分布筛选、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、分析页空输入提示、详情缺失成分页、收藏空态入口、复配括号/剂量后缀解析、分析置信度、解析质量面板、数据审核摘要渲染、来源/版本聚合、数据来源页渲染和纠错入口、相关成分推荐、文本拆分、食品样例、过敏原警告、首次启动引导、会员中心路由/渲染/服务、支持中心路由/渲染/本机反馈记录/预填草稿、隐私与条款路由/渲染/入口、成分对比路由/渲染/本机存储、路由解析、扫描页渲染、404、页面标题、详情页标题、历史删除、历史记录开关、移动端导航链接、PWA service worker 注册和缓存清单、Capacitor 插件版本、Camera/Share 非 native 降级、viewport-fit、安全区和 iOS 权限文档、本地报告保存/导出/导入/检索/解读区块/来源引用、旧版报告兼容、OCR/AI 协议/响应校验/fallback 和页面展示；后端 Vitest 覆盖 health、CORS、404 和 500 JSON；已有 CI 工作流 | E2E、浏览器验收、后端业务 API 测试、移动端真机、支付沙盒、覆盖率 | 仍不能支撑移动端和付费上线 |
| 后端 | 成分 API 与基础鉴权完成 | 28% | `backend/` 已有 Node.js 20 + TypeScript + Hono 工程、`GET /health`、CORS、请求日志、全局 JSON 错误处理、`.env.example`、Dockerfile、PostgreSQL 15 compose、Drizzle `ingredients` / `ingredient_sources` / `users` / `sessions` schema、迁移、100 条 seed、`/api/ingredients` 列表/分类/详情 API、`/api/auth/register`、`/api/auth/login`、`/api/auth/logout`、`/api/auth/me`、`DELETE /api/auth/account`、Vitest 测试和 CI 后端校验 | 用户数据 API、任务队列、缓存、日志结构化、限流、支付回调、OCR/AI 代理 | 正式产品已有只读成分 API 和账号底座，但付费、同步和第三方代理仍未开始 |
...
| 收藏页 | 基础实现 | 展示已收藏成分，空状态引导去当前类别搜索页，支持内联取消收藏，并可从收藏卡片加入对比 | 排序、批量管理、云同步 | `src/pages/favoritesPage.js` |
...
| 分析报告页 | 基础实现 | 支持本地报告列表、详情、关键词检索、删除、清空、重新分析、类别隔离、风险/过敏原/数据边界/下一步建议解读区块、来源引用与草稿审核状态、Markdown 复制、Markdown 下载、JSON 下载和原生分享 / Web Share / 复制 fallback | 云同步、PDF/图片导出、原生文件导出 | `src/pages/reportsPage.js`、`src/store/userStore.js`、`src/services/reportExportService.js`、`src/services/shareService.js`、`src/services/nativeBridgeService.js` |
...
| 后端 API | 成分只读 API 基础完成 | 已有 Hono 应用、`GET /health`、CORS、请求日志、全局 JSON 错误处理、配置读取、`GET /api/ingredients`、`GET /api/ingredients/categories`、`GET /api/ingredients/:id`、分页和非法参数 400 | analyze、ocr、reports、favorites、profile、entitlements 等接口 | 已能从 PostgreSQL 查询食品添加剂，但用户和 Pro 能力 API 仍未开始 |
| 数据库 | 成分库基础完成 | Drizzle `ingredients` 与 `ingredient_sources` 表、迁移、100 条食品添加剂 seed 和幂等 upsert | 用户表、过敏原档案、收藏、报告、订阅权益、日志表 | 只读成分库起步，用户和权益数据仍未开始 |
...
| 单元测试 | 很早期 | 函数断言覆盖搜索、搜索建议、分类摘要、搜索排序、风险分布筛选、分类分布筛选、相关成分推荐、解析、过敏原、食品样例、首次启动引导、会员中心、支持中心、隐私与条款、成分对比、路由、404、页面标题、本机数据导出/导入/清空、本地报告保存、报告导出、Capacitor 插件版本、非 native Camera/Share 降级、viewport-fit、安全区、iOS 权限文档、AI 协议校验和降级服务；后端 Vitest 覆盖 health、CORS、404 和 500 JSON | 更多边界、后端业务 API、错误路径、浏览器行为测试 | `scripts/test.mjs`、`backend/tests/health.test.ts` |
...
状态：后端项目初始化、成分数据库 schema、seed、只读成分 API 和基础账号鉴权已完成，云同步未开始。

| 任务 | 状态 | 验收标准 |
|---|---|---|
| 后端项目初始化 | 已完成 | API 服务可本地启动，有健康检查、CORS、配置管理、请求日志、全局 JSON 错误处理、Docker/Compose 和 Vitest 测试 |
| 数据库 schema | 部分完成 | `ingredients`、`ingredient_sources`、`users`、`sessions` 表已完成迁移，食品添加剂已有 100 条 seed；profiles、favorites、reports、entitlements 仍待后续批次 |
