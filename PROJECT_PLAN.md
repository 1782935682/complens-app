...
> 整体产品进度：**25%**（口径：iOS/Android 可上线 + 可付费订阅）

| 里程碑 | 名称 | 状态 | 完成度 |
|---|---|---|---|
| M0 | 口径校准与开发规则 | ✅ 已完成 | 100% |
| M1 | Web/PWA 可用 MVP + 食品数据基础 | 🔄 进行中 | ~62% |
| M2 | 移动端 App 基座（Capacitor/原生工程） | 🔄 进行中 | ~18% |
| M3 | 后端、账号与云同步 | ⏳ 未开始 | 0% |
...
**当前瓶颈（P0）：** 移动端技术基座 → 后端/账号 → 订阅支付 → 合规材料。前端 Web 原型仍在 M1 迭代，M2 已完成 Vite 与 Capacitor 基础脚手架，M3–M7 完全未开始，决定整体进度的是 P0 基础设施，不是前端页面数量。

**当前最早未完成任务**：见 [`CODEX_TASKS.md`](./CODEX_TASKS.md) → `Batch 2-C：原生权限与移动端适配`
...
| 移动端 App 工程与跨端体验 | 12% | 18% | 2.16% | 已有 Vite 前端工程化基座、Capacitor 依赖与 `capacitor.config.json`、Web 小屏底部主导航、安全区底部预留、类别感知链接、独立扫描路由、首次启动引导页、成分对比页移动端布局、PWA manifest 和 service worker 离线 app shell | 原生相机权限、iOS/Android 工程人工配置、深链、推送、真机适配 |
...
| 移动端 App 基座 | Web Shell + Capacitor 基座起步 | 22% | 已新增 Vite 前端工程化基座、Capacitor 依赖、`capacitor.config.json`、`cap:sync` / `cap:open:*` 命令、Web 小屏底部主导航、安全区底部预留、类别感知移动链接、扫描页 active 态、首次启动引导页、成分对比移动端布局、会员中心移动端单列样式、PWA manifest / 图标和 service worker 离线 app shell | 原生权限、iOS/Android 工程人工签名配置、原生启动屏、相机权限、文件权限、深链、真机验收 | 已完成 Capacitor 可同步脚手架和移动 Web Shell，但还不是可安装验收的原生 App |
...
状态：进行中，Vite 前端工程化基座和 Capacitor 项目脚手架已完成，原生权限与人工签名配置未开始。
