...
| 核心查询与分析体验 | 14% | 54% | 7.56% | 已有搜索、搜索建议、搜索结果分页、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、首页/搜索/详情食品数据审核状态提示、数据来源与审核状态页、详情、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、食品/化妆品类别化文本分析 summary、扫描页图片预览、旋转预览、移除图片、校正入口和草稿恢复、分析页 AI 协议状态与本地 fallback 展示、本地报告保存、报告列表/详情、报告历史检索、报告解读区块、报告来源引用、Markdown/JSON 导出、本机数据导出/导入/清空、搜索历史隐私开关、收藏、历史、筛选、类别路由、移动端底部主导航、PWA 安装元数据、service worker 离线 app shell 和基础渲染测试 | 移动端主流程重设计、拼音/纠错、原生分享、异常边界 |
| 过敏原与个人化档案 | 8% | 22% | 1.76% | 14 类过敏原枚举、localStorage 持久化、搜索/详情/分析基础警告，设置页可导出、导入或清空本机过敏原档案，并可关闭搜索历史记录 | 多端同步、真实数据逐条审核、提醒强度设置 |
| 移动端 App 工程与跨端体验 | 12% | 4% | 0.48% | 已有 Web 小屏底部主导航、安全区底部预留、类别感知链接、独立扫描路由、PWA manifest 和 service worker 离线 app shell | Capacitor 或其他移动框架、iOS/Android 工程、原生相机权限、深链、推送、真机适配 |
...
| 测试、CI、质量与可观测性 | 8% | 42% | 3.36% | 有本地 lint/test/build、数据校验、移动端导航链接测试、PWA service worker 注册和缓存清单测试、数据来源页路由/渲染/来源聚合测试、本机数据摘要/导出/导入/清空测试、搜索历史隐私开关测试、扫描页路由与渲染测试、搜索建议、分页、排序、风险分布、分类分布、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、数据审核摘要渲染、本地报告保存/导出/导入/检索/解读区块/来源引用测试、旧版报告兼容测试、OCR/AI 协议/响应校验/fallback/页面展示测试和 GitHub Actions 工作流 | E2E、移动端真机测试、支付沙盒测试、崩溃监控、覆盖率 |
| 上架、合规与运营 | 6% | 11% | 0.66% | README 有克制表达要求，分析页有基础免责声明，食品数据已有来源与审核状态透明页，报告详情和导出包含来源引用与草稿审核状态，设置页支持本机数据导出、导入、清空和搜索历史记录关闭 | App Store/Google Play 资料、隐私政策、服务条款、数据安全表、客服渠道、订阅条款、账号删除 |
| 化妆品与多类别扩展 | 4% | 2% | 0.08% | 原有化妆品原型代码可复用结构 | 独立数据、字段、审核和类别运营 |

折算结果约 18.30%，当前按 **18%** 记录。
...
| 首页 | 原型完成（已加类别入口） | 24% | 搜索入口、扫描入口、分析入口、收藏入口、历史、分类入口、食品/化妆品类别切换；食品首页已有热门种子数据入口和数据状态摘要 | 移动端首页信息架构、免费/Pro 价值展示、真实热门内容 | 食品类别可展示草稿数据状态，但仍不是正式内容规模 |
| 路由 | 基础完成（已扩展） | 64% | hash 路由支持首页、搜索、扫描、详情、分析、报告列表/详情、收藏、设置、基础 404，并支持 `/food` 和 `/cosmetics` 前缀；路由切换同步 `document.title`，顶部导航根据类别更新链接和 active 态 | 更完整的参数边界、面包屑 | 类别层级、扫描/分析分层、基础异常页、报告路由和导航状态已加，仍未到完整路由系统 |
...
| **过敏原用户档案** | **基础完成** | **21%** | 已定义 EU 14 类标准过敏原枚举 `src/data/allergens.js`，提供 `getUserAllergens()` / `setUserAllergens(ids)`，并新增设置页选择、保存、清空过敏原和本机数据导出/导入/清空 | 多端同步、设置页说明增强、更多地区标准映射 | 单机可用，已能驱动搜索/详情/分析警告，并提供基础本机隐私控制 |
...
| 移动端 App 基座 | Web Shell 起步 | 6% | 已新增 Web 小屏底部主导航、安全区底部预留、类别感知移动链接、扫描页 active 态、PWA manifest / 图标和 service worker 离线 app shell | iOS/Android 工程、原生启动屏、相机权限、文件权限、深链、真机验收 | 已开始移动体验外壳、扫描入口和 PWA 离线基础，但还不是原生 App 工程 |
...
| 测试 | 很早期 | 40% | 函数断言覆盖搜索、搜索建议、搜索分页、搜索排序、风险分布筛选、分类分布筛选、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、数据审核摘要渲染、来源/版本聚合、数据来源页渲染、相关成分推荐、文本拆分、食品样例、过敏原警告、路由解析、扫描页渲染、404、页面标题、详情页标题、历史删除、历史记录开关、移动端导航链接、PWA service worker 注册和缓存清单、本地报告保存/导出/导入/检索/解读区块/来源引用、旧版报告兼容、OCR/AI 协议/响应校验/fallback 和页面展示；已有 CI 工作流 | E2E、浏览器验收、移动端真机、支付沙盒、覆盖率 | 仍不能支撑移动端和付费上线 |
...
| 合规与内容治理 | 很早期 | 10% | README 有克制表达要求，分析页有基础免责声明，数据来源与审核状态页，设置页提供本机数据导出、导入、清空和搜索历史记录关闭 | 隐私政策、服务条款、订阅条款、数据安全表、客服入口、账号删除、数据授权、内容审核 | 上线前必须补正式政策和平台资料 |
...
| PWA/移动安装元数据 | 基础实现 | 已添加 manifest、theme color、iOS Web App meta、SVG 应用图标、扫描快捷方式、service worker 注册、核心资源缓存和离线首页回退 | 真机安装验收、PNG 多尺寸图标、商店素材、离线浏览器验收 | `src/index.html`、`src/manifest.webmanifest`、`src/app-icon.svg`、`src/main.js`、`src/sw.js`、`scripts/test.mjs` |
...
| 过敏原 localStorage 持久化 | 基础完成 | `getUserAllergens()` / `setUserAllergens(ids)` 使用 `compcheck:allergens`，设置页已接入，并可随本机数据快照导出或清空 | 云同步、隐私开关 | 已扩展 `userStore.js` |
| 本机数据与隐私控制 | 基础完成 | 设置页展示收藏、历史、报告、扫描草稿和过敏原数量；支持导出本机 JSON 快照、从 JSON 快照导入恢复和一键清空本机数据 | 账号删除、云端数据导出、正式隐私政策 | `src/pages/settingsPage.js`、`src/store/userStore.js`、`src/main.js`、`scripts/test.mjs` |
...
| 单元测试 | 很早期 | 函数断言覆盖搜索、搜索建议、分类摘要、搜索排序、风险分布筛选、分类分布筛选、相关成分推荐、解析、过敏原、食品样例、路由、404、页面标题、本机数据导出/导入/清空、本地报告保存、报告导出、AI 协议校验和降级服务 | 更多边界、错误路径、浏览器行为测试 | `scripts/test.mjs` |
