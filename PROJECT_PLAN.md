...
2026-06-11 按新目标重新校准：主线仍是食品添加剂，但产品范围扩大为 iOS/Android 上线和付费订阅。前端基础框架（路由、搜索、收藏、存储）可复用；食品添加剂 schema、过敏原枚举、过敏原设置页、数据校验脚本、50 条食品添加剂草稿种子数据、食品数据审核摘要展示、食品详情字段基础展示、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、食品分析类别化 summary、基础扫描页、扫描图片类型/大小校验、图片预览旋转/移除、扫描草稿本地保存/恢复、PWA service worker 离线 app shell、基础 404、页面标题、SPA 焦点管理、搜索筛选器、类别感知顶部导航、最近查询单条删除、本地分析报告保存和 Markdown/JSON 导出已完成。新增口径下，原生移动端工程、账号、订阅、服务端权益校验、商店审核、支付沙盒、真机测试和上线合规均未开始，因此总进度仍处于早期。
...
|---|---:|---:|---:|---|---|
| 食品添加剂数据与内容治理 | 18% | 16% | 2.88% | 已定义 `FoodAdditive` schema、数据校验脚本和 50 条草稿种子数据，达到 MVP 数量下限并覆盖甜味剂、增稠剂、乳化剂、防腐/护色剂、酸度调节剂、抗氧化剂、抗结剂等常见类别 | GB 2760 逐食品类别限量、来源审核、ADI 原文核验、内容版本管理 |
| 核心查询与分析体验 | 14% | 45% | 6.30% | 已有搜索、搜索建议、搜索结果分页、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、首页/搜索/详情食品数据审核状态提示、详情、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、食品/化妆品类别化文本分析 summary、扫描页图片预览、旋转预览、移除图片、校正入口和草稿恢复、本地报告保存、报告列表/详情、Markdown/JSON 导出、收藏、历史、筛选、类别路由、移动端底部主导航、PWA 安装元数据、service worker 离线 app shell 和基础渲染测试 | 移动端主流程重设计、拼音/纠错、原生分享、异常边界 |
...
| 测试、CI、质量与可观测性 | 8% | 32% | 2.56% | 有本地 lint/test/build、数据校验、移动端导航链接测试、PWA service worker 注册和缓存清单测试、扫描页路由与渲染测试、搜索建议、分页、排序、风险分布、分类分布、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、数据审核摘要渲染、本地报告保存与导出测试和 GitHub Actions 工作流 | E2E、移动端真机测试、支付沙盒测试、崩溃监控、覆盖率 |
...
| **食品添加剂数据** | **草稿数量达到 MVP 下限，内容审核未完成** | **16%** | 已定义 `FoodAdditive` 类型、`src/data/foodAdditives.js` 含 50 条草稿种子数据、`npm run validate:data` 校验脚本 | GB 2760 逐食品类别限量、E-number 对照审核、ADI 原文核验、真实过敏原标注审核、继续扩到 100 条 | **主线核心已有草稿规模，但还不是可正式上线的数据集** |
...
| 测试 | 很早期 | 32% | 函数断言覆盖搜索、搜索建议、搜索分页、搜索排序、风险分布筛选、分类分布筛选、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、数据审核摘要渲染、相关成分推荐、文本拆分、食品样例、过敏原警告、路由解析、扫描页渲染、404、页面标题、详情页标题、历史删除、移动端导航链接、PWA service worker 注册和缓存清单、本地报告保存/导出和 AI/OCR 降级；已有 CI 工作流 | E2E、浏览器验收、移动端真机、支付沙盒、覆盖率 | 仍不能支撑移动端和付费上线 |
...
| 数据审核状态摘要 | 基础实现 | 食品首页展示草稿数量、分类数、已复核/验证数和限量覆盖；食品搜索页和详情页显示草稿审核提示 | 审核后台、逐条条款状态、数据版本说明页 | `src/services/ingredientService.js`、`src/pages/homePage.js`、`src/pages/searchPage.js`、`src/pages/detailPage.js`、`scripts/test.mjs` |
...
| 数据来源追溯 | 部分完成 | 50 条食品种子数据含 GB 2760、Codex INS、JECFA、EU 数据库来源链接 | 每条数据可追溯到具体标准条目、发布日期、适用地区 | 目前只有来源级链接，缺逐条条款编号 |
