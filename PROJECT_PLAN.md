...
|---|---|---|---|
| 2026-06-11 | 新增数据纠错反馈闭环：详情页和数据来源页可带入数据纠错草稿进入支持中心，支持中心解析 query 预填问题类型、标题、描述和联系方式，保存后清理预填 query，补充移动端样式、PWA shell 缓存更新和测试；整体产品进度更新为 21% | Codex | `npm run validate:data`、`npm run lint`、`npm run test`、`npm run build`、`git diff --check` 通过；`curl -I http://127.0.0.1:5178/`、`/main.js`、`/pages/supportPage.js`、`/pages/detailPage.js`、`/pages/dataPage.js`、`/services/supportService.js` 和 `/sw.js` HTTP 200 |
