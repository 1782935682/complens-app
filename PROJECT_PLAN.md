...
> 整体产品进度：**52%**（新口径：OCR 拍照识别产品闭环为核心，数据底座 + 数据库对接优先；订阅、支付、上架、iOS/Android 签名后置）
...
- 当前食品基础库为 112 条：32 条 `verified_jecfa` 安全评价匹配、12 条 `common_ingredient` 普通配料词库、68 条 `unverified` seed 参考；GB 2760-2024 官方标准文本来源已确认并写入元数据，但 `verified_regulation` 仍为 0，全部 `isVerified: false`，不能视为完整成分库或 GB 2760 法规库。
...
**当前瓶颈（P0）**：
1. 数据源准确性（GB 2760-2024 官方标准文本来源已确认；32 条 JECFA 安全评价匹配，12 条普通配料词库，68 条仍未验证；GB 2760 逐条法规条款和限量待人工确认）
...
|---|---:|---:|---:|---|---|
| 数据源准确性 | 22% | 36% | 7.92% | 已建立 `verified_regulation` / `verified_jecfa` / `mapped_candidate` / `common_ingredient` / `unverified` / `unknown_from_ocr` 分层；GB 2760-2024 官方标准文本来源已确认到国家卫健委公告和食品安全国家标准数据检索平台；32 条 JECFA 安全评价匹配，68 条仍未验证；不把 AI 输出当权威来源 | GB 2760 逐添加剂条款编号、适用食品类别、最大使用量/残留量和原文片段 |
| 数据完整度 | 18% | 27% | 4.86% | 当前覆盖 100 条食品添加剂 seed 和 12 条常见普通配料词库；数据治理页已聚合 OCR 未收录、低置信候选和静态未验证数据，按官方数据导入 + 人工校验队列持续扩充 | GB 2760 基础权威库、逐食品类别限量、ADI 原文、过敏原和特殊人群审核 |
...
折算结果约 38.56%，考虑已有账号同步、前端登录入口、移动端基座、结构化报告详情、OCR manual/fallback 主路径、OCR 未收录人工校验队列入口、产品档案/历史收藏闭环、个人档案高亮和关注/忌口登录态同步、首页/导航设计系统起步、PWA 安装/离线基础体验、数据版本/审核状态基础设施、GB 2760-2024 官方标准文本来源确认、32 条 JECFA reviewed 数据和测试基础，当前按 **52%** 记录。
...
| **食品添加剂数据** | **基础权威库已起步，内容审核未完成** | **46%** | 已定义食品数据分层字段，`src/data/foodAdditives.js` + `src/data/commonFoodIngredients.js` 合计 112 条；GB 2760-2024 官方标准文本来源已确认并写入 seed 元数据；32 条为 `verified_jecfa`，12 条为 `common_ingredient`，68 条为 `unverified`；`npm run validate:data` 输出质量报告；来源/版本聚合、数据来源与审核状态页、人工校验队列已同步 | GB 2760 逐食品类别限量、E-number 对照审核、ADI 原文核验、真实过敏原标注审核、完整审核后台 | **主线核心已建立基础权威库结构、官方标准来源和待审核入口，但还不是可正式上线的完整法规数据集** |
...
| 数据来源追溯 | 基础展示完成 | 食品基础库含国家卫健委公告（2024年第1号）、食品安全国家标准数据检索平台 GB 2760-2024 标准文本记录、Codex INS、JECFA、EU 数据库来源链接和检索日期；32 条记录已指向 JECFA 具体条目 URL；12 条普通配料标记为内部样例词库；数据页聚合展示来源、地区、检索日期、引用记录数和数据状态 | 每条法规数据可追溯到具体 GB 2760 标准条目、适用食品类别和限量 | 目前已有 GB 2760-2024 标准文本记录和 JECFA 单条 URL，仍缺 GB 2760 逐添加剂条款编号 |
...
| 建立基础权威数据底座 | 进行中 | 已有 100 条食品添加剂 seed、GB 2760-2024 官方标准文本来源、32 条 JECFA 安全评价匹配、12 条普通配料词库、分层状态字段和只读人工校验队列；仍需补 GB 2760 逐条限量来源和完整审核后台 |
...
|---|---|---|---|
| 2026-06-12 | 确认 GB 2760-2024 官方来源：以国家卫健委公告（2024年第1号）和食品安全国家标准数据检索平台为准，记录标准文本 ID、附件 ID、发布日期、实施日期和下载接口；未验证 seed 的主来源更新为官方平台记录，并将平台检索结果写入 `rawSourceText`；不使用第三方网站作为 `official_standard`，不导入逐食品类别限量，不升级 `verified_regulation` 或 `isVerified` | Codex | `npm run validate:data`、`npm run lint`、`npm run test`、`npm run build`、`git diff --check` 通过 |
