...
> 整体产品进度：**27%**（口径：iOS/Android 可上线 + 可付费订阅）
...
**当前瓶颈（P0）：** 移动端技术基座 → 后端/账号 → 订阅支付 → 合规材料。前端 Web 原型仍在 M1 迭代，M2 已完成 Vite、Capacitor 基础脚手架、原生 Camera/Share 适配层和 iOS 权限说明文档，M3 已完成最小 Hono API 骨架但数据库、账号和同步仍未开始，M4–M7 完全未开始，决定整体进度的是 P0 基础设施，不是前端页面数量。

**当前最早未完成任务**：见 [`CODEX_TASKS.md`](./CODEX_TASKS.md) → `Batch 2-D：iOS 工程配置 [人工]`；下一个 Codex 批次为 `Batch 3-B：数据库 Schema 与成分 API`
...
| 后端 | Node/TypeScript API + 数据库 + 对象存储 + 队列 | Hono/TypeScript API 骨架已初始化，数据库未接入 | 账号、订阅校验、OCR、AI 都依赖后端 |
...
| 后端 API、同步与安全基础设施 | 12% | 5% | 0.60% | 已有 `backend/` Node.js 20 + TypeScript + Hono 项目骨架、`GET /health`、CORS、请求日志、全局 JSON 错误处理、`.env.example`、Dockerfile、PostgreSQL 15 compose 和 Vitest 基础测试 | 数据库 schema、业务 API、鉴权、对象存储、队列、限流、审计日志、密钥管理 |
...
折算结果约 26.58%，当前按 **27%** 记录。

## 5. 已实现与待实现总表

| 模块 | 当前状态 | 模块完成度 | 已实现 | 需要实现 | 真实评价 |
|---|---|---:|---|---|---|
| 项目结构 | 部分完成 | 33% | `src/components`、`src/pages`、`src/router`、`src/services`、`src/store`、`src/data`、`src/utils` 已建立，Vite 配置和 `public/` 静态资源目录已接入；`backend/` 已初始化 Hono/TypeScript API 工程 | API client、数据库 schema、移动端平台工程、平台能力封装 | 能继续验证业务并已有标准前端构建和后端骨架，但还不是移动端可上线工程 |
| 开发脚本 | 基础完成 | 63% | `npm run dev`、`npm run build`、`npm run preview`、`npm run lint`、`npm run test`、`npm run validate:data`、`npm run cap:sync`、`npm run cap:open:ios`、`npm run cap:open:android` 已配置；`backend/` 已有 `dev`、`typecheck`、`test`、`build` 和 Docker Compose 命令，仓库已有 GitHub Actions 工作流并执行 `npm ci` | 覆盖率、后端 CI 矩阵、发布流水线、移动端真机脚本 | 本地、PR、Capacitor 同步和后端基础校验命令可控，但还不覆盖真机、业务 API 和发布 |
...
| 后端 | 项目初始化完成 | 5% | `backend/` 已有 Node.js 20 + TypeScript + Hono 工程、`GET /health`、CORS、请求日志、全局 JSON 错误处理、`.env.example`、Dockerfile、PostgreSQL 15 compose、README 和 Vitest 测试 | 数据库、业务 API、任务队列、缓存、日志结构化、限流、鉴权、支付回调 | 正式产品核心只完成服务骨架，业务能力仍未开始 |
...
| 后端 API | 骨架起步，业务 API 未开始 | 已有 Hono 应用、`GET /health`、CORS、请求日志、全局 JSON 错误处理和配置读取 | ingredients、search、analyze、ocr、reports、favorites、profile、entitlements 等接口 | 已有服务入口和错误边界，但业务接口完全未开始 |
