...
| 核心查询与分析体验 | 14% | 55% | 7.70% | 已有搜索、搜索建议、搜索结果分页、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、首页/搜索/详情食品数据审核状态提示、首次启动引导、数据来源与审核状态页、详情、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、食品/化妆品类别化文本分析 summary、扫描页图片预览、旋转预览、移除图片、校正入口和草稿恢复、分析页 AI 协议状态与本地 fallback 展示、本地报告保存、报告列表/详情、报告历史检索、报告解读区块、报告来源引用、Markdown/JSON 导出、本机数据导出/导入/清空、搜索历史隐私开关、收藏、历史、筛选、类别路由、移动端底部主导航、PWA 安装元数据、service worker 离线 app shell 和基础渲染测试 | 移动端主流程重设计、拼音/纠错、原生分享、异常边界 |
...
| 测试、CI、质量与可观测性 | 8% | 43% | 3.44% | 有本地 lint/test/build、数据校验、首次启动引导测试、移动端导航链接测试、PWA service worker 注册和缓存清单测试、数据来源页路由/渲染/来源聚合测试、本机数据摘要/导出/导入/清空测试、搜索历史隐私开关测试、扫描页路由与渲染测试、搜索建议、分页、排序、风险分布、分类分布、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、数据审核摘要渲染、本地报告保存/导出/导入/检索/解读区块/来源引用测试、旧版报告兼容测试、OCR/AI 协议/响应校验/fallback/页面展示测试和 GitHub Actions 工作流 | E2E、移动端真机测试、支付沙盒测试、崩溃监控、覆盖率 |
| 上架、合规与运营 | 6% | 12% | 0.72% | README 有克制表达要求，首次启动引导要求确认数据草稿与使用边界，分析页有基础免责声明，食品数据已有来源与审核状态透明页，报告详情和导出包含来源引用与草稿审核状态，设置页支持本机数据导出、导入、清空和搜索历史记录关闭 | App Store/Google Play 资料、隐私政策、服务条款、数据安全表、客服渠道、订阅条款、账号删除 |
| 化妆品与多类别扩展 | 4% | 2% | 0.08% | 原有化妆品原型代码可复用结构 | 独立数据、字段、审核和类别运营 |

折算结果约 19.06%，当前按 **19%** 记录。
...
| 路由 | 基础完成（已扩展） | 66% | hash 路由支持首页、搜索、扫描、首次设置、详情、分析、报告列表/详情、收藏、设置、基础 404，并支持 `/food` 和 `/cosmetics` 前缀；路由切换同步 `document.title`，顶部导航根据类别更新链接和 active 态 | 更完整的参数边界、面包屑 | 类别层级、扫描/分析分层、首次设置、基础异常页、报告路由和导航状态已加，仍未到完整路由系统 |
...
| 移动端 App 基座 | Web Shell 起步 | 7% | 已新增 Web 小屏底部主导航、安全区底部预留、类别感知移动链接、扫描页 active 态、首次启动引导页、PWA manifest / 图标和 service worker 离线 app shell | iOS/Android 工程、原生启动屏、相机权限、文件权限、深链、真机验收 | 已开始移动体验外壳、扫描入口、首次设置和 PWA 离线基础，但还不是原生 App 工程 |
...
| UI 样式 | 部分完成 | 18% | `src/styles.css` 有基础视觉、响应式样式、首次启动引导样式和移动端底部主导航 | 移动端设计系统、过敏原警告样式、空态/加载/错误态、小屏真机验收 | 看起来有雏形，未验收 |
...
| 测试 | 很早期 | 41% | 函数断言覆盖搜索、搜索建议、搜索分页、搜索排序、风险分布筛选、分类分布筛选、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、数据审核摘要渲染、来源/版本聚合、数据来源页渲染、相关成分推荐、文本拆分、食品样例、过敏原警告、首次启动引导、路由解析、扫描页渲染、404、页面标题、详情页标题、历史删除、历史记录开关、移动端导航链接、PWA service worker 注册和缓存清单、本地报告保存/导出/导入/检索/解读区块/来源引用、旧版报告兼容、OCR/AI 协议/响应校验/fallback 和页面展示；已有 CI 工作流 | E2E、浏览器验收、移动端真机、支付沙盒、覆盖率 | 仍不能支撑移动端和付费上线 |
...
| 合规与内容治理 | 很早期 | 12% | README 有克制表达要求，首次启动引导有数据草稿和使用边界确认，分析页有基础免责声明，数据来源与审核状态页，设置页提供本机数据导出、导入、清空和搜索历史记录关闭 | 隐私政策、服务条款、订阅条款、数据安全表、客服入口、账号删除、数据授权、内容审核 | 上线前必须补正式政策和平台资料 |
...
| 单元测试 | 很早期 | 函数断言覆盖搜索、搜索建议、分类摘要、搜索排序、风险分布筛选、分类分布筛选、相关成分推荐、解析、过敏原、食品样例、首次启动引导、路由、404、页面标题、本机数据导出/导入/清空、本地报告保存、报告导出、AI 协议校验和降级服务 | 更多边界、错误路径、浏览器行为测试 | `scripts/test.mjs` |
