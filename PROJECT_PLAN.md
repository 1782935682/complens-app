...
2026-06-11 按新目标重新校准：主线仍是食品添加剂，但产品范围扩大为 iOS/Android 上线和付费订阅。前端基础框架（路由、搜索、收藏、存储）可复用；食品添加剂 schema、过敏原枚举、过敏原设置页、首次启动引导、本机数据导出/导入/清空、搜索历史隐私开关、数据校验脚本、50 条食品添加剂草稿种子数据、食品数据审核摘要展示、数据来源与审核状态页、食品详情字段基础展示、详情页相关成分推荐、详情页功能分类导航、详情页和数据页纠错反馈预填入口、首页分类筛选入口、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、搜索拼音/首字母/常见误写/近似纠错辅助、食品分析类别化 summary、真实包装成分表括号复配解析、分析置信度和解析质量面板、基础扫描页、扫描图片类型/大小校验、图片预览旋转/移除、扫描草稿本地保存/恢复、PWA service worker 离线 app shell、基础 404、页面标题、SPA 焦点管理、搜索筛选器、类别感知顶部导航、最近查询单条删除、本地分析报告保存、报告历史检索、报告 v2 解读区块、报告来源引用和 Markdown/JSON 导出、详情/报告/成分对比 Web Share 与复制 fallback、会员中心前端信息架构和订阅操作未接入提示、支持中心和本机反馈记录、隐私与条款合规材料入口、成分对比本机列表和横向对比页已完成。新增口径下，原生移动端工程、账号、订阅购买、服务端权益校验、商店审核、支付沙盒、真机测试和正式上线合规均未开始，因此总进度仍处于早期。
...
| 核心查询与分析体验 | 14% | 62% | 8.68% | 已有搜索、搜索建议、搜索结果分页、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、搜索拼音/首字母/常见误写/近似纠错辅助、成分对比本机列表、搜索/详情/收藏加入对比入口、横向对比页、详情/报告/成分对比 Web Share 与复制 fallback、首页/搜索/详情食品数据审核状态提示、详情页和数据页纠错反馈预填入口、首次启动引导、会员中心前端闭环、支持中心本机反馈闭环、数据来源与审核状态页、详情、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、食品/化妆品类别化文本分析 summary、真实包装成分表前缀/复配括号/剂量后缀解析、分析置信度和解析质量面板、扫描页图片预览、旋转预览、移除图片、校正入口和草稿恢复、分析页 AI 协议状态与本地 fallback 展示、本地报告保存、报告列表/详情、报告历史检索、报告解读区块、报告来源引用、Markdown/JSON 导出、本机数据导出/导入/清空、搜索历史隐私开关、收藏、历史、筛选、类别路由、移动端底部主导航、PWA 安装元数据、service worker 离线 app shell 和基础渲染测试 | 移动端主流程重设计、大规模搜索索引、异常边界 |
...
| 成分搜索 | 原型完成（逻辑可复用） | 46% | 中文名、英文名、别名、拼音、首字母、常见误写、近似纠错、分类、描述和食品 E-number/INS 编码本地搜索，支持按类别切换数据集、搜索建议、搜索结果分页、搜索结果排序、风险分布筛选、分类分布筛选、首页分类筛选入口、搜索结果加入对比，并支持风险等级和成分分类组合筛选；食品搜索页显示草稿数据审核状态和“可能相关”候选 | 大规模索引、真实搜索日志召回评估、更多食品数据搜索样例、移动端筛选交互 | 算法可复用，食品数据仍是草稿库 |
| 成分详情 | 原型完成（食品字段基础接入） | 30% | 展示名称、英文名、别名、功能、风险说明、收藏按钮、加入对比按钮和分享按钮；食品详情页已展示 E-number、INS、ADI、适用食品类别、GB 状态、草稿状态、来源链接、草稿审核提示和相关成分推荐，功能分类芯片可跳转到搜索筛选或关键词搜索，并可一键带入数据纠错反馈草稿 | 精确使用限量、字段解释、来源可信度标识、相关产品、来源可信度说明 | 食品字段、基础关联推荐、详情页横向浏览入口、分享入口和纠错入口已有展示，但还不是完整法规详情页 |
...
| 成分表文本分析 | 基础完成（真实包装格式起步） | 32% | 按分隔符拆分，支持 `配料：` 等标签前缀、复配添加剂括号展开、剂量/百分比后缀清理、单/双甘油脂肪酸酯保护，匹配本地库并展示匹配原因、置信度、覆盖率、低置信和暂未收录核对提示；食品/化妆品 summary 使用类别化语境，支持保存本地报告快照、导出 Markdown/JSON 和分享，并有真实食品样例测试 | 更多真实包装格式、复合原料归类、置信度校准、OCR 结果噪声纠错 | 已能验证更接近真实包装的文本分析闭环，但仍不是生产级解析器 |
...
| 成分对比 | 基础完成 | 23% | localStorage 保存同类别最多 4 个对比项，搜索结果、详情页和收藏页可加入对比，`/compare` 展示关注等级、分类、功能、提醒、过敏原、食品编码、GB 状态和数据状态，支持移出、清空、空状态、移动端横向表格、Web Share 和复制 fallback | 拖拽排序、云同步、真实移动端截图验收 | 已完成单机可验证用户流程和基础分享，但还不是跨端或 Pro 级对比能力 |
...
| 报告与同步 | 本地报告起步 | 25% | localStorage 保存分析报告快照，支持报告列表、详情、删除、清空、关键词检索、重新分析、类别隔离、风险/过敏原/数据边界/下一步建议解读区块、来源引用与审核状态、复制 Markdown、下载 Markdown、下载 JSON、Web Share 和复制 fallback，并可随本机数据快照整体导出、导入或清空 | 云同步、离线队列、PDF/图片导出 | 已完成单机闭环、基础导出/导入、历史检索、来源引用、本地报告解读骨架和移动端分享入口，但还不是 Pro 级报告系统 |
