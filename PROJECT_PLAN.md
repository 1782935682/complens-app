...
> 整体产品进度：**40%**（新口径：OCR 拍照识别产品闭环为核心，数据底座 + 数据库对接优先；订阅、支付、上架、iOS/Android 签名后置）
...
3. 历史收藏管理强化（F-A 产品档案基础已完成，F-B 尚未完成；报告仍缺 PDF/图片导出和跨设备验收）

**当前最早未完成任务**：见 [`CODEX_TASKS.md`](./CODEX_TASKS.md)
- 人工：`Data Batch 1-B：官方来源导入与逐条审核流程`
- Codex 可立即执行：`Batch F-B：历史列表与产品收藏管理`
