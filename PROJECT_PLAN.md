...
2026-06-11 按新目标重新校准：主线仍是食品添加剂，但产品范围扩大为 iOS/Android 上线和付费订阅。前端基础框架（路由、搜索、收藏、存储）可复用；食品添加剂 schema、过敏原枚举、过敏原设置页、本机数据导出/清空、数据校验脚本、50 条食品添加剂草稿种子数据、食品数据审核摘要展示、数据来源与审核状态页、食品详情字段基础展示、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、食品分析类别化 summary、基础扫描页、扫描图片类型/大小校验、图片预览旋转/移除、扫描草稿本地保存/恢复、PWA service worker 离线 app shell、基础 404、页面标题、SPA 焦点管理、搜索筛选器、类别感知顶部导航、最近查询单条删除、本地分析报告保存和 Markdown/JSON 导出已完成。新增口径下，原生移动端工程、账号、订阅、服务端权益校验、商店审核、支付沙盒、真机测试和上线合规均未开始，因此总进度仍处于早期。
...
| 核心查询与分析体验 | 14% | 49% | 6.86% | 已有搜索、搜索建议、搜索结果分页、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、首页/搜索/详情食品数据审核状态提示、数据来源与审核状态页、详情、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、食品/化妆品类别化文本分析 summary、扫描页图片预览、旋转预览、移除图片、校正入口和草稿恢复、本地报告保存、报告列表/详情、Markdown/JSON 导出、本机数据导出/清空、收藏、历史、筛选、类别路由、移动端底部主导航、PWA 安装元数据、service worker 离线 app shell 和基础渲染测试 | 移动端主流程重设计、拼音/纠错、原生分享、异常边界 |
| 过敏原与个人化档案 | 8% | 20% | 1.60% | 14 类过敏原枚举、localStorage 持久化、搜索/详情/分析基础警告，设置页可导出或清空本机过敏原档案 | 多端同步、真实数据逐条审核、隐私控制、提醒强度设置 |
...
| OCR 与 AI 分析能力 | 8% | 8% | 0.64% | 有扫描页图片选择、类型/大小校验、预览、旋转预览、移除图片、OCR 降级反馈、可编辑校正文本、草稿保存/恢复和本地文本分析 | 真实 OCR、图片预处理、模型代理、结构化报告、成本控制 |
| 测试、CI、质量与可观测性 | 8% | 35% | 2.80% | 有本地 lint/test/build、数据校验、移动端导航链接测试、PWA service worker 注册和缓存清单测试、数据来源页路由/渲染/来源聚合测试、本机数据摘要/导出/清空测试、扫描页路由与渲染测试、搜索建议、分页、排序、风险分布、分类分布、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、数据审核摘要渲染、本地报告保存与导出测试和 GitHub Actions 工作流 | E2E、移动端真机测试、支付沙盒测试、崩溃监控、覆盖率 |
| 上架、合规与运营 | 6% | 8% | 0.48% | README 有克制表达要求，分析页有基础免责声明，食品数据已有来源与审核状态透明页，设置页支持本机数据导出和清空 | App Store/Google Play 资料、隐私政策、服务条款、数据安全表、客服渠道、订阅条款、账号删除 |
| 化妆品与多类别扩展 | 4% | 2% | 0.08% | 原有化妆品原型代码可复用结构 | 独立数据、字段、审核和类别运营 |

折算结果约 16.54%，当前按 **17%** 记录。
...
| **过敏原用户档案** | **基础完成** | **20%** | 已定义 EU 14 类标准过敏原枚举 `src/data/allergens.js`，提供 `getUserAllergens()` / `setUserAllergens(ids)`，并新增设置页选择、保存、清空过敏原和本机数据导出/清空 | 多端同步、设置页说明增强、更多地区标准映射 | 单机可用，已能驱动搜索/详情/分析警告，并提供基础本机隐私控制 |
...
| 图片 OCR | 基础扫描闭环，真实识别未接入 | 6% | 独立扫描页支持拍照/相册文件选择、常见图片类型和 8 MB 大小校验、图片预览、旋转预览、移除图片、OCR 降级反馈、手动校正文本、本地草稿保存/恢复并跳转分析 | OCR 服务、图片压缩、方向修正、真实识别、裁剪、失败重试 | 已有可用入口和校正闭环，但不具备真实图片识别能力 |
...
| 报告与同步 | 本地报告起步 | 13% | localStorage 保存分析报告快照，支持报告列表、详情、删除、清空、重新分析、类别隔离、复制 Markdown、下载 Markdown、下载 JSON，并可随本机数据快照整体导出或清空 | 云同步、原生分享、历史检索、离线队列、报告来源引用、PDF/图片导出 | 已完成单机闭环和基础导出，但还不是 Pro 级报告系统 |
...
| 测试 | 很早期 | 33% | 函数断言覆盖搜索、搜索建议、搜索分页、搜索排序、风险分布筛选、分类分布筛选、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、数据审核摘要渲染、来源/版本聚合、数据来源页渲染、相关成分推荐、文本拆分、食品样例、过敏原警告、路由解析、扫描页渲染、404、页面标题、详情页标题、历史删除、移动端导航链接、PWA service worker 注册和缓存清单、本地报告保存/导出和 AI/OCR 降级；已有 CI 工作流 | E2E、浏览器验收、移动端真机、支付沙盒、覆盖率 | 仍不能支撑移动端和付费上线 |
...
| 合规与内容治理 | 很早期 | 8% | README 有克制表达要求，分析页有基础免责声明，数据来源与审核状态页，设置页提供本机数据导出和清空 | 隐私政策、服务条款、订阅条款、数据安全表、客服入口、账号删除、数据授权、内容审核 | 上线前必须补正式政策和平台资料 |
...
| 扫描页 | 基础完成（真实 OCR 未接入） | 独立 `/scan` 页面支持拍照/相册文件选择、图片类型和大小校验、图片预览、旋转预览、移除图片、OCR 降级反馈、手动录入/校正文本、本地草稿自动保存/恢复，并可跳转分析页 | 真实 OCR、图片裁剪、识别进度、失败重试、权限恢复、真机验收 | `src/pages/scanPage.js`、`src/router/router.js`、`src/main.js`、`src/store/userStore.js`、`src/utils/imageFile.js`、`scripts/test.mjs` |
...
| 本机数据与隐私控制 | 基础完成 | 设置页展示收藏、历史、报告、扫描草稿和过敏原数量；支持导出本机 JSON 快照和一键清空本机数据 | 导入恢复、账号删除、云端数据导出、正式隐私政策 | `src/pages/settingsPage.js`、`src/store/userStore.js`、`src/main.js`、`scripts/test.mjs` |
...
| 功能项 | 状态 | 已实现 | 还需要实现 | 真实评价 |
|---|---|---|---|---|
| 图片上传入口 | 基础完成 | 扫描页支持 image 文件选择、移动端相机 capture、常见图片类型和 8 MB 大小校验、图片预览、旋转预览、移除图片、OCR 降级提示、校正文本区和本地草稿恢复 | 压缩、裁剪、权限失败提示 | 入口可用但仍不识别 |
...
| 单元测试 | 很早期 | 函数断言覆盖搜索、搜索建议、分类摘要、搜索排序、风险分布筛选、分类分布筛选、相关成分推荐、解析、过敏原、食品样例、路由、404、页面标题、本地报告保存、报告导出和降级服务 | 更多边界、错误路径、浏览器行为测试 | `scripts/test.mjs` |
