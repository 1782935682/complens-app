...
| 核心查询与分析体验 | 14% | 44% | 6.16% | 已有搜索、搜索建议、搜索结果分页、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、首页/搜索/详情食品数据审核状态提示、详情、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、食品/化妆品类别化文本分析 summary、扫描页图片预览、旋转预览、移除图片、校正入口和草稿恢复、本地报告保存、报告列表/详情、Markdown/JSON 导出、收藏、历史、筛选、类别路由、移动端底部主导航、PWA 安装元数据和基础渲染测试 | 移动端主流程重设计、拼音/纠错、原生分享、异常边界 |
