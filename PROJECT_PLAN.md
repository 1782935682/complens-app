...
> 整体产品进度：**28%**（口径：iOS/Android 可上线 + 可付费订阅）
...
**当前瓶颈（P0）：** 移动端技术基座 → 后端/账号 → 订阅支付 → 合规材料。前端 Web 原型仍在 M1 迭代，M2 已完成 Vite、Capacitor 基础脚手架、原生 Camera/Share 适配层和 iOS 权限说明文档，M3 已完成 Hono API 骨架、食品添加剂数据库 schema、迁移、seed 和只读成分 API，但账号和同步仍未开始，M4–M7 完全未开始，决定整体进度的是 P0 基础设施，不是前端页面数量。

**当前最早未完成任务**：见 [`CODEX_TASKS.md`](./CODEX_TASKS.md) → `Batch 2-D：iOS 工程配置 [人工]`；下一个 Codex 批次为 `Batch 3-C：账号与鉴权`
...
| 后端 | Node/TypeScript API + 数据库 + 对象存储 + 队列 | Hono/TypeScript API 骨架、食品添加剂 schema/seed 和只读成分 API 已完成 | 账号、订阅校验、OCR、AI 都依赖后端 |
...
- 已有 `backend/` Hono/TypeScript API 骨架、健康检查、CORS、请求日志、全局 JSON 错误处理、Docker 和 Vitest 基础测试，但没有业务 API。
- 没有数据库。
- 没有账号系统。
...
| 后端 API、同步与安全基础设施 | 12% | 18% | 2.16% | 已有 `backend/` Node.js 20 + TypeScript + Hono 项目骨架、`GET /health`、CORS、请求日志、全局 JSON 错误处理、`.env.example`、Dockerfile、PostgreSQL 15 compose、Drizzle `ingredients` / `ingredient_sources` schema、迁移、100 条食品添加剂 seed、`/api/ingredients` 列表/分类/详情 API 和 Vitest 测试 | 鉴权、用户数据 API、对象存储、队列、限流、审计日志、密钥管理 |
...
| 后端 | 成分 API 基础完成 | 18% | `backend/` 已有 Node.js 20 + TypeScript + Hono 工程、`GET /health`、CORS、请求日志、全局 JSON 错误处理、`.env.example`、Dockerfile、PostgreSQL 15 compose、Drizzle `ingredients` / `ingredient_sources` schema、迁移、100 条 seed、`/api/ingredients` 列表/分类/详情 API、Vitest 测试和 CI 后端校验 | 账号、用户数据 API、任务队列、缓存、日志结构化、限流、鉴权、支付回调 | 正式产品核心已有只读成分 API，但用户、付费、同步和第三方代理仍未开始 |
...
状态：后端项目初始化、成分数据库 schema、seed 和只读成分 API 已完成，账号和云同步未开始。

| 任务 | 状态 | 验收标准 |
|---|---|---|
| 后端项目初始化 | 已完成 | API 服务可本地启动，有健康检查、CORS、配置管理、请求日志、全局 JSON 错误处理、Docker/Compose 和 Vitest 测试 |
| 数据库 schema | 部分完成 | `ingredients`、`ingredient_sources` 表已完成迁移和 100 条 seed；users、profiles、favorites、reports、entitlements 仍待后续批次 |
