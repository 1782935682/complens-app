...
> 整体产品进度：**24%**（口径：iOS/Android 可上线 + 可付费订阅）

| 里程碑 | 名称 | 状态 | 完成度 |
|---|---|---|---|
| M0 | 口径校准与开发规则 | ✅ 已完成 | 100% |
| M1 | Web/PWA 可用 MVP + 食品数据基础 | 🔄 进行中 | ~62% |
| M2 | 移动端 App 基座（Capacitor/原生工程） | 🔄 进行中 | ~10% |
| M3 | 后端、账号与云同步 | ⏳ 未开始 | 0% |
...
**当前最早未完成任务**：见 [`CODEX_TASKS.md`](./CODEX_TASKS.md) → `Batch 2-B：Capacitor 项目脚手架`
...
| 核心查询与分析体验 | 14% | 64% | 8.96% | 已有搜索、搜索建议、搜索结果分页、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、搜索拼音/首字母/常见误写/近似纠错辅助、搜索无结果热门分类入口、成分对比本机列表、搜索/详情/收藏加入对比入口、横向对比页、详情/报告/成分对比 Web Share 与复制 fallback、首页/搜索/详情食品数据审核状态提示、详情页和数据页纠错反馈预填入口、详情缺失成分友好页、首次启动引导、会员中心前端闭环、支持中心本机反馈闭环、数据来源与审核状态页、详情、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、食品/化妆品类别化文本分析 summary、分析页空输入防御、真实包装成分表前缀/复配括号/剂量后缀解析、分析置信度和解析质量面板、扫描页图片预览、旋转预览、移除图片、校正入口和草稿恢复、OCR 未接入手动输入提示、分析页 AI 协议状态与本地 fallback 展示、本地报告保存、报告列表/详情、报告历史检索、报告解读区块、报告来源引用、Markdown/JSON 导出、本机数据导出/导入/清空、搜索历史隐私开关、收藏、收藏空态搜索入口、历史、筛选、类别路由、移动端底部主导航、PWA 安装元数据、service worker 离线 app shell 和基础渲染测试 | 移动端主流程重设计、大规模搜索索引、异常边界 |
...
| 移动端 App 工程与跨端体验 | 12% | 12% | 1.44% | 已有 Vite 前端工程化基座、Web 小屏底部主导航、安全区底部预留、类别感知链接、独立扫描路由、首次启动引导页、成分对比页移动端布局、PWA manifest 和 service worker 离线 app shell | Capacitor 原生工程、iOS/Android 工程、原生相机权限、深链、推送、真机适配 |
...
| 开发脚本 | 基础完成 | 55% | `npm run dev`、`npm run build`、`npm run preview`、`npm run lint`、`npm run test`、`npm run validate:data` 已配置，仓库已有 GitHub Actions 工作流并执行 `npm ci` | 覆盖率、移动端构建脚本、发布流水线 | 本地和 PR 基础质量可控，但还不覆盖移动端 |
...
| 图片 OCR | 协议和基础扫描闭环，真实识别未接入 | 10% | 独立扫描页支持拍照/相册文件选择、常见图片类型和 8 MB 大小校验、图片预览、旋转预览、移除图片、OCR 协议状态展示、未接入手动输入提示、OCR 请求/响应 contract、响应校验、本地 fallback、手动校正文本、本地草稿保存/恢复并跳转分析 | OCR 服务端代理、图片压缩、方向修正、真实识别、裁剪、失败重试、成本控制 | 已有可用入口、校正闭环和可接服务端的结构化协议，但不具备真实图片识别能力 |
...
| 移动端 App 基座 | Web Shell + Vite 基座起步 | 14% | 已新增 Vite 前端工程化基座、Web 小屏底部主导航、安全区底部预留、类别感知移动链接、扫描页 active 态、首次启动引导页、成分对比移动端布局、会员中心移动端单列样式、PWA manifest / 图标和 service worker 离线 app shell | iOS/Android 工程、原生启动屏、相机权限、文件权限、深链、真机验收 | 已开始移动体验外壳、扫描入口、首次设置、对比入口、会员入口、Vite 构建和 PWA 离线基础，但还不是原生 App 工程 |
...
| PWA/移动安装元数据 | 基础实现 | 已添加 manifest、theme color、iOS Web App meta、SVG 应用图标、扫描快捷方式、service worker 注册、Vite public 静态资源、运行时缓存和离线首页回退 | 真机安装验收、PNG 多尺寸图标、商店素材、离线浏览器验收 | `src/index.html`、`public/manifest.webmanifest`、`public/app-icon.svg`、`src/main.js`、`public/sw.js`、`scripts/test.mjs` |
...
| 扫描页 | 基础完成（真实 OCR 未接入） | 独立 `/scan` 页面支持拍照/相册文件选择、图片类型和大小校验、图片预览、旋转预览、移除图片、OCR 协议状态展示、OCR 未接入手动输入提示、OCR 降级反馈、手动录入/校正文本、本地草稿自动保存/恢复，并可跳转分析页 | 真实 OCR、图片裁剪、识别进度、失败重试、权限恢复、真机验收 | `src/pages/scanPage.js`、`src/router/router.js`、`src/main.js`、`src/store/userStore.js`、`src/utils/imageFile.js`、`src/services/ocrService.js`、`scripts/test.mjs` |
...
状态：进行中，Vite 前端工程化基座已完成，原生 iOS/Android 工程未开始。
...
|---|---|---|---|
| 2026-06-11 | 完成 `CODEX_TASKS.md` Batch 2-A 前端工程化迁移：引入 Vite、更新 `npm run dev/build/preview`、新增 `vite.config.js`、将 manifest/icon/service worker 迁移到 `public/`，service worker 改为适配 Vite hash 产物的静态入口和运行时缓存，CI 使用 Node 20.19、增加 `npm ci` 并覆盖 Vite/public/lockfile 触发路径；整体产品进度更新为 24% | Codex | `npm run validate:data`、`npm run lint`、`npm run test`、`npm run build`、`git diff --check` 通过；5173/5174 被占用后使用 `npm run dev -- --host 127.0.0.1 --port 5173` 自动落到 5175，`curl -I http://127.0.0.1:5175/`、`/manifest.webmanifest`、`/sw.js` 和 `/main.js` HTTP 200 |
