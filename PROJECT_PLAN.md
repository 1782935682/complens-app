...
> 整体产品进度：**38%**（新口径：OCR 拍照识别产品闭环为核心，数据底座 + 数据库对接优先；订阅、支付、上架、iOS/Android 签名后置）
...
3. 产品档案与历史管理强化（F-A/F-B 尚未完成，报告仍缺 PDF/图片导出和跨设备验收）

**当前最早未完成任务**：见 [`CODEX_TASKS.md`](./CODEX_TASKS.md)
- 人工：`Data Batch 1-B：官方来源导入与逐条审核流程`
- Codex 可立即执行：`Batch F-A：产品档案与 IndexedDB 图片存储`
...
2026-06-12 重新校准：当前主线为食品添加剂 / 食品成分方向。已完成数据溯源字段补齐、本地数据统一未验证标识、Drizzle schema/migration/seed、只读成分 API、搜索 API、批量匹配 API、前端 API 优先和本地降级、详情页来源与可信等级展示、数据校验脚本增强、数据来源文档、OCR 拍照入口、图片预处理、IndexedDB 图片存储、OCR 抽象层、确认页、配料解析、数据库匹配和结构化食品配料分析报告；但正式数据导入、官方标准逐条核验、生产数据库、审核后台、发布流程、真实 OCR、真实 AI、产品档案、前端登录和跨设备验收仍未完成。
...
| 成分表文本分析 | 10% | 70% | 7.00% | `parseIngredientList` 支持 OCR 噪声修正、复配括号、E-number、顺序、去重、未知项保留；分析页展示数据库匹配摘要、低置信和未匹配项；报告页已结构化展示整体评级、关注摘要、配料顺序、添加剂分类和来源说明 | 更多真实包装格式、浏览器 E2E、PDF/图片导出 |
...
折算结果约 32.86%，考虑已有账号同步、移动端基座、结构化报告详情、OCR manual/fallback 主路径和测试基础，当前按 **38%** 记录。
...
| 路由 | 基础完成（已扩展） | 70% | hash 路由支持首页、搜索、扫描、首次设置、成分对比、会员中心、详情、分析、报告列表/详情、收藏、设置、基础 404，并支持 `/food` 和 `/cosmetics` 前缀；报告详情兼容 `/reports/:id` 和 `/report/:id`；路由切换同步 `document.title`，顶部导航根据类别更新链接和 active 态 | 更完整的参数边界、面包屑 | 类别层级、扫描/分析分层、首次设置、成分对比、会员中心、基础异常页、报告路由和导航状态已加，仍未到完整路由系统 |
...
| 报告与同步 | 本地报告闭环，登录态同步起步 | 45% | localStorage 保存分析报告快照，支持报告列表、结构化详情、删除、清空、关键词检索、重新分析、类别隔离、整体风险评级、关注摘要、配料顺序说明、添加剂分类统计、未收录项、特殊人群提示、来源说明、复制 Markdown、下载 Markdown、下载 JSON、原生分享 / Web Share / 复制 fallback，并可随本机数据快照整体导出、导入或清空；后端已提供 `/api/user/reports`，前端 storageService 在登录态可后台同步 | 离线同步队列、PDF/图片导出、原生文件持久化、真实跨设备验收 | 已完成单机结构化报告闭环和登录态同步基础，但还不是 Pro 级报告系统 |
| 化妆品成分模块 | 原型（待多类别改造） | 10% | 原有原型结构可复用 | 分离为 `/cosmetics/` 类别、化妆品专用数据和字段 | 后续迭代 |
| UI 样式 | 部分完成 | 23% | `src/styles.css` 有基础视觉、响应式样式、首次启动引导样式、成分对比卡片/表格/移动端样式、会员中心套餐/用量移动端样式、报告评级卡片/成分条目样式和移动端底部主导航 | 移动端设计系统、过敏原警告样式、空态/加载/错误态、小屏真机验收 | 看起来有雏形，未验收 |
...
| 测试 | 基础覆盖扩展中 | 62% | 函数断言覆盖搜索、解析、OCR 确认页、图片处理/IndexedDB 降级、批量匹配、数据库匹配摘要、报告评级/详情/导出/分享、移动端扫描入口、AI/OCR 协议/fallback；后端 Vitest 覆盖 health、CORS、404、500 JSON、ingredients API、batch-search、OCR 无 Key 降级、auth API、user sync API 和 JWT middleware；已有 CI 工作流 | E2E、浏览器验收、移动端真机、支付沙盒、覆盖率 | 单元/结构测试覆盖继续增强，但仍不能替代真机和浏览器 E2E |
