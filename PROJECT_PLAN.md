...
> 整体产品进度：**41%**（新口径：OCR 拍照识别产品闭环为核心，数据底座 + 数据库对接优先；订阅、支付、上架、iOS/Android 签名后置）
...
3. 前端账号登录 UI 和跨设备同步验收（F-A/F-B 已完成产品档案、历史列表和产品收藏管理；报告仍缺 PDF/图片导出）

**当前最早未完成任务**：见 [`CODEX_TASKS.md`](./CODEX_TASKS.md)
- 人工：`Data Batch 1-B：官方来源导入与逐条审核流程`
- Codex 可立即执行：`Batch Q-A：前端登录/注册页面`
...
2026-06-12 重新校准：当前主线为食品添加剂 / 食品成分方向。已完成数据溯源字段补齐、本地数据统一未验证标识、Drizzle schema/migration/seed、只读成分 API、搜索 API、批量匹配 API、前端 API 优先和本地降级、详情页来源与可信等级展示、数据校验脚本增强、数据来源文档、OCR 拍照入口、图片预处理、IndexedDB 图片存储、OCR 抽象层、确认页、配料解析、数据库匹配、结构化食品配料分析报告、产品档案、分析历史列表和产品收藏管理；但正式数据导入、官方标准逐条核验、生产数据库、审核后台、发布流程、真实 OCR、真实 AI、前端登录和跨设备验收仍未完成。
...
折算结果约 32.86%，考虑已有账号同步、移动端基座、结构化报告详情、OCR manual/fallback 主路径、产品档案/历史收藏闭环和测试基础，当前按 **41%** 记录。
...
| 首页 | 原型完成（已加类别入口） | 28% | 搜索入口、扫描入口、分析入口、收藏入口、历史、分类入口、首次设置提示、食品/化妆品类别切换；食品首页已有热门种子数据入口、数据状态摘要和最近分析产品缩略图入口 | 移动端首页信息架构、免费/Pro 价值展示、真实热门内容 | 食品类别可展示草稿数据状态和本机档案入口，但仍不是正式内容规模 |
| 路由 | 基础完成（已扩展） | 73% | hash 路由支持首页、搜索、扫描、首次设置、成分对比、会员中心、详情、分析、报告列表/详情、产品档案、分析历史、收藏、设置、基础 404，并支持 `/food` 和 `/cosmetics` 前缀；报告详情兼容 `/reports/:id` 和 `/report/:id`；路由切换同步 `document.title`，顶部导航根据类别更新链接和 active 态 | 更完整的参数边界、面包屑 | 类别层级、扫描/分析分层、首次设置、成分对比、会员中心、报告和历史路由已加，仍未到完整路由系统 |
...
| 收藏 | 基础完成，登录态同步起步 | 48% | localStorage 成分收藏、取消收藏、详情页按钮、收藏页展示、收藏页空态搜索入口、收藏页内联取消、收藏页加入对比、类别感知收藏记录；收藏页新增“收藏产品”Tab，展示产品缩略图、名称、评级和日期；后端已提供 `/api/user/favorites`，前端 storageService 在登录态读写会同步服务端并保持未登录本地行为 | 批量管理、离线同步队列、真实跨设备验收 | 单机成分/产品收藏流程可用，登录态云同步基础已接入，但还缺前端账号入口和跨设备验收 |
...
| 移动端 App 基座 | Web Shell + Capacitor 原生适配起步 | 28% | 已新增 Vite 前端工程化基座、Capacitor 依赖、`capacitor.config.json`、`cap:sync` / `cap:open:*` 命令、Camera/Filesystem/Share 插件、原生相机/相册入口、原生分享入口、Web 降级、viewport-fit、safe-area bottom、iOS 权限描述文档、Web 小屏底部主导航（首页/扫描/搜索/历史/我的）、类别感知移动链接、扫描页和历史页 active 态、首次启动引导页、成分对比移动端布局、会员中心移动端单列样式、PWA manifest / 图标和 service worker 离线 app shell | iOS/Android 工程人工签名配置、原生启动屏、权限恢复页、深链、推送、真机验收 | 已完成 Capacitor 可同步脚手架、基础原生能力适配和移动 Web Shell，但还不是可安装验收的原生 App |
...
| 产品档案与分析历史 | 基础完成，登录态同步起步 | 45% | 报告可保存为产品档案，完整图片保留 IndexedDB，localStorage 只保存 `imageId`、元数据和小缩略图；新增 `/products`、`/product/:id` 和 `/history`，历史页支持搜索、全部/收藏/高关注筛选、最多显示 50 条、清空和移动端左滑删除；首页展示最近分析产品，收藏页新增收藏产品 Tab；后端已提供 `/api/user/products` | 前端登录后真实跨设备验收、生产 CDN 图片存储、离线同步队列、真机图片读取和滑动手势验收 | 单机产品档案和历史回看闭环可用，跨设备和生产图片存储仍未完成 |
| 化妆品成分模块 | 原型（待多类别改造） | 10% | 原有原型结构可复用 | 分离为 `/cosmetics/` 类别、化妆品专用数据和字段 | 后续迭代 |
| UI 样式 | 部分完成 | 25% | `src/styles.css` 有基础视觉、响应式样式、首次启动引导样式、成分对比卡片/表格/移动端样式、会员中心套餐/用量移动端样式、报告评级卡片/成分条目样式、历史卡片左滑删除样式和移动端底部主导航 | 移动端设计系统、过敏原警告样式、空态/加载/错误态、小屏真机验收 | 看起来有雏形，未验收 |
...
| 测试 | 基础覆盖扩展中 | 63% | 函数断言覆盖搜索、解析、OCR 确认页、图片处理/IndexedDB 降级、批量匹配、数据库匹配摘要、报告评级/详情/导出/分享、产品档案、分析历史、收藏产品 Tab、首页最近分析、移动端扫描入口、AI/OCR 协议/fallback；后端 Vitest 覆盖 health、CORS、404、500 JSON、ingredients API、batch-search、OCR 无 Key 降级、auth API、user sync API 和 JWT middleware；已有 CI 工作流 | E2E、浏览器验收、移动端真机、支付沙盒、覆盖率 | 单元/结构测试覆盖继续增强，但仍不能替代真机和浏览器 E2E |
