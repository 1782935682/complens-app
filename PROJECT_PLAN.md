...
| 核心查询与分析体验 | 14% | 61% | 8.54% | 已有搜索、搜索建议、搜索结果分页、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、搜索拼音/首字母/常见误写/近似纠错辅助、成分对比本机列表、搜索/详情/收藏加入对比入口、横向对比页、详情/报告/成分对比 Web Share 与复制 fallback、首页/搜索/详情食品数据审核状态提示、详情页和数据页纠错反馈预填入口、首次启动引导、会员中心前端闭环、支持中心本机反馈闭环、数据来源与审核状态页、详情、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、食品/化妆品类别化文本分析 summary、扫描页图片预览、旋转预览、移除图片、校正入口和草稿恢复、分析页 AI 协议状态与本地 fallback 展示、本地报告保存、报告列表/详情、报告历史检索、报告解读区块、报告来源引用、Markdown/JSON 导出、本机数据导出/导入/清空、搜索历史隐私开关、收藏、历史、筛选、类别路由、移动端底部主导航、PWA 安装元数据、service worker 离线 app shell 和基础渲染测试 | 移动端主流程重设计、大规模搜索索引、异常边界 |
