...
2026-06-10 按新目标重新校准：主线仍是食品添加剂，但产品范围扩大为 iOS/Android 上线和付费订阅。前端基础框架（路由、搜索、收藏、存储）可复用；食品添加剂 schema、过敏原枚举、过敏原设置页、数据校验脚本、12 条食品添加剂草稿种子数据、食品详情字段基础展示、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、基础 404、页面标题、SPA 焦点管理、搜索筛选器、类别感知顶部导航、最近查询单条删除、本地分析报告保存和 Markdown/JSON 导出已完成。新增口径下，移动端工程、账号、订阅、服务端权益校验、商店审核、支付沙盒、真机测试和上线合规均未开始，因此总进度仍处于早期。
...
| 核心查询与分析体验 | 14% | 40% | 5.60% | 已有搜索、搜索建议、搜索结果分页、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、详情、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、文本分析、本地报告保存、报告列表/详情、Markdown/JSON 导出、收藏、历史、筛选、类别路由、移动端底部主导航、PWA 安装元数据和基础渲染测试 | 移动端主流程重设计、拼音/纠错、原生分享、异常边界 |
...
| 测试、CI、质量与可观测性 | 8% | 26% | 2.08% | 有本地 lint/test/build、数据校验、移动端导航链接测试、搜索建议、分页、排序、风险分布、分类分布测试、本地报告保存与导出测试和 GitHub Actions 工作流 | E2E、移动端真机测试、支付沙盒测试、崩溃监控、覆盖率 |
...
| 成分搜索 | 原型完成（逻辑可复用） | 39% | 中文名、英文名、别名、分类、描述和食品 E-number/INS 编码本地搜索，支持按类别切换数据集、搜索建议、搜索结果分页、搜索结果排序、风险分布筛选、分类分布筛选、首页分类筛选入口，并支持风险等级和成分分类组合筛选 | 大规模索引、拼音、错别字纠错、更多食品数据搜索样例、移动端筛选交互 | 算法可复用，食品数据规模仍很小 |
| 成分详情 | 原型完成（食品字段基础接入） | 25% | 展示名称、英文名、别名、功能、风险说明、收藏按钮；食品详情页已展示 E-number、INS、ADI、适用食品类别、GB 状态、草稿状态、来源链接和相关成分推荐，功能分类芯片可跳转到搜索筛选或关键词搜索 | 精确使用限量、字段解释、来源可信度标识、相关产品、来源可信度说明 | 食品字段、基础关联推荐和详情页横向浏览入口已有展示，但还不是完整法规详情页 |
...
| 测试 | 很早期 | 26% | 函数断言覆盖搜索、搜索建议、搜索分页、搜索排序、风险分布筛选、分类分布筛选、相关成分推荐、文本拆分、食品样例、过敏原警告、路由解析、404、页面标题、详情页标题、历史删除、移动端导航链接、本地报告保存/导出和 AI/OCR 降级；已有 CI 工作流 | E2E、浏览器验收、移动端真机、支付沙盒、覆盖率 | 仍不能支撑移动端和付费上线 |
...
| 搜索页 | 基础实现 | 展示结果数量、列表、无结果提示，支持按关注等级和成分分类筛选，根据输入展示匹配建议，并通过 `page` 参数分页展示结果，支持默认、关注等级优先和中文名排序，结果区展示风险分布和分类分布并可快速切换筛选 | 拼音/错别字纠错、排序解释增强、真实数据规模下的筛选性能 | `src/pages/searchPage.js`、`src/services/ingredientService.js` |
| 详情页 | 已实现原型 | 展示成分基础信息、收藏按钮、相关成分推荐，并支持点击功能分类芯片继续搜索或筛选 | 更完整法规说明、相关产品、来源可信度说明 | `src/pages/detailPage.js`、`src/services/ingredientService.js` |
...
| 增强搜索测试（食品方向） | 基础完成 | 已覆盖 E-number、INS 编码、食品成分表匹配、首页分类筛选入口、搜索排序、风险分布筛选、分类分布筛选、详情页相关成分、过敏原标注、路由解析、404、搜索筛选和筛选控件渲染；仍需补更多边界输入 |
