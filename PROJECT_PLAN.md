...
| 上架、合规与运营 | 6% | 13% | 0.78% | README 有克制表达要求，首次启动引导要求确认数据草稿与使用边界，分析页有基础免责声明，食品数据已有来源与审核状态透明页，报告详情和导出包含来源引用与草稿审核状态，会员中心明确真实权益必须由服务端和商店票据校验并可进入支持中心，设置页支持本机数据导出、导入、清空、搜索历史记录关闭和本机反馈记录 | App Store/Google Play 资料、隐私政策、服务条款、正式客服渠道、订阅条款、账号删除 |
...
| 测试 | 很早期 | 43% | 函数断言覆盖搜索、搜索建议、搜索分页、搜索排序、风险分布筛选、分类分布筛选、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、数据审核摘要渲染、来源/版本聚合、数据来源页渲染、相关成分推荐、文本拆分、食品样例、过敏原警告、首次启动引导、会员中心路由/渲染/服务、支持中心路由/渲染/本机反馈记录、成分对比路由/渲染/本机存储、路由解析、扫描页渲染、404、页面标题、详情页标题、历史删除、历史记录开关、移动端导航链接、PWA service worker 注册和缓存清单、本地报告保存/导出/导入/检索/解读区块/来源引用、旧版报告兼容、OCR/AI 协议/响应校验/fallback 和页面展示；已有 CI 工作流 | E2E、浏览器验收、移动端真机、支付沙盒、覆盖率 | 仍不能支撑移动端和付费上线 |
...
| 合规与内容治理 | 很早期 | 13% | README 有克制表达要求，首次启动引导有数据草稿和使用边界确认，分析页有基础免责声明，会员中心说明订阅和权益未接入且不能模拟购买成功并可进入支持中心，数据来源与审核状态页，设置页提供本机数据导出、导入、清空、搜索历史记录关闭、对比列表清理和本机反馈记录 | 隐私政策、服务条款、订阅条款、数据安全表、正式客服渠道、账号删除、数据授权、内容审核 | 上线前必须补正式政策和平台资料 |
