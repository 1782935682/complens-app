...
> 整体产品进度：**43%**（新口径：OCR 拍照识别产品闭环为核心，数据底座 + 数据库对接优先；订阅、支付、上架、iOS/Android 签名后置）
...
3. 跨设备同步验收与离线队列（前端登录入口已完成；F-A/F-B 已完成产品档案、历史列表和产品收藏管理；报告仍缺 PDF/图片导出）

**当前最早未完成任务**：见 [`CODEX_TASKS.md`](./CODEX_TASKS.md)
- 人工：`Data Batch 1-B：官方来源导入与逐条审核流程`
- Codex 可立即执行：`Batch U-A：关注成分、忌口项、过敏原与全局高亮`
...
折算结果约 32.86%，考虑已有账号同步、前端登录入口、移动端基座、结构化报告详情、OCR manual/fallback 主路径、产品档案/历史收藏闭环和测试基础，当前按 **43%** 记录。
...
| **过敏原用户档案** | **基础完成，登录态同步起步** | **34%** | 已定义 EU 14 类标准过敏原枚举 `src/data/allergens.js`，提供 `getUserAllergens()` / `setUserAllergens(ids)`，并新增首次启动引导和设置页选择、保存、清空过敏原、本机数据导出/导入/清空；后端已提供 `/api/user/allergens`，前端登录后会把本机快照写入登录态存储并由 storageService 后台同步 | 设置页说明增强、更多地区标准映射、真实跨设备验收、离线队列 | 单机可用，并已有前端登录入口和登录态云同步触发；仍缺真机/跨设备验收 |
...
| UI 样式 | 部分完成 | 27% | `src/styles.css` 有基础视觉、响应式样式、首次启动引导样式、登录/注册表单、成分对比卡片/表格/移动端样式、会员中心套餐/用量移动端样式、报告评级卡片/成分条目样式、历史卡片左滑删除样式和移动端底部主导航 | 移动端设计系统、过敏原警告样式、空态/加载/错误态、小屏真机验收 | 看起来有雏形，未验收 |
...
| 测试 | 基础覆盖扩展中 | 64% | 函数断言覆盖搜索、解析、OCR 确认页、图片处理/IndexedDB 降级、批量匹配、数据库匹配摘要、报告评级/详情/导出/分享、产品档案、分析历史、收藏产品 Tab、首页最近分析、登录/注册页渲染、JWT 过期清理、退出登录、本机数据登录态同步、移动端扫描入口、AI/OCR 协议/fallback；后端 Vitest 覆盖 health、CORS、404、500 JSON、ingredients API、batch-search、OCR 无 Key 降级、auth API、user sync API 和 JWT middleware；已有 CI 工作流 | E2E、浏览器验收、移动端真机、支付沙盒、覆盖率 | 单元/结构测试覆盖继续增强，但仍不能替代真机和浏览器 E2E |
...
| P1 | **跨设备同步验收** | 前端登录 UI 和后端账号 API 已完成基础接入 | 云同步和个性化设置尚未做真实多设备验收 | 验证登录后收藏、历史、过敏原、报告和产品档案元数据可跨设备恢复 |
...
| 云同步 | 基础完成 | 收藏、历史、过敏原档案、报告和产品档案已有 JWT 鉴权 API 或同步接口，前端登录后会把本机快照写入登录态存储并触发 storageService 后台同步；仍需离线队列和真实跨设备验收 |
