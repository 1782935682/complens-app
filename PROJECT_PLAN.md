...
2026-06-11 按新目标重新校准：主线仍是食品添加剂，但产品范围扩大为 iOS/Android 上线和付费订阅。前端基础框架（路由、搜索、收藏、存储）可复用；食品添加剂 schema、过敏原枚举、过敏原设置页、数据校验脚本、50 条食品添加剂草稿种子数据、食品数据审核摘要展示、食品详情字段基础展示、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、食品分析类别化 summary、基础扫描页、扫描草稿本地保存/恢复、基础 404、页面标题、SPA 焦点管理、搜索筛选器、类别感知顶部导航、最近查询单条删除、本地分析报告保存和 Markdown/JSON 导出已完成。新增口径下，移动端工程、账号、订阅、服务端权益校验、商店审核、支付沙盒、真机测试和上线合规均未开始，因此总进度仍处于早期。
...
| 核心查询与分析体验 | 14% | 44% | 6.16% | 已有搜索、搜索建议、搜索结果分页、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、首页/搜索/详情食品数据审核状态提示、详情、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、食品/化妆品类别化文本分析 summary、扫描页图片预览、校正入口和草稿恢复、本地报告保存、报告列表/详情、Markdown/JSON 导出、收藏、历史、筛选、类别路由、移动端底部主导航、PWA 安装元数据和基础渲染测试 | 移动端主流程重设计、拼音/纠错、原生分享、异常边界 |
| 过敏原与个人化档案 | 8% | 18% | 1.44% | 14 类过敏原枚举、localStorage 持久化、搜索/详情/分析基础警告 | 多端同步、真实数据逐条审核、隐私控制、提醒强度设置 |
| 移动端 App 工程与跨端体验 | 12% | 2% | 0.24% | 已有 Web 小屏底部主导航、安全区底部预留、类别感知链接、独立扫描路由和 PWA manifest | Capacitor 或其他移动框架、iOS/Android 工程、原生相机权限、深链、推送、真机适配 |
...
| OCR 与 AI 分析能力 | 8% | 8% | 0.64% | 有扫描页图片选择、预览、OCR 降级反馈、可编辑校正文本、草稿保存/恢复和本地文本分析 | 真实 OCR、图片预处理、模型代理、结构化报告、成本控制 |
| 测试、CI、质量与可观测性 | 8% | 31% | 2.48% | 有本地 lint/test/build、数据校验、移动端导航链接测试、扫描页路由与渲染测试、搜索建议、分页、排序、风险分布、分类分布、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、数据审核摘要渲染、本地报告保存与导出测试和 GitHub Actions 工作流 | E2E、移动端真机测试、支付沙盒测试、崩溃监控、覆盖率 |
...
| 图片 OCR | 基础扫描闭环，真实识别未接入 | 6% | 独立扫描页支持拍照/相册文件选择、图片预览、OCR 降级反馈、手动校正文本、本地草稿保存/恢复并跳转分析 | OCR 服务、图片压缩、方向修正、真实识别、裁剪/旋转、失败重试 | 已有可用入口和校正闭环，但不具备真实图片识别能力 |
...
| 移动端 App 基座 | Web Shell 起步 | 4% | 已新增 Web 小屏底部主导航、安全区底部预留、类别感知移动链接、扫描页 active 态和 PWA manifest / 图标 | iOS/Android 工程、原生启动屏、相机权限、文件权限、深链、真机验收 | 已开始移动体验外壳和扫描入口，但还不是原生 App 工程 |
...
| 测试 | 很早期 | 31% | 函数断言覆盖搜索、搜索建议、搜索分页、搜索排序、风险分布筛选、分类分布筛选、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、数据审核摘要渲染、相关成分推荐、文本拆分、食品样例、过敏原警告、路由解析、扫描页渲染、404、页面标题、详情页标题、历史删除、移动端导航链接、本地报告保存/导出和 AI/OCR 降级；已有 CI 工作流 | E2E、浏览器验收、移动端真机、支付沙盒、覆盖率 | 仍不能支撑移动端和付费上线 |
...
| PWA/移动安装元数据 | 基础实现 | 已添加 manifest、theme color、iOS Web App meta、SVG 应用图标和扫描快捷方式 | 真机安装验收、PNG 多尺寸图标、商店素材 | `src/index.html`、`src/manifest.webmanifest`、`src/app-icon.svg`、`scripts/test.mjs` |
| 首次启动引导 | 未开始 | 无 | 说明产品边界、选择关注类别、设置过敏原、解释隐私和相机权限 | 上线转化和合规都需要 |
| 扫描页 | 基础开始 | 独立 `/scan` 页面支持拍照/相册文件选择、图片预览、OCR 降级反馈、手动录入/校正文本、本地草稿自动保存/恢复，并可跳转分析页 | 真实 OCR、图片裁剪/旋转、识别进度、失败重试、权限恢复、真机验收 | `src/pages/scanPage.js`、`src/router/router.js`、`src/main.js`、`src/store/userStore.js`、`scripts/test.mjs` |
...
| 功能项 | 状态 | 已实现 | 还需要实现 | 真实评价 |
|---|---|---|---|---|
| 图片上传入口 | 基础完成 | 扫描页支持 image 文件选择、移动端相机 capture、图片预览、OCR 降级提示、校正文本区和本地草稿恢复 | 压缩、格式校验增强、裁剪/旋转、权限失败提示 | 入口可用但仍不识别 |
...
| 移动端 Web 适配 | 基础开始 | 已新增小屏底部主导航、隐藏桌面顶部导航、为主内容预留底部安全区，补充独立扫描页和 PWA 扫描快捷方式；仍需真实视口截图验收 |
...
|---|---|---|---|
| 2026-06-11 | 新增独立扫描页基础闭环：移动端扫描入口改为 `/scan`，页面支持拍照/相册选择、图片预览、OCR 降级反馈、可编辑校正文本、本地草稿自动保存/恢复并跳转成分表分析；扫描预览限制为常见 raster 图片类型；同步 PWA 扫描快捷方式、路由测试和进度到 14% | Codex | `npm run validate:data`、`npm run lint`、`npm run test`、`npm run build`、`git diff --check` 通过；`curl -I http://127.0.0.1:5173/`、`/pages/scanPage.js` 和 `/store/userStore.js` HTTP 200 |
