...
2026-06-11 按新目标重新校准：主线仍是食品添加剂，但产品范围扩大为 iOS/Android 上线和付费订阅。前端基础框架（路由、搜索、收藏、存储）可复用；食品添加剂 schema、过敏原枚举、过敏原设置页、数据校验脚本、50 条食品添加剂草稿种子数据、食品数据审核摘要展示、数据来源与审核状态页、食品详情字段基础展示、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、食品分析类别化 summary、基础扫描页、扫描图片类型/大小校验、图片预览旋转/移除、扫描草稿本地保存/恢复、PWA service worker 离线 app shell、基础 404、页面标题、SPA 焦点管理、搜索筛选器、类别感知顶部导航、最近查询单条删除、本地分析报告保存和 Markdown/JSON 导出已完成。新增口径下，原生移动端工程、账号、订阅、服务端权益校验、商店审核、支付沙盒、真机测试和上线合规均未开始，因此总进度仍处于早期。
...
| 核心查询与分析体验 | 14% | 46% | 6.44% | 已有搜索、搜索建议、搜索结果分页、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、首页/搜索/详情食品数据审核状态提示、数据来源与审核状态页、详情、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、食品/化妆品类别化文本分析 summary、扫描页图片预览、旋转预览、移除图片、校正入口和草稿恢复、本地报告保存、报告列表/详情、Markdown/JSON 导出、收藏、历史、筛选、类别路由、移动端底部主导航、PWA 安装元数据、service worker 离线 app shell 和基础渲染测试 | 移动端主流程重设计、拼音/纠错、原生分享、异常边界 |
...
| 测试、CI、质量与可观测性 | 8% | 33% | 2.64% | 有本地 lint/test/build、数据校验、移动端导航链接测试、PWA service worker 注册和缓存清单测试、数据来源页路由/渲染/来源聚合测试、扫描页路由与渲染测试、搜索建议、分页、排序、风险分布、分类分布、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、数据审核摘要渲染、本地报告保存与导出测试和 GitHub Actions 工作流 | E2E、移动端真机测试、支付沙盒测试、崩溃监控、覆盖率 |
| 上架、合规与运营 | 6% | 3% | 0.18% | README 有克制表达要求，分析页有基础免责声明，食品数据已有来源与审核状态透明页 | App Store/Google Play 资料、隐私政策、数据安全表、客服渠道、订阅条款 |
| 化妆品与多类别扩展 | 4% | 2% | 0.08% | 原有化妆品原型代码可复用结构 | 独立数据、字段、审核和类别运营 |

折算结果约 15.50%，当前按 **16%** 记录。
