...
| 核心查询与分析体验 | 14% | 56% | 7.84% | 已有搜索、搜索建议、搜索结果分页、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、首页/搜索/详情食品数据审核状态提示、首次启动引导、会员中心前端闭环、数据来源与审核状态页、详情、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、食品/化妆品类别化文本分析 summary、扫描页图片预览、旋转预览、移除图片、校正入口和草稿恢复、分析页 AI 协议状态与本地 fallback 展示、本地报告保存、报告列表/详情、报告历史检索、报告解读区块、报告来源引用、Markdown/JSON 导出、本机数据导出/导入/清空、搜索历史隐私开关、收藏、历史、筛选、类别路由、移动端底部主导航、PWA 安装元数据、service worker 离线 app shell 和基础渲染测试 | 移动端主流程重设计、拼音/纠错、原生分享、异常边界 |
...
| 移动端 App 工程与跨端体验 | 12% | 6% | 0.72% | 已有 Web 小屏底部主导航、安全区底部预留、类别感知链接、独立扫描路由、首次启动引导页、PWA manifest 和 service worker 离线 app shell | Capacitor 或其他移动框架、iOS/Android 工程、原生相机权限、深链、推送、真机适配 |
...
| 路由 | 基础完成（已扩展） | 67% | hash 路由支持首页、搜索、扫描、首次设置、会员中心、详情、分析、报告列表/详情、收藏、设置、基础 404，并支持 `/food` 和 `/cosmetics` 前缀；路由切换同步 `document.title`，顶部导航根据类别更新链接和 active 态 | 更完整的参数边界、面包屑 | 类别层级、扫描/分析分层、首次设置、会员中心、基础异常页、报告路由和导航状态已加，仍未到完整路由系统 |
| 成分搜索 | 原型完成（逻辑可复用） | 40% | 中文名、英文名、别名、分类、描述和食品 E-number/INS 编码本地搜索，支持按类别切换数据集、搜索建议、搜索结果分页、搜索结果排序、风险分布筛选、分类分布筛选、首页分类筛选入口，并支持风险等级和成分分类组合筛选；食品搜索页显示草稿数据审核状态 | 大规模索引、拼音、错别字纠错、更多食品数据搜索样例、移动端筛选交互 | 算法可复用，食品数据仍是草稿库 |
| 成分详情 | 原型完成（食品字段基础接入） | 26% | 展示名称、英文名、别名、功能、风险说明、收藏按钮；食品详情页已展示 E-number、INS、ADI、适用食品类别、GB 状态、草稿状态、来源链接、草稿审核提示和相关成分推荐，功能分类芯片可跳转到搜索筛选或关键词搜索 | 精确使用限量、字段解释、来源可信度标识、相关产品、来源可信度说明 | 食品字段、基础关联推荐和详情页横向浏览入口已有展示，但还不是完整法规详情页 |
...
| 移动端 App 基座 | Web Shell 起步 | 8% | 已新增 Web 小屏底部主导航、安全区底部预留、类别感知移动链接、扫描页 active 态、首次启动引导页、会员中心移动端单列样式、PWA manifest / 图标和 service worker 离线 app shell | iOS/Android 工程、原生启动屏、相机权限、文件权限、深链、真机验收 | 已开始移动体验外壳、扫描入口、首次设置、会员入口和 PWA 离线基础，但还不是原生 App 工程 |
...
| UI 样式 | 部分完成 | 19% | `src/styles.css` 有基础视觉、响应式样式、首次启动引导样式、会员中心套餐/用量移动端样式和移动端底部主导航 | 移动端设计系统、过敏原警告样式、空态/加载/错误态、小屏真机验收 | 看起来有雏形，未验收 |
...
| 测试 | 很早期 | 42% | 函数断言覆盖搜索、搜索建议、搜索分页、搜索排序、风险分布筛选、分类分布筛选、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、数据审核摘要渲染、来源/版本聚合、数据来源页渲染、相关成分推荐、文本拆分、食品样例、过敏原警告、首次启动引导、会员中心路由/渲染/服务、路由解析、扫描页渲染、404、页面标题、详情页标题、历史删除、历史记录开关、移动端导航链接、PWA service worker 注册和缓存清单、本地报告保存/导出/导入/检索/解读区块/来源引用、旧版报告兼容、OCR/AI 协议/响应校验/fallback 和页面展示；已有 CI 工作流 | E2E、浏览器验收、移动端真机、支付沙盒、覆盖率 | 仍不能支撑移动端和付费上线 |
...
| 合规与内容治理 | 很早期 | 13% | README 有克制表达要求，首次启动引导有数据草稿和使用边界确认，分析页有基础免责声明，会员中心说明订阅和权益未接入且不能模拟购买成功，数据来源与审核状态页，设置页提供本机数据导出、导入、清空和搜索历史记录关闭 | 隐私政策、服务条款、订阅条款、数据安全表、客服入口、账号删除、数据授权、内容审核 | 上线前必须补正式政策和平台资料 |
...
| 搜索页 | 基础实现 | 展示结果数量、列表、无结果提示，支持按关注等级和成分分类筛选，根据输入展示匹配建议，并通过 `page` 参数分页展示结果，支持默认、关注等级优先和中文名排序，结果区展示风险分布和分类分布并可快速切换筛选；食品类别显示草稿数据审核状态 | 拼音/错别字纠错、排序解释增强、真实数据规模下的筛选性能 | `src/pages/searchPage.js`、`src/services/ingredientService.js` |
| 详情页 | 已实现原型 | 展示成分基础信息、收藏按钮、相关成分推荐，并支持点击功能分类芯片继续搜索或筛选；食品详情显示草稿审核提示 | 更完整法规说明、相关产品、来源可信度说明 | `src/pages/detailPage.js`、`src/services/ingredientService.js` |
...
| PWA/移动安装元数据 | 基础实现 | 已添加 manifest、theme color、iOS Web App meta、SVG 应用图标、扫描快捷方式、service worker 注册、核心资源缓存、首次启动页面缓存和离线首页回退 | 真机安装验收、PNG 多尺寸图标、商店素材、离线浏览器验收 | `src/index.html`、`src/manifest.webmanifest`、`src/app-icon.svg`、`src/main.js`、`src/sw.js`、`scripts/test.mjs` |
...
| 本机数据与隐私控制 | 基础完成 | 设置页展示收藏、历史、报告、扫描草稿和过敏原数量；支持导出本机 JSON 快照、从 JSON 快照导入恢复和一键清空本机数据；首次启动偏好会随本机数据快照导出/导入 | 账号删除、云端数据导出、正式隐私政策 | `src/pages/settingsPage.js`、`src/pages/onboardingPage.js`、`src/store/userStore.js`、`src/main.js`、`scripts/test.mjs` |
...
| 单元测试 | 很早期 | 函数断言覆盖搜索、搜索建议、分类摘要、搜索排序、风险分布筛选、分类分布筛选、相关成分推荐、解析、过敏原、食品样例、首次启动引导、会员中心、路由、404、页面标题、本机数据导出/导入/清空、本地报告保存、报告导出、AI 协议校验和降级服务 | 更多边界、错误路径、浏览器行为测试 | `scripts/test.mjs` |
...
| PWA 辅助入口 | 基础实现 | 已有 manifest、theme color、iOS Web App meta、应用图标、service worker 注册和基础离线 app shell | iOS/Android 添加到主屏幕后完成启动、图标、显示模式和离线启动验收 |
...
| 增强搜索测试（食品方向） | 基础完成 | 已覆盖 E-number、INS 编码、新增食品数据编码入口、食品成分表匹配、首页分类筛选入口、搜索排序、风险分布筛选、分类分布筛选、详情页相关成分、过敏原标注、路由解析、404、搜索筛选和筛选控件渲染；仍需补更多边界输入 |
