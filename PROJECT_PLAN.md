...
| 核心查询与分析体验 | 14% | 57% | 7.98% | 已有搜索、搜索建议、搜索结果分页、搜索结果排序、搜索风险分布筛选、搜索分类分布筛选、成分对比本机列表、搜索/详情/收藏加入对比入口、横向对比页、首页/搜索/详情食品数据审核状态提示、首次启动引导、会员中心前端闭环、支持中心本机反馈闭环、数据来源与审核状态页、详情、详情页相关成分推荐、详情页功能分类导航、首页分类筛选入口、食品/化妆品类别化文本分析 summary、扫描页图片预览、旋转预览、移除图片、校正入口和草稿恢复、分析页 AI 协议状态与本地 fallback 展示、本地报告保存、报告列表/详情、报告历史检索、报告解读区块、报告来源引用、Markdown/JSON 导出、本机数据导出/导入/清空、搜索历史隐私开关、收藏、历史、筛选、类别路由、移动端底部主导航、PWA 安装元数据、service worker 离线 app shell 和基础渲染测试 | 移动端主流程重设计、拼音/纠错、原生分享、异常边界 |
...
| 测试、CI、质量与可观测性 | 8% | 46% | 3.68% | 有本地 lint/test/build、数据校验、首次启动引导测试、会员中心路由/渲染/服务测试、支持中心路由/渲染/本机反馈测试、隐私与条款路由/渲染/入口测试、成分对比路由/渲染/本机存储测试、移动端导航链接测试、PWA service worker 注册和缓存清单测试、数据来源页路由/渲染/来源聚合测试、本机数据摘要/导出/导入/清空测试、搜索历史隐私开关测试、扫描页路由与渲染测试、搜索建议、分页、排序、风险分布、分类分布、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、数据审核摘要渲染、本地报告保存/导出/导入/检索/解读区块/来源引用测试、旧版报告兼容测试、OCR/AI 协议/响应校验/fallback/页面展示测试和 GitHub Actions 工作流 | E2E、移动端真机测试、支付沙盒测试、崩溃监控、覆盖率 |
| 上架、合规与运营 | 6% | 18% | 1.08% | README 有克制表达要求，首次启动引导要求确认数据草稿与使用边界，分析页有基础免责声明，食品数据已有来源与审核状态透明页，报告详情和导出包含来源引用与草稿审核状态，会员中心明确真实权益必须由服务端和商店票据校验并可进入支持中心，设置页支持本机数据导出、导入、清空、搜索历史记录关闭和本机反馈记录，应用内已有隐私政策、服务条款、订阅说明、数据安全和内容来源说明草案入口 | App Store/Google Play 资料、正式隐私政策、正式服务条款、正式客服渠道、订阅条款法务复核、账号删除 |
| 化妆品与多类别扩展 | 4% | 2% | 0.08% | 原有化妆品原型代码可复用结构 | 独立数据、字段、审核和类别运营 |

折算结果约 20.36%，当前按 **20%** 记录。
...
| 成分详情 | 原型完成（食品字段基础接入） | 27% | 展示名称、英文名、别名、功能、风险说明、收藏按钮和加入对比按钮；食品详情页已展示 E-number、INS、ADI、适用食品类别、GB 状态、草稿状态、来源链接、草稿审核提示和相关成分推荐，功能分类芯片可跳转到搜索筛选或关键词搜索 | 精确使用限量、字段解释、来源可信度标识、相关产品、来源可信度说明 | 食品字段、基础关联推荐和详情页横向浏览入口已有展示，但还不是完整法规详情页 |
...
| 测试 | 很早期 | 44% | 函数断言覆盖搜索、搜索建议、搜索分页、搜索排序、风险分布筛选、分类分布筛选、新增食品编码/分类数据、高关注筛选、食品分析 summary 文案、数据审核摘要渲染、来源/版本聚合、数据来源页渲染、相关成分推荐、文本拆分、食品样例、过敏原警告、首次启动引导、会员中心路由/渲染/服务、支持中心路由/渲染/本机反馈记录、隐私与条款路由/渲染/入口、成分对比路由/渲染/本机存储、路由解析、扫描页渲染、404、页面标题、详情页标题、历史删除、历史记录开关、移动端导航链接、PWA service worker 注册和缓存清单、本地报告保存/导出/导入/检索/解读区块/来源引用、旧版报告兼容、OCR/AI 协议/响应校验/fallback 和页面展示；已有 CI 工作流 | E2E、浏览器验收、移动端真机、支付沙盒、覆盖率 | 仍不能支撑移动端和付费上线 |
...
| 合规与内容治理 | 很早期 | 18% | README 有克制表达要求，首次启动引导有数据草稿和使用边界确认，分析页有基础免责声明，会员中心说明订阅和权益未接入且不能模拟购买成功并可进入支持中心，支持中心有本机反馈记录，数据来源与审核状态页，隐私与条款中心提供隐私政策、服务条款、订阅说明、数据安全和内容来源草案，设置页提供本机数据导出、导入、清空、搜索历史记录关闭、对比列表清理和本机反馈记录 | 正式隐私政策、正式服务条款、平台数据安全表、正式客服渠道、账号删除、数据授权、内容审核 | 上线前必须补正式政策、平台资料和法务复核 |
...
| 数据审核状态摘要 | 基础实现 | 食品首页展示草稿数量、分类数、已复核/验证数和限量覆盖；食品搜索页和详情页显示草稿审核提示；独立数据页展示来源清单、版本、审核状态和分类覆盖 | 审核后台、逐条条款状态、数据版本发布流程 | `src/services/ingredientService.js`、`src/pages/homePage.js`、`src/pages/searchPage.js`、`src/pages/detailPage.js`、`src/pages/dataPage.js`、`scripts/test.mjs` |
| 数据来源与审核页 | 基础完成 | `/data` 路由展示来源覆盖、限量覆盖、审核状态、数据版本、来源清单、分类覆盖和上线前缺口；首页数据状态卡片可跳转 | 来源条款逐条编号、数据版本发布记录、后台审核流 | `src/pages/dataPage.js`、`src/router/router.js`、`src/services/ingredientService.js`、`scripts/test.mjs` |
| 隐私与条款中心 | 草案入口完成 | `/legal` 展示隐私政策、服务条款、订阅说明、数据安全和内容来源说明草案；设置页、会员中心、支持中心和页脚可进入对应材料；未知文档 ID 进入 404 | 法务复核、平台隐私表/数据安全表对齐、真实 SDK/后端/支付/客服接入后更新 | `src/data/legalContent.js`、`src/pages/legalPage.js`、`src/router/router.js`、`src/pages/settingsPage.js`、`src/pages/membershipPage.js`、`src/pages/supportPage.js`、`scripts/test.mjs` |
...
| 详情页 | 已实现原型 | 展示成分基础信息、收藏按钮、加入对比按钮、相关成分推荐，并支持点击功能分类芯片继续搜索或筛选；食品详情显示草稿审核提示 | 更完整法规说明、相关产品、来源可信度说明 | `src/pages/detailPage.js`、`src/services/ingredientService.js` |
...
| 支持中心 | 本机基础版完成，正式客服未接入 | `/support` 可记录订阅、数据纠错、扫描分析、隐私数据和功能问题，并支持复制、删除、清空和随本机数据导出导入 | 上线前仍需接正式客服邮箱、工单后台、账号关联和 SLA |
...
| 支持中心基础版 | 基础完成 | `/support` 可从设置页和会员中心进入，保存本机反馈记录，支持复制、删除、清空，并随本机数据导出/导入/清空 | 补正式客服渠道、工单后台、账号关联和移动端真机验收 |
...
|---|---|---|---|
| 2026-06-11 | 新增隐私与条款产品闭环：新增 `/legal` 合规中心、隐私政策/服务条款/订阅说明/数据安全/内容来源草案、设置页/会员中心/支持中心/页脚入口、无效文档 404、移动端样式和 PWA shell 缓存更新；整体产品进度维持 20% | Codex | `npm run validate:data`、`npm run lint`、`npm run test`、`npm run build`、`git diff --check` 通过；`curl -I http://127.0.0.1:5178/`、`/pages/legalPage.js`、`/data/legalContent.js` 和 `/sw.js` HTTP 200 |
