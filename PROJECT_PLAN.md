...
> 整体产品进度：**26%**（口径：iOS/Android 可上线 + 可付费订阅）
...
**当前瓶颈（P0）：** 移动端技术基座 → 后端/账号 → 订阅支付 → 合规材料。前端 Web 原型仍在 M1 迭代，M2 已完成 Vite、Capacitor 基础脚手架、原生 Camera/Share 适配层和 iOS 权限说明文档，M3–M7 完全未开始，决定整体进度的是 P0 基础设施，不是前端页面数量。

**当前最早未完成任务**：见 [`CODEX_TASKS.md`](./CODEX_TASKS.md) → `Batch 2-D：iOS 工程配置 [人工]`
...
当前阶段：第二阶段 / 移动端 App 基座已开始，付费和后端能力未开始
...
折算结果约 25.82%，当前按 **26%** 记录。

## 5. 已实现与待实现总表

| 模块 | 当前状态 | 模块完成度 | 已实现 | 需要实现 | 真实评价 |
|---|---|---:|---|---|---|
| 项目结构 | 部分完成 | 30% | `src/components`、`src/pages`、`src/router`、`src/services`、`src/store`、`src/data`、`src/utils` 已建立，Vite 配置和 `public/` 静态资源目录已接入 | TypeScript 评估、移动端工程、API client、平台能力封装 | 能继续验证业务并已有标准前端构建，但还不是移动端可上线工程 |
| 开发脚本 | 基础完成 | 60% | `npm run dev`、`npm run build`、`npm run preview`、`npm run lint`、`npm run test`、`npm run validate:data`、`npm run cap:sync`、`npm run cap:open:ios`、`npm run cap:open:android` 已配置，仓库已有 GitHub Actions 工作流并执行 `npm ci` | 覆盖率、发布流水线、移动端真机脚本 | 本地、PR 和 Capacitor 同步基础命令可控，但还不覆盖真机和发布 |
...
| 单元测试 | 很早期 | 函数断言覆盖搜索、搜索建议、分类摘要、搜索排序、风险分布筛选、分类分布筛选、相关成分推荐、解析、过敏原、食品样例、首次启动引导、会员中心、支持中心、隐私与条款、成分对比、路由、404、页面标题、本机数据导出/导入/清空、本地报告保存、报告导出、Capacitor 插件版本、非 native Camera/Share 降级、viewport-fit、安全区、iOS 权限文档、AI 协议校验和降级服务 | 更多边界、错误路径、浏览器行为测试 | `scripts/test.mjs` |
