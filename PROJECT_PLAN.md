...
> 整体产品进度：**36%**（新口径：OCR 拍照识别产品闭环为核心，数据底座 + 数据库对接优先；订阅、支付、上架、iOS/Android 签名后置）
...
| M2 | 数据库真实对接与 API 查询（本地完成，生产待补） | 🔄 进行中 | ~60% |
| **M3** | **OCR 拍照识别产品闭环**（核心，前置） | **🔄 进行中** | **60%** |
| M4 | 配料解析 + 成分匹配 + 分析报告 | 🔄 进行中 | 45% |
...
- Codex 可立即执行：`Batch R-A：食品配料分析报告页`
...
当前判断：项目已有 Web/PWA 原型、Vite 前端、Hono 后端、Drizzle/PostgreSQL schema、migration、seed、成分 API、账号/同步 API，以及 OCR manual/fallback 主路径、图片预处理、确认页、配料解析和批量匹配。数据仍是 100 条食品添加剂 seed 样本，不能视为完整成分库或可上线权威数据集。当前最重要的工作不是继续做订阅、支付、上架或 iOS/Android 签名，而是继续把可信数据、真实数据库、OCR 到分析报告的主路径做稳。
...
2026-06-12 重新校准：当前主线为食品添加剂 / 食品成分方向。已完成数据溯源字段补齐、本地数据统一未验证标识、Drizzle schema/migration/seed、只读成分 API、搜索 API、批量匹配 API、前端 API 优先和本地降级、详情页来源与可信等级展示、数据校验脚本增强、数据来源文档、OCR 拍照入口、图片预处理、IndexedDB 图片存储、OCR 抽象层、确认页、配料解析和数据库匹配；但正式数据导入、官方标准逐条核验、生产数据库、审核后台、发布流程、真实 OCR、AI 和完整报告页仍未完成。
...
| 数据库真实对接 | 16% | 60% | 9.60% | Drizzle/PostgreSQL schema、migration、seed、列表/详情/分类/search API、批量匹配 API 和本地数据库验证已完成 | 生产数据库、备份、数据发布流程、导入审计、运维监控 |
| 搜索和详情体验 | 12% | 50% | 6.00% | 前端食品搜索/详情优先请求后端 API，失败降级本地数据；详情页展示来源与可信等级，搜索结果标识未验证 | 大数据量索引、来源筛选、审核状态筛选、浏览器 E2E |
| 成分表文本分析 | 10% | 60% | 6.00% | `parseIngredientList` 支持 OCR 噪声修正、复配括号、E-number、顺序、去重、未知项保留；分析页展示数据库匹配摘要、低置信和未匹配项 | 更多真实包装格式、报告页结构化总结、浏览器 E2E |
| OCR 识别 | 6% | 45% | 2.70% | 拍照/相册入口、图片预处理、IndexedDB 存图、OCR real/manual/fallback 抽象、确认页和后端 `/api/ocr` 无 Key 降级已完成 | OCR API Key、供应商适配、真实识别、额度/限流、真机验收 |
...
折算结果约 31.86%，考虑已有账号同步、移动端基座、报告历史、OCR manual/fallback 主路径和测试基础，当前按 **36%** 记录。
...
| 成分表文本分析 | 基础完成（OCR 文本解析和匹配起步） | 60% | `parseIngredientList` 输出结构化配料项，支持 OCR 中文空格/换行噪声修正、`配料：` 等前缀、复配添加剂括号展开、剂量/百分比后缀清理、E-number 提取、顺序保留、重复标记、未知项保留；分析页展示数据库匹配摘要、低置信和未匹配项 | 更多真实包装格式、复合原料归类、置信度校准、结构化报告页 | 已能完成确认文本到数据库匹配的基础闭环，但仍不是生产级解析器 |
| 图片 OCR | manual/fallback 主路径完成，真实识别未接入 | 45% | 独立扫描页支持拍照/相册、Web 文件选择降级、8 MB 校验、图片预处理、EXIF 修正、压缩、IndexedDB 存图、OCR real/manual/fallback 抽象、后端 `/api/ocr` 鉴权占位、无 Key 503 降级、确认页编辑和产品名称字段 | OCR API Key、供应商适配、真实识别、裁剪、重试、额度/限流、真机验收 | 用户可在无 Key 情况下完成拍照/手动确认/解析/匹配主路径，但真实 OCR 识别仍未完成 |
...
| 测试 | 基础覆盖扩展中 | 60% | 函数断言覆盖搜索、解析、OCR 确认页、图片处理/IndexedDB 降级、批量匹配、数据库匹配摘要、移动端扫描入口、AI/OCR 协议/fallback；后端 Vitest 覆盖 health、CORS、404、500 JSON、ingredients API、batch-search、OCR 无 Key 降级、auth API、user sync API 和 JWT middleware；已有 CI 工作流 | E2E、浏览器验收、移动端真机、支付沙盒、覆盖率 | 单元/结构测试覆盖继续增强，但仍不能替代真机和浏览器 E2E |
| 后端 | 成分 API、OCR 占位、基础鉴权和用户同步完成 | 45% | `backend/` 已有 Node.js 20 + TypeScript + Hono 工程、`GET /health`、CORS、请求日志、全局 JSON 错误处理、`.env.example`、Dockerfile、PostgreSQL 15 compose、Drizzle schema、迁移、100 条 seed、`/api/ingredients` 列表/分类/详情/search/batch-search API、`/api/ocr` 鉴权占位和无 Key/供应商未接入降级、`/api/auth/*`、`/api/user/*` 同步 API、Vitest 测试和 CI 后端校验 | 真实 OCR/AI 供应商代理、任务队列、缓存、日志结构化、限流、支付回调、生产部署 | 正式产品已有成分 API、批量匹配、账号底座、用户同步和 OCR 安全占位，但付费、真实 OCR/AI 和生产安全仍未完成 |
