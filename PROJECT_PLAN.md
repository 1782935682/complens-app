...
> 整体产品进度：**28%**（新口径：可信食品添加剂数据底座 + 数据库真实对接优先；订阅、支付、OCR、AI、上架、iOS/Android 签名暂停推进）
...
本阶段主线按现有 `src/data/foodAdditives.js` 推进食品添加剂 / 食品成分方向，不继续混入新的化妆品、护肤品或药品数据。订阅、支付、OCR、AI、上架、iOS/Android 签名相关开发暂停，直到可信数据底座和数据库查询闭环稳定。

关键边界：

- 当前食品添加剂数据量为 100 条 seed 样本，全部标记为 `confidenceLevel: "unverified"`、`isVerified: false`，不能视为完整成分库。
- 数据库已完成本地/开发环境 Drizzle schema、migration、seed 和只读 API 对接；生产数据库未完成，不能标记为生产数据库已完成。
- OCR 和 AI 依赖可信数据底座，当前只保留既有协议和本地降级，不继续开发真实识别或模型总结。
- AI 只能做解释和总结，不能作为原始数据来源；任何未确认数据必须保留未验证标识。

| 里程碑 | 名称 | 状态 | 完成度 |
|---|---|---|---|
| M0 | 口径校准与开发规则 | ✅ 已完成 | 100% |
| M1 | 食品添加剂可信数据底座 | 🔄 进行中 | ~28% |
| M2 | 数据库真实对接与 API 查询 | 🔄 进行中 | ~55% |
| M3 | 搜索、详情和文本分析体验 | 🔄 进行中 | ~50% |
| M4 | OCR 识别 | ⏸ 暂缓 | 0% |
| M5 | AI 总结 | ⏸ 暂缓 | 0% |
| M6 | 订阅、支付、上架、iOS/Android 签名 | ⏸ 暂缓 | 0% |

**当前瓶颈（P0）：** 数据源准确性和数据完整度。现有 100 条食品添加剂 seed 已可通过本地数据库 migration/seed 导入并由 API 查询，前端食品搜索/详情优先请求后端 API、失败降级本地数据；但逐条法规条款、来源版本、生效日期、ADI 原文和逐食品类别限量仍未完成可信审核。

**当前最早未完成任务**：见 [`CODEX_TASKS.md`](./CODEX_TASKS.md) → `Data Batch 1-B：官方来源导入与逐条审核流程`
...
当前阶段：数据优先重排 / 可信食品添加剂数据底座 + 本地数据库真实对接闭环。

当前判断：项目已有 Web/PWA 原型、Vite 前端、Hono 后端、Drizzle/PostgreSQL schema、migration、seed 和只读 API，但数据仍是 100 条食品添加剂 seed 样本，不能视为完整成分库或可上线权威数据集。当前最重要的工作不是继续做订阅、支付、OCR、AI、上架或 iOS/Android 签名，而是先把来源、审核、数据库查询和前端可信展示闭环做稳。
...
- 已新增本地/开发环境数据库迁移和 seed，100 条食品添加剂 seed 可以导入 PostgreSQL，并通过 `GET /api/ingredients`、`GET /api/ingredients/:id`、`GET /api/ingredients/categories`、`GET /api/ingredients/search?q=` 查询。
- 前端食品搜索和食品详情优先请求后端 API，Vite 本地开发代理 `/api` 到后端；后端不可用时降级本地 seed，并明确显示加载、错误、空态和未验证数据标识。
- 当前 100 条食品添加剂数据统一标记为 `confidenceLevel: "unverified"`、`isVerified: false`，因为逐条法规条款、来源版本、生效日期、ADI 原文和逐食品类别限量仍未完成可信审核。
- 生产数据库未完成；当前仅完成本地数据库或开发环境数据库对接，不能写成“生产数据库已完成”。
- OCR 和 AI 依赖可信数据底座，当前暂缓继续开发；AI 只能用于解释和总结，不能作为原始数据来源。
...
2026-06-12 重新校准：当前主线为食品添加剂 / 食品成分方向。已完成数据溯源字段补齐、本地数据统一未验证标识、Drizzle schema/migration/seed、只读成分 API、搜索 API、前端 API 优先和本地降级、详情页来源与可信等级展示、数据校验脚本增强、数据来源文档；但正式数据导入、官方标准逐条核验、生产数据库、审核后台、发布流程、真实 OCR 和真实 AI 都未完成。
...
|---|---:|---:|---:|---|---|
| 数据源准确性 | 22% | 18% | 3.96% | 100 条食品添加剂 seed 已补齐来源字段，全部未验证，不把 AI 输出当权威来源 | 官方文件逐条核验、版本号、生效日期、条款编号、原文片段 |
| 数据完整度 | 18% | 20% | 3.60% | 当前覆盖常见食品添加剂类别，保留 `usageLimits` 空数组避免伪造限量 | GB 2760 全量、逐食品类别限量、ADI 原文、过敏原和特殊人群审核 |
| 数据库真实对接 | 16% | 55% | 8.80% | Drizzle/PostgreSQL schema、migration、seed、列表/详情/分类/search API 和本地数据库验证已完成 | 生产数据库、备份、数据发布流程、导入审计、运维监控 |
| 搜索和详情体验 | 12% | 50% | 6.00% | 前端食品搜索/详情优先请求后端 API，失败降级本地数据；详情页展示来源与可信等级，搜索结果标识未验证 | 大数据量索引、来源筛选、审核状态筛选、浏览器 E2E |
| 成分表文本分析 | 10% | 35% | 3.50% | 本地文本拆分、复配括号解析、匹配和置信度面板已可用 | 与数据库 API 的统一召回、更多包装格式、OCR 噪声容错 |
| OCR 识别 | 6% | 0% | 0.00% | 当前仅保留已有入口和本地手动输入降级 | 暂缓；可信数据底座完成后再接真实识别 |
...
折算结果约 25.86%，考虑已有通用前端、后端和测试基础，当前按 **28%** 记录。
