...
const outDir = getArg('--out-dir') || 'reports/product-review/screenshots/upload-card-smoke';
...
try {
  ...
  await step('enter-capture', async () => clickFirstVisible(page, ['拍照识别', '开始识别']));
  await page.waitForURL(/\/pages\/capture\/index/u, { timeout: 8_000 });
  await page.waitForTimeout(400);
  await screenshot(page, '02-capture');

  await step('click-upload-card-title', async () => {
