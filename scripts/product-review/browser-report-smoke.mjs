...
const outDir = getArg('--out-dir') || 'reports/product-review/screenshots/report-smoke';
...
try {
  ...
  for (const [index, text] of clickTexts.entries()) {
    ...
    await screenshot(page, `04-after-click-${index + 1}`);
  }
  ...
  const result = {
    schemaVersion: 'product-review-browser-report-smoke-v1',
  ...
  console.log(JSON.stringify({
    ...
    screenshots,
    networkErrors,
    missing: textAssertions.missing,
    forbiddenPresent: textAssertions.forbiddenPresent
  }, null, 2));
