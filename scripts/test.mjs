...
assert.match(backendEnvExample, /OCR_PROVIDER=rapidocr/);
assert.match(backendEnvExample, /OCR_SERVICE_URL=http:\/\/127\.0\.0\.1:8000/);
...
assert.equal(gb2760OfficialFAdditiveIndexRows.length, 287);
assert.equal(gb2760OfficialReferenceRows.length, 2800);
...
assert.equal(gb2760OfficialB2NaturalFlavorRows[0].flavorNameEn, 'Clove leaf oil (Eugenia spp.)');
...
const b2NaturalFlavorN059 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N059');
assert.equal(b2NaturalFlavorN059.flavorNameEn, 'Chrysanthemum Hang Zhou flower oil (Dendranthema morifolium or Chrysanthemum morifolium)');
...
assert.equal(b2NaturalFlavorN060.flavorNameEn, 'Chrysanthemum Hang Zhou flower extract (Dendranthema morifolium or Chrysanthemum morifolium)');
assert.doesNotMatch(b2NaturalFlavorN060.rawSourceText, /浸膏\)l/u);
assert.match(b2NaturalFlavorN060.rawSourceText, /Dendranthema morifolium or Chrysanthemum morifolium/u);
const b2NaturalFlavorN025 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N025');
assert.equal(b2NaturalFlavorN025.flavorNameEn, 'China nardostachys oil (Nardostachys chinensis Batal.)');
const b2NaturalFlavorN034 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N034');
assert.equal(b2NaturalFlavorN034.flavorNameEn, 'Lime oil [Citrus aurantifolia (Christman) Swingle]');
const b2NaturalFlavorN083 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N083');
assert.equal(b2NaturalFlavorN083.flavorNameEn, 'Pummelo peel oil [Citrus grandis (L.) Osbeck]');
const b2NaturalFlavorN086 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N086');
assert.equal(b2NaturalFlavorN086.flavorNameEn, 'Lemon oil [Citrus limon (L.) Burm.f.]');
const b2NaturalFlavorN090 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N090');
assert.equal(b2NaturalFlavorN090.flavorNameEn, 'Lemongrass oil (Cymbopogon citratus DC. and C. flexuosus)');
const b2NaturalFlavorN101 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N101');
assert.equal(b2NaturalFlavorN101.flavorNameEn, 'Bergamot oil (Citrus aurantium L. subsp. bergamia)');
const b2NaturalFlavorN127 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N127');
assert.equal(b2NaturalFlavorN127.flavorNameEn, 'Green tea tincture (Thea sinensis or Camellia sinensis)');
const b2NaturalFlavorN137 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N137');
assert.equal(b2NaturalFlavorN137.flavorNameEn, 'Peppermint oil (Mentha piperita L.)');
const b2NaturalFlavorN188 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N188');
assert.equal(b2NaturalFlavorN188.flavorNameEn, 'Bois de rose oil (Aniba rosaeodora Ducke)');
const b2NaturalFlavorN209 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N209');
assert.equal(b2NaturalFlavorN209.flavorNameEn, 'Mimosa absolute (Acacia decurrens Will. var. dealbata)');
const b2NaturalFlavorN010 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N010');
assert.equal(b2NaturalFlavorN010.flavorNameEn, 'Cardamom oil (Elletaria cardamomum)');
const b2NaturalFlavorN011 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N011');
assert.equal(b2NaturalFlavorN011.flavorNameEn, 'Cardamom tincture (Elletaria cardamomum)');
const b2NaturalFlavorN053 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N053');
assert.equal(b2NaturalFlavorN053.flavorNameEn, 'Chinese date (common Jujube) tincture (Ziziphus jujuba Mill.)');
const b2NaturalFlavorN072 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N072');
assert.equal(b2NaturalFlavorN072.flavorNameEn, 'Sarcodactylis oil (Citrus medica var. Sarcodactylis Swingle)');
const b2NaturalFlavorN268 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N268');
assert.equal(b2NaturalFlavorN268.flavorNameEn, 'Perilla leaf oil (Shiso oil) (Perilla frutescens)');
const b2NaturalFlavorN393 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N393');
assert.equal(b2NaturalFlavorN393.flavorNameEn, 'Date concrete (Ziziphus jujuba)');
const b2NaturalFlavorN035 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N035');
assert.equal(b2NaturalFlavorN035.flavorNameEn, 'Lime oil terpene');
const b2NaturalFlavorN036 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N036');
assert.equal(b2NaturalFlavorN036.flavorNameEn, 'Ginger oleoresin (Zingiber officinale Rosc.)');
const b2NaturalFlavorN075 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N075');
assert.equal(b2NaturalFlavorN075.flavorNameEn, 'Ginger oil (Zingiber officinale Rosc.)');
const b2NaturalFlavorN162 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N162');
assert.equal(b2NaturalFlavorN162.flavorNameEn, 'Ginger extract (Ginger concrete.) (Zingiber officinale)');
const b2NaturalFlavorN243 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N243');
assert.equal(b2NaturalFlavorN243.flavorNameEn, 'Angelica seed oil (Angelica archangelica L. or Angelica sinensis)');
const b2NaturalFlavorN244 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N244');
assert.equal(b2NaturalFlavorN244.flavorNameEn, 'Angelica root oil (Angelica archangelica L. or Angelica sinensis)');
const b2NaturalFlavorN262 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N262');
assert.equal(b2NaturalFlavorN262.flavorNameEn, 'Lime oil, expressed terpeneless (Citrus aurantifolia Swingle)');
const b2NaturalFlavorN279 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N279');
assert.equal(b2NaturalFlavorN279.flavorNameEn, 'Alfalfa extract (Medicago sativa L.)');
const b2NaturalFlavorN290 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N290');
assert.equal(b2NaturalFlavorN290.flavorNameEn, 'Clover tops red extract solid (Trifolium pratense L.)');
const b2NaturalFlavorN294 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N294');
assert.equal(b2NaturalFlavorN294.flavorNameEn, 'Mountain maple extract solid (Acer spicatum Lam.)');
const b2NaturalFlavorN313 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N313');
assert.equal(b2NaturalFlavorN313.flavorNameEn, 'Geranium extract (Pelargonium spp.)');
const b2NaturalFlavorN350 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N350');
assert.equal(b2NaturalFlavorN350.flavorNameEn, 'Licorice extract powder (Glycyrrhiza glabra L.)');
const b2NaturalFlavorN361 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N361');
assert.equal(b2NaturalFlavorN361.flavorNameEn, 'Civet absolute (Viverra civetta Schreber V. zibetha Schreber)');
const b2NaturalFlavorN396 = gb2760OfficialB2NaturalFlavorRows.find((row) => row.flavorCode === 'N396');
assert.equal(b2NaturalFlavorN396.flavorNameEn, 'Algin (Laminaria spp. and other kelps)');
assert.equal(
...
assert.equal(gb2760OfficialB3SyntheticFlavorRows[0].flavorNameEn, '1,2-Propanediol (Propylene glycol)');
const b3SyntheticFlavorRow3 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0003');
assert.equal(b3SyntheticFlavorRow3.flavorNameEn, 'Isopropyl alcohol');
const b3SyntheticFlavorRow4 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0004');
assert.equal(b3SyntheticFlavorRow4.flavorNameEn, '1-Butanol (Butyl alcohol)');
const b3SyntheticFlavorRow14 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0014');
assert.equal(b3SyntheticFlavorRow14.flavorNameEn, '1-Octanol (Octyl alcohol)');
const b3SyntheticFlavorRow22 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0022');
assert.equal(b3SyntheticFlavorRow22.flavorNameEn, '1-Decanol (Decyl alcohol)');
const b3SyntheticFlavorRow27 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0027');
assert.equal(b3SyntheticFlavorRow27.flavorNameEn, 'Leaf alcohol (cis-3-Hexen-1-ol)');
const b3SyntheticFlavorRow57 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0057');
assert.equal(b3SyntheticFlavorRow57.flavorNameEn, 'p,α-Dimethylbenzyl alcohol');
const b3SyntheticFlavorRow60 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0060');
assert.equal(b3SyntheticFlavorRow60.flavorNameEn, 'β-Caryophyllene alcohol');
const b3SyntheticFlavorRow65 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0065');
assert.equal(b3SyntheticFlavorRow65.flavorNameEn, 'Perilla alcohol');
const b3SyntheticFlavorRow70 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0070');
assert.equal(b3SyntheticFlavorRow70.flavorNameEn, '(E,E)-2,4-Decadien-1-ol (trans,trans-2,4-Decadien-1-ol)');
const b3SyntheticFlavorRow280 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0280');
assert.equal(b3SyntheticFlavorRow280.flavorNameEn, '1,5,5,9-Tetramethyl-13-oxatricyclo[8.3.0.0(4,9)]tridecane');
const b3SyntheticFlavorRow293 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0293');
assert.equal(b3SyntheticFlavorRow293.flavorNameEn, 'Acetic acid');
const b3SyntheticFlavorRow294 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0294');
assert.equal(b3SyntheticFlavorRow294.flavorNameEn, 'Propionic acid');
const b3SyntheticFlavorRow295 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0295');
assert.equal(b3SyntheticFlavorRow295.flavorNameEn, 'Pyruvic acid');
const b3SyntheticFlavorRow306 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0306');
assert.equal(b3SyntheticFlavorRow306.flavorNameEn, 'trans-2-Hexenoic acid');
const b3SyntheticFlavorRow312 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0312');
assert.equal(b3SyntheticFlavorRow312.flavorNameEn, 'Dodecanoic acid (Lauric acid)');
const b3SyntheticFlavorRow381 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0381');
assert.equal(b3SyntheticFlavorRow381.flavorNameEn, 'Decyl acetate');
const b3SyntheticFlavorRow413 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0413');
assert.equal(b3SyntheticFlavorRow413.flavorNameEn, 'Methyl 2-methylbutyrate');
const b3SyntheticFlavorRow272 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0272');
assert.equal(b3SyntheticFlavorRow272.flavorNameEn, '3-(Methylthio)butanal');
const b3SyntheticFlavorRow516 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0516');
assert.equal(b3SyntheticFlavorRow516.flavorNameEn, '2-Methylbutyl acetate');
const b3SyntheticFlavorRow664 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0664');
assert.equal(b3SyntheticFlavorRow664.flavorNameEn, '2-Formyl-6,6-dimethylbicyclo[3.1.1]-hept-2-ene (Myrtenal)');
const b3SyntheticFlavorRow795 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0795');
assert.equal(b3SyntheticFlavorRow795.flavorNameEn, 'Furfuryl alcohol');
const b3SyntheticFlavorRow850 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0850');
assert.equal(b3SyntheticFlavorRow850.flavorNameEn, 'Pyroligneous acid extract');
const b3SyntheticFlavorRow880 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0880');
assert.equal(b3SyntheticFlavorRow880.flavorNameEn, 'Linoleic acid');
const b3SyntheticFlavorRow912 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0912');
assert.equal(b3SyntheticFlavorRow912.flavorNameEn, 'Methyl(methylthio)acetate');
const b3SyntheticFlavorRow983 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S0983');
assert.equal(b3SyntheticFlavorRow983.flavorNameEn, 'Thiamine hydrochloride');
const b3SyntheticFlavorRow1229 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S1229');
assert.equal(b3SyntheticFlavorRow1229.flavorNameEn, 'Artificial cognac oil');
const b3SyntheticFlavorRow1370 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S1370');
assert.equal(b3SyntheticFlavorRow1370.flavorNameEn, 'N-(heptan-4-yl)benzo[d][1,3]dioxole-5-carboxamide');
const b3SyntheticFlavorRow1355 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S1355');
assert.equal(b3SyntheticFlavorRow1355.flavorNameEn, 'Geranyl 2-ethylbutyrate');
const b3SyntheticFlavorRow1404 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S1404');
assert.equal(b3SyntheticFlavorRow1404.flavorNameEn, 'Allyl 2-ethylbutyrate');
const b3SyntheticFlavorRow1471 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S1471');
assert.equal(b3SyntheticFlavorRow1471.flavorNameEn, '4-Amino-5,6-dimethylthieno[2,3-d]pyrimidin-2(1H)-one hydrochloride');
const b3SyntheticFlavorRow1481 = gb2760OfficialB3SyntheticFlavorRows.find((row) => row.flavorCode === 'S1481');
assert.equal(b3SyntheticFlavorRow1481.flavorNameEn, 'Vanillyl alcohol');
...
assert.equal(c1ProcessingAidRow17.processingAidNameEn, 'potassium chloride');
...
const c2ProcessingAidRow2 = gb2760OfficialC2ProcessingAidRows.find((row) => row.rowNumber === 2);
assert.equal(c2ProcessingAidRow2.processingAidNameEn, 'vegetable oils extraction solvent');
const c2ProcessingAidRow9 = gb2760OfficialC2ProcessingAidRows.find((row) => row.rowNumber === 9);
assert.equal(c2ProcessingAidRow9.processingAidNameEn, '1,2-propanediol');
const c2ProcessingAidRow10 = gb2760OfficialC2ProcessingAidRows.find((row) => row.rowNumber === 10);
assert.equal(c2ProcessingAidRow10.processingAidNameEn, 'carnauba wax');
...
assert.equal(c2ProcessingAidRow12.processingAidNameEn, 'insoluble polyvinylpolypyrrolidone (PVPP)');
assert.match(c2ProcessingAidRow12.useScope, /^啤酒、葡萄酒、果酒、黄酒、配制酒/u);
const c2ProcessingAidRow13 = gb2760OfficialC2ProcessingAidRows.find((row) => row.rowNumber === 13);
assert.equal(c2ProcessingAidRow13.processingAidNameEn, 'Propionic acid, sodium propionate, calcium propionate');
const c2ProcessingAidRow19 = gb2760OfficialC2ProcessingAidRows.find((row) => row.rowNumber === 19);
assert.equal(c2ProcessingAidRow19.processingAidNameEn, 'silica gel');
const c2ProcessingAidRow33 = gb2760OfficialC2ProcessingAidRows.find((row) => row.rowNumber === 33);
assert.equal(c2ProcessingAidRow33.processingAidNameEn, 'Polyoxypropylene oxyethylene glycerol ether (GPE)');
...
assert.equal(c2ProcessingAidRow34.processingAidNameEn, 'polyoxyethylene (20) sorbitan monolaurate, polyoxyethylene (20) sorbitan monopalmitate, polyoxyethylene (20) sorbitan monostearate, polyoxyethylene (20) sorbitan monooleate');
assert.match(c2ProcessingAidRow34.useScope, /^制糖工艺、发酵工艺、提取工艺/u);
const c2ProcessingAidRow35 = gb2760OfficialC2ProcessingAidRows.find((row) => row.rowNumber === 35);
assert.equal(c2ProcessingAidRow35.processingAidNameEn, 'polyoxyethylene polyoxypropylene amine ether (BAPE)');
const c2ProcessingAidRow36 = gb2760OfficialC2ProcessingAidRows.find((row) => row.rowNumber === 36);
assert.equal(c2ProcessingAidRow36.processingAidNameEn, 'polyoxyethylene polyoxypropylene pentaerythritol ether (PPE)');
...
assert.equal(c2ProcessingAidRow43.processingAidNameEn, 'sodium dihydrogen phosphate');
assert.doesNotMatch(c2ProcessingAidRow43.rawSourceText, /包括磷酸|磷酸湿法/u);
const c2ProcessingAidRow63 = gb2760OfficialC2ProcessingAidRows.find((row) => row.rowNumber === 63);
assert.equal(c2ProcessingAidRow63.processingAidNameEn, 'vitamin B family');
...
assert.equal(gb2760OfficialC2ProcessingAidRows.at(-1).processingAidNameEn, 'Vegetable activated carbon (Source of rice husk)');
assert.equal(gb2760OfficialC2ProcessingAidRows.at(-1).useScope, '油脂加工工艺');
const c3EnzymeRow1 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 1);
assert.equal(c3EnzymeRow1.enzymeName, 'D-阿洛酮糖-3-差向异构酶 D-psicose 3-epimerase');
assert.match(c3EnzymeRow1.donor, /CAG55 Ruminococcus sp\. CAG55/u);
assert.doesNotMatch(c3EnzymeRow1.donor, /CAG 55/u);
...
assert.equal(c3EnzymeRow2.source, '黑曲霉 Aspergillus niger');
assert.equal(c3EnzymeRow2.donor, '');
assert.doesNotMatch(c3EnzymeRow2.rawSourceText, /Geobacillusstearothermophilus|Cytophagasp/u);
...
assert.equal(c3EnzymeRow3.enzymeName, 'α-淀粉酶 Alpha-amylase');
assert.match(c3EnzymeRow3.source, /^地衣芽孢杆菌 Bacillus licheniformis/u);
assert.match(c3EnzymeRow3.source, /hog or bovine pancreas/u);
assert.doesNotMatch(c3EnzymeRow3.source, /hogorbovinepancreas/u);
assert.doesNotMatch(c3EnzymeRow3.source, /^misstearothermoph/u);
assert.match(c3EnzymeRow3.donor, /Geobacillus stearothermophilus/u);
assert.match(c3EnzymeRow3.donor, /Cytophaga sp\./u);
assert.match(c3EnzymeRow3.donor, /Rhizomucor pusillus/u);
assert.match(c3EnzymeRow3.donor, /Aspergillus kawachii/u);
const c3EnzymeRow5 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 5);
assert.match(c3EnzymeRow5.source, /^大麦、山芋、大豆、小麦和麦芽/u);
assert.doesNotMatch(c3EnzymeRow5.source, /^barley/u);
const c3EnzymeRow6 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 6);
assert.equal(c3EnzymeRow6.source, 'Aspergillus fijiensis');
assert.doesNotMatch(c3EnzymeRow6.source, /Bacillus flexus/u);
const c3EnzymeRow11 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 11);
assert.equal(c3EnzymeRow11.source, '菠萝 Ananas spp.');
assert.equal(c3EnzymeRow11.donor, '');
assert.doesNotMatch(c3EnzymeRow11.rawSourceText, /Cryphonectria|Bacillusamyloliq/u);
const c3EnzymeRow12 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 12);
assert.equal(c3EnzymeRow12.enzymeName, '蛋白酶(包括乳凝块酶) Protease (including milk clotting enzymes)');
assert.match(c3EnzymeRow12.source, /^寄生内座壳\(栗疫菌\) Cryphonectria parasitica/u);
assert.match(c3EnzymeRow12.source, /Endothia parasitica/u);
assert.match(c3EnzymeRow12.source, /Bacillus licheniformis/u);
assert.match(c3EnzymeRow12.source, /Anoxybacillus caldiproteolyticus/u);
assert.match(c3EnzymeRow12.donor, /^寄生内座壳\(栗疫菌\) Cryphonectria parasitica/u);
assert.match(c3EnzymeRow12.donor, /Calf stomach/u);
assert.match(c3EnzymeRow12.donor, /Alkalihalobacillus clausii/u);
assert.match(c3EnzymeRow12.donor, /Malbranchea sulfurea/u);
assert.doesNotMatch(
  `${c3EnzymeRow12.enzymeName} ${c3EnzymeRow12.source} ${c3EnzymeRow12.donor}`,
  /includingmilkclottingenzymes|Endothiaparasitica|Calfstomach|Anoxybacilluscaldiproteolyticus|Alkalihaloba-cillusclausii|Malbrancheasulfurea|lichenifor-mis/u
);
const c3EnzymeRow22 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 22);
assert.match(c3EnzymeRow22.source, /bovine, pig or horse liver/u);
assert.doesNotMatch(c3EnzymeRow22.source, /pigor|horseliver/u);
assert.match(c3EnzymeRow22.source, /Micrococcus lysode-icticus$/u);
assert.doesNotMatch(c3EnzymeRow22.source, /lysode-$/u);
const c3EnzymeRow23 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 23);
assert.equal(c3EnzymeRow23.source, '橘青霉 Penicillium citrinum');
assert.doesNotMatch(c3EnzymeRow23.source, /^icticus/u);
const c3EnzymeRow31 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 31);
assert.equal(c3EnzymeRow31.enzymeName, '磷脂酶 A2 Phospholipase A2');
assert.match(c3EnzymeRow31.source, /porcine pancreas/u);
assert.match(c3EnzymeRow31.donor, /porcine pancreas/u);
const c3EnzymeRow33 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 33);
assert.equal(c3EnzymeRow33.enzymeName, '磷酸肌醇磷脂酶 C Phosphoinositide phospholipase C');
assert.match(c3EnzymeRow33.source, /Pseudomonas fluorescens/u);
assert.match(c3EnzymeRow33.donor, /Pseudomonas sp\./u);
assert.doesNotMatch(
  `${c3EnzymeRow33.enzymeName} ${c3EnzymeRow33.source} ${c3EnzymeRow33.donor}`,
  /Phosphoinositidephospholipase|Pseudomonasfluorescens|Pseudomonassp/u
);
const c3EnzymeRow35 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 35);
assert.match(c3EnzymeRow35.donor, /Geobacillus stearothermophilus/u);
assert.doesNotMatch(c3EnzymeRow35.donor, /stearothe\s+ilus\s+rmoph|stearotheilusrmoph/u);
const c3EnzymeRow37 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 37);
assert.equal(c3EnzymeRow37.source, '木瓜 Carica papaya');
assert.equal(c3EnzymeRow37.donor, '');
const c3EnzymeRow38 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 38);
assert.match(c3EnzymeRow38.source, /^Fusarium venenatum/u);
assert.match(c3EnzymeRow38.source, /地衣芽孢杆菌 Bacillus licheniformis$/u);
assert.match(c3EnzymeRow38.donor, /Aspergillus niger var\. tubingensis/u);
assert.match(c3EnzymeRow38.donor, /Thermopolyspora flexuosa/u);
assert.doesNotMatch(c3EnzymeRow38.donor, /vart\.ubingensis/u);
const c3EnzymeRow39 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 39);
assert.equal(c3EnzymeRow39.source, '大肠杆菌 K-12 Escherichia Coli K-12');
assert.equal(c3EnzymeRow39.donor, '小牛前凝乳酶 A 基因 calfprochy-mosin A gene');
assert.doesNotMatch(c3EnzymeRow39.donor, /calfprochy-$/u);
const c3EnzymeRow40 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 40);
assert.equal(c3EnzymeRow40.source, '黑曲霉泡盛变种 Aspergillus niger var. awamori 乳克鲁维酵母 Kluyveromyces lac-tis');
assert.match(c3EnzymeRow40.donor, /^小牛前凝乳酶 B 基因 calfprochy-mosin B gene/u);
assert.doesNotMatch(c3EnzymeRow40.donor, /^mosinBgene/u);
const c3EnzymeRow41 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 41);
assert.equal(c3EnzymeRow41.source, '小牛、山羊或羔羊的皱胃 calf, kid, or lamb abomasum');
assert.doesNotMatch(c3EnzymeRow41.source, /orlamb|lambabomasum/u);
const c3EnzymeRow43 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 43);
assert.doesNotMatch(c3EnzymeRow43.source, /橄榄产色链霉菌/u);
const c3EnzymeRow44 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 44);
assert.match(c3EnzymeRow44.source, /^橄榄产色链霉菌 Streptomyces olivochromogenes/u);
assert.match(c3EnzymeRow44.source, /鼠灰链霉菌 Streptomyces murinus/u);
assert.equal(c3EnzymeRow44.donor, '锈棕色链霉菌 Streptomyces rubig-inosus');
const c3EnzymeRow45 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 45);
assert.equal(c3EnzymeRow45.source, '黑曲霉 Aspergillus niger');
assert.equal(c3EnzymeRow45.donor, '');
const c3EnzymeRow46 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 46);
assert.equal(c3EnzymeRow46.source, '李氏木霉 Trichoderma reesei');
assert.equal(c3EnzymeRow46.donor, '黑曲霉 Aspergillus niger');
const c3EnzymeRow47 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 47);
assert.match(c3EnzymeRow47.source, /^产气克雷伯氏菌 Klebsiella aerogenes/u);
assert.match(c3EnzymeRow47.source, /嗜酸普鲁兰芽孢杆菌 Bacillus acidopullulyticus/u);
assert.match(c3EnzymeRow47.source, /长野解普鲁兰杆菌 Pullulanibacillus naganoensis/u);
assert.doesNotMatch(c3EnzymeRow47.source, /Klebsiellaaero|aci-dopullulyticus|Pullulanibacil-lus/u);
const c3EnzymeRow48 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 48);
assert.equal(c3EnzymeRow48.source, '米曲霉 Aspergillus oryzae');
assert.doesNotMatch(c3EnzymeRow48.source, /^lusnaganoensis/u);
const c3EnzymeRow49 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 49);
assert.equal(c3EnzymeRow49.source, '黑曲霉 Aspergillus niger 黑曲霉 Aspergillus niger 李氏木霉 Trichoderma reesei');
assert.doesNotMatch(c3EnzymeRow49.source, /Kluyveromyceslac-$/u);
...
assert.match(c3EnzymeRow50.source, /^脆壁克鲁维酵母 Kluyveromyces fragilis/u);
assert.match(c3EnzymeRow50.source, /Bifidobacterium bifidum/u);
assert.doesNotMatch(c3EnzymeRow50.source, /^tis/u);
const c3EnzymeRow51 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 51);
assert.equal(c3EnzymeRow51.source, '黑曲霉 Aspergillus niger 米曲霉 Aspergillus oryzae');
assert.equal(c3EnzymeRow51.donor, '黑曲霉 Aspergillus niger 米曲霉 Aspergillus oryzae');
const c3EnzymeRow55 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 55);
assert.equal(c3EnzymeRow55.source, '猪、小牛、小羊、禽类的胃组织 hog, calf, lamb (kid) or poultry stomach');
assert.doesNotMatch(c3EnzymeRow55.source, /orpoultrystomach/u);
const c3EnzymeRow59 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 59);
assert.equal(c3EnzymeRow59.source, '猪或牛的胰腺 porcine or bovine pancreas');
const c3EnzymeRow60 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 60);
assert.equal(c3EnzymeRow60.source, '猪或牛的胰腺 porcine or bovine pancreas');
const c3EnzymeRow61 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 61);
assert.equal(c3EnzymeRow61.source, '米曲霉 Aspergillus oryzae');
assert.equal(c3EnzymeRow61.donor, '');
const c3EnzymeRow62 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 62);
assert.equal(c3EnzymeRow62.enzymeName, '脂肪酶 Lipase');
assert.match(c3EnzymeRow62.source, /^黑曲霉 Aspergillus niger 黑曲霉 Aspergillus niger/u);
assert.match(c3EnzymeRow62.source, /salivary glands or forestomach of calf, kid, or lamb/u);
assert.match(c3EnzymeRow62.source, /goat gullets/u);
assert.match(c3EnzymeRow62.source, /hog or bovine pancreas/u);
assert.doesNotMatch(c3EnzymeRow62.source, /salivaryglandsorforestomach|goatgullets|hogorbovinepancreas/u);
assert.match(c3EnzymeRow62.source, /李氏木霉 Trichoderma reesei 李氏木霉 Trichoderma reesei$/u);
assert.match(c3EnzymeRow62.donor, /Candida antarctica/u);
assert.match(c3EnzymeRow62.donor, /Aspergillus niger var\. tubingensis/u);
assert.doesNotMatch(c3EnzymeRow62.donor, /vart\.ubingensis/u);
const c3EnzymeRow63 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 63);
assert.equal(c3EnzymeRow63.source, '黑曲霉 Aspergillus niger 李氏木霉 Trichoderma reesei 米黑根毛霉 Rhizomucor miehei');
assert.equal(c3EnzymeRow63.donor, '');
assert.doesNotMatch(c3EnzymeRow63.donor, /^vart\.ubingensis/u);
const c3EnzymeRow64 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 64);
assert.equal(c3EnzymeRow64.source, '黑曲霉 Aspergillus niger 黑曲霉 Aspergillus niger');
assert.equal(c3EnzymeRow64.donor, '塔宾曲霉 Aspergillus tubingensis');
assert.equal(gb2760OfficialC3EnzymePreparationRows.at(-1).enzymeName, '转葡糖苷酶 Transglucosidase');
...
const appendixFAdditiveIndexRow229 = gb2760OfficialFAdditiveIndexRows.find((row) => row.rowNumber === 229);
assert.equal(appendixFAdditiveIndexRow229.additiveNameCn, '甜蜜素(又名环己基氨基磺酸钠),环己基氨基磺酸钙');
assert.equal(appendixFAdditiveIndexRow229.insNumber, '952(iv),952(ii)');
assert.equal(appendixFAdditiveIndexRow229.a1PageNumber, 116);
const appendixFAdditiveIndexRow230 = gb2760OfficialFAdditiveIndexRows.find((row) => row.rowNumber === 230);
assert.match(appendixFAdditiveIndexRow230.additiveNameCn, /^吐温类\[聚氧乙烯\(20\)山梨醇酐单月桂酸酯/u);
assert.equal(appendixFAdditiveIndexRow230.insNumber, '432,434,435,433');
assert.equal(appendixFAdditiveIndexRow230.a1PageNumber, 117);
const appendixFAdditiveIndexRow231 = gb2760OfficialFAdditiveIndexRows.find((row) => row.rowNumber === 231);
assert.equal(appendixFAdditiveIndexRow231.additiveNameCn, '脱氢乙酸及其钠盐(包括脱氢乙酸,脱氢乙酸钠)');
assert.equal(appendixFAdditiveIndexRow231.insNumber, '265,266');
assert.equal(appendixFAdditiveIndexRow231.a1PageNumber, 118);
assert.equal(gb2760OfficialFAdditiveIndexRows.some((row) => row.insNumber.includes('952(ii)432') || row.insNumber.includes('433265')), false);
...
assert.match(validateGb2760OfficialReferenceTables(gb2760OfficialReferenceRows.map((row) => (
  row.id === 'gb2760-2024-c3-enzyme-preparation-039'
    ? { ...row, rowData: { ...row.rowData, donor: '小牛前凝乳酶B基因calfprochy-' } }
    : row
))).join('\n'), /orphan C\.3 continuation fragment/);
assert.match(validateGb2760OfficialReferenceTables(gb2760OfficialReferenceRows.map((row) => (
  row.id === 'gb2760-2024-c3-enzyme-preparation-038'
    ? { ...row, rowData: { ...row.rowData, donor: `${row.rowData.donor}Aspergillusnigervart.ubingensis` } }
    : row
))).join('\n'), /orphan C\.3 continuation fragment/);
...
assert.equal(gb2760ReferenceSummary.fAdditiveIndexCount, 287);
...
assert.equal(gb2760ReferenceQualityReport.totalRows, 2800);
...
assert.equal(gb2760ReferenceQualityReport.fAdditiveIndexCount, 287);
...
const lowConfidenceReport = saveAnalysisReport('食用柠檬酸', 'food', { productName: '低置信样本' });
const lowConfidenceIndex = lowConfidenceReport.matchResults.findIndex((item) => item.confidence >= 0.55 && item.confidence < 0.9);
assert.notEqual(lowConfidenceIndex, -1);
assert.equal(lowConfidenceReport.pendingCount, 1);
assert.equal(lowConfidenceReport.matchedCount, 0);
assert.equal(lowConfidenceReport.matchRate, 0);
assert.deepEqual(lowConfidenceReport.unknownItems, []);
const highRiskCandidateReport = createAnalysisReport('亚硝酸', 'food', { productName: '高风险候选样本' });
assert.equal(highRiskCandidateReport.pendingCount, 1);
assert.equal(highRiskCandidateReport.matchedCount, 0);
assert.equal(highRiskCandidateReport.matchRate, 0);
assert.equal(highRiskCandidateReport.riskGrade, 'A');
assert.equal(highRiskCandidateReport.riskSummary.highRisk, 0);
assert.equal(highRiskCandidateReport.riskSummary.unmatched, 0);
assert.equal(highRiskCandidateReport.riskSummary.unverifiedData, 1);
assert.deepEqual(highRiskCandidateReport.highlightIngredientIds, []);
assert.match(highRiskCandidateReport.summary, /1 项待确认/);
const lowConfidenceReportHtml = renderRoute(resolveRoute(`#/food/reports/${lowConfidenceReport.id}`));
assert.match(lowConfidenceReportHtml, /待确认匹配/);
assert.match(lowConfidenceReportHtml, /低置信候选/);
assert.match(lowConfidenceReportHtml, /⚠️ 请确认/);
assert.match(lowConfidenceReportHtml, /data-report-match-decision="confirmed"/);
assert.match(lowConfidenceReportHtml, /data-report-match-decision="rejected"/);
assert.match(lowConfidenceReportHtml, /已匹配 0 种/);
const confirmedLowConfidenceReport = updateAnalysisReportMatchDecision(lowConfidenceReport.id, lowConfidenceIndex, 'confirmed');
assert.equal(confirmedLowConfidenceReport.matchResults[lowConfidenceIndex].reviewDecision, 'confirmed');
assert.equal(confirmedLowConfidenceReport.pendingCount, 0);
assert.equal(confirmedLowConfidenceReport.matchedCount, 1);
assert.equal(confirmedLowConfidenceReport.matchRate, 1);
assert.deepEqual(confirmedLowConfidenceReport.unknownItems, []);
const confirmedLowConfidenceHtml = renderRoute(resolveRoute(`#/food/reports/${lowConfidenceReport.id}`));
assert.match(confirmedLowConfidenceHtml, /已确认匹配/);
assert.match(confirmedLowConfidenceHtml, /已匹配 1 种/);
assert.doesNotMatch(confirmedLowConfidenceHtml, /低置信候选/);
const rejectedLowConfidenceReport = updateAnalysisReportMatchDecision(lowConfidenceReport.id, lowConfidenceIndex, 'rejected');
assert.equal(rejectedLowConfidenceReport.matchResults[lowConfidenceIndex].reviewDecision, 'rejected');
assert.equal(rejectedLowConfidenceReport.pendingCount, 0);
assert.equal(rejectedLowConfidenceReport.matchedCount, 0);
assert.equal(rejectedLowConfidenceReport.matchRate, 0);
assert.deepEqual(rejectedLowConfidenceReport.unknownItems, ['食用柠檬酸']);
assert.equal(rejectedLowConfidenceReport.unknownItemRecords[0].reason, 'low_confidence_rejected');
const rejectedLowConfidenceHtml = renderRoute(resolveRoute(`#/food/reports/${lowConfidenceReport.id}`));
assert.match(rejectedLowConfidenceHtml, /1 种需人工核对/);
assert.match(rejectedLowConfidenceHtml, /已驳回候选：柠檬酸/);
assert.doesNotMatch(rejectedLowConfidenceHtml, /data-report-match-decision=/);
const rejectedQueueHtml = renderRoute(resolveRoute('#/food/data'));
assert.match(rejectedQueueHtml, /驳回候选/);
assert.match(rejectedQueueHtml, /驳回未收录/);
assert.match(rejectedQueueHtml, /低置信样本/);
assert.match(rejectedQueueHtml, /食用柠檬酸/);
