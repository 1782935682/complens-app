...
assert.deepEqual(backendTestTsConfig.include, ['src/**/*.ts', 'tests/**/*.ts', 'vitest.config.ts']);
const backendEnvExample = await readFile(new URL('../backend/.env.example', import.meta.url), 'utf8');
assert.match(backendEnvExample, /DATABASE_URL=postgres:\/\/postgres:password@localhost:5432\/compcheck/);
