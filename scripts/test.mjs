...
assert.equal(c3EnzymeRow2.source, '黑曲霉Aspergillusniger');
...
assert.match(c3EnzymeRow3.source, /^地衣芽孢杆菌Bacilluslichenifor-mis/u);
assert.doesNotMatch(c3EnzymeRow3.source, /^misstearothermoph/u);
assert.match(c3EnzymeRow3.donor, /Geobacillusstearothermophilus/u);
...
assert.equal(c3EnzymeRow6.source, 'Aspergillusfijiensis');
assert.doesNotMatch(c3EnzymeRow6.source, /Bacillusflexus/u);
const c3EnzymeRow11 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 11);
assert.equal(c3EnzymeRow11.source, '菠萝Ananasspp.');
...
assert.match(c3EnzymeRow12.source, /^寄生内座壳\(栗疫菌\)Cryphonectriaparasitica/u);
assert.match(c3EnzymeRow12.donor, /^寄生内座壳\(栗疫菌\)Cryphonectriaparasitica/u);
const c3EnzymeRow22 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 22);
assert.match(c3EnzymeRow22.source, /Micrococcuslysode-icticus$/u);
...
assert.equal(c3EnzymeRow37.source, '木瓜Caricapapaya');
assert.equal(c3EnzymeRow37.donor, '');
const c3EnzymeRow38 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 38);
assert.match(c3EnzymeRow38.source, /^Fusariumvenenatum/u);
assert.match(c3EnzymeRow38.source, /地衣芽孢杆菌Bacilluslichenifor-mis$/u);
assert.match(c3EnzymeRow38.donor, /Aspergillusnigervar\.tubingensis/u);
assert.match(c3EnzymeRow38.donor, /Thermopolysporaflexuosa/u);
...
assert.equal(c3EnzymeRow39.source, '大肠杆菌K-12EscherichiaColiK-12');
assert.equal(c3EnzymeRow39.donor, '小牛前凝乳酶A基因calfprochy-mosinAgene');
...
assert.equal(c3EnzymeRow40.source, '黑曲霉泡盛变种Aspergillusnigervar.awamori乳克鲁维酵母Kluyveromyceslac-tis');
assert.match(c3EnzymeRow40.donor, /^小牛前凝乳酶B基因calfprochy-mosinBgene/u);
...
assert.match(c3EnzymeRow44.source, /^橄榄产色链霉菌Streptomycesolivochromogenes/u);
assert.match(c3EnzymeRow44.source, /鼠灰链霉菌Streptomycesmurinus/u);
assert.equal(c3EnzymeRow44.donor, '锈棕色链霉菌Streptomycesrubig-inosus');
const c3EnzymeRow45 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 45);
assert.equal(c3EnzymeRow45.source, '黑曲霉Aspergillusniger');
assert.equal(c3EnzymeRow45.donor, '');
const c3EnzymeRow46 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 46);
assert.equal(c3EnzymeRow46.source, '李氏木霉Trichodermareesei');
assert.equal(c3EnzymeRow46.donor, '黑曲霉Aspergillusniger');
const c3EnzymeRow47 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 47);
assert.match(c3EnzymeRow47.source, /^产气克雷伯氏菌Klebsiellaaero-genes/u);
assert.match(c3EnzymeRow47.source, /长野解普鲁兰杆菌Pullulanibacil-lusnaganoensis/u);
...
assert.equal(c3EnzymeRow49.source, '黑曲霉Aspergillusniger黑曲霉Aspergillusniger李氏木霉Trichodermareesei');
...
assert.match(c3EnzymeRow50.source, /^脆壁克鲁维酵母Kluyveromycesfragilis/u);
...
assert.equal(c3EnzymeRow51.source, '黑曲霉Aspergillusniger米曲霉Aspergillusoryzae');
assert.equal(c3EnzymeRow51.donor, '黑曲霉Aspergillusniger米曲霉Aspergillusoryzae');
const c3EnzymeRow61 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 61);
assert.equal(c3EnzymeRow61.source, '米曲霉Aspergillusoryzae');
assert.equal(c3EnzymeRow61.donor, '');
const c3EnzymeRow62 = gb2760OfficialC3EnzymePreparationRows.find((row) => row.rowNumber === 62);
assert.match(c3EnzymeRow62.source, /^黑曲霉Aspergillusniger黑曲霉Aspergillusniger/u);
assert.match(c3EnzymeRow62.source, /李氏木霉Trichodermareesei李氏木霉Trichodermareesei$/u);
assert.match(c3EnzymeRow62.donor, /Candidaantarctica/u);
assert.match(c3EnzymeRow62.donor, /Aspergillusnigervar\.tubingensis/u);
...
assert.equal(c3EnzymeRow63.source, '黑曲霉Aspergillusniger李氏木霉Trichodermareesei米黑根毛霉Rhizomucormiehei');
...
assert.equal(c3EnzymeRow64.source, '黑曲霉Aspergillusniger黑曲霉Aspergillusniger');
assert.equal(c3EnzymeRow64.donor, '塔宾曲霉Aspergillustubingensis');
