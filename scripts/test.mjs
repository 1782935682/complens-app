...
assert.equal(foodAuditSummary.dataStatusCounts.verified_regulation, 0);
assert.equal(foodAuditSummary.dataStatusCounts.verified_jecfa, 32);
...
assert.equal(foodSourceSummaries.length, 37);
assert.equal(foodSourceSummaries[0].recordCount, 68);
...
assert.match(dataPageHtml, /37 个来源/);
...
assert.match(filteredReviewedDataPageHtml, /当前筛选 32 \/ 112 条记录/);
...
assert.equal(reportDraft.dataStatusCounts.verified_jecfa, 1);
...
assert.match(reportDetailHtml, /JECFA 安全评价/);
...
assert.match(reportMarkdown, /数据状态：JECFA 安全评价/);
