...
assert.equal(foodAuditSummary.draftCount, 68);
assert.equal(foodAuditSummary.reviewedCount, 32);
assert.equal(foodAuditSummary.reviewedOrVerifiedCount, 32);
assert.equal(foodAuditSummary.confidenceCounts.medium, 32);
...
assert.deepEqual(foodVersionSummaries, [{ version: 'food-additives-official-review-v2', count: 100, latestUpdatedAt: '2026-06-12' }]);
...
assert.match(dataPageHtml, /68%[\s\S]*待审核/);
...
assert.match(dataPageHtml, /food-additives-official-review-v2/);
...
assert.match(filteredDataPageHtml, /当前筛选 68 \/ 100 条记录/);
...
assert.match(filteredReviewedDataPageHtml, /当前筛选 32 \/ 100 条记录/);
