...
assert.equal(reviewedCitricAcid.reviewStatus, 'reviewed');
assert.equal(reviewedCitricAcid.confidenceLevel, 'medium');
assert.equal(reviewedCitricAcid.isVerified, false);
assert.equal(reviewedCitricAcid.sourceName, 'WHO JECFA Food Additives and Contaminants Database');
assert.match(reviewedCitricAcid.sourceUrl, /Home\/Chemical\/3594$/);
...
assert.equal(foodAuditSummary.draftCount, 90);
assert.equal(foodAuditSummary.reviewedCount, 10);
assert.equal(foodAuditSummary.reviewedOrVerifiedCount, 10);
assert.equal(foodAuditSummary.confidenceCounts.medium, 10);
assert.equal(foodAuditSummary.confidenceCounts.unverified, 90);
...
assert.deepEqual(foodVersionSummaries, [{ version: 'food-additives-official-review-v1', count: 100, latestUpdatedAt: '2026-06-12' }]);
...
assert.match(detailHtmlWithRelatedIngredients, /中可信/);
...
assert.match(homeHtmlWithCategoryFilters, /100 条[\s\S]*当前记录/);
assert.match(homeHtmlWithCategoryFilters, /10 条[\s\S]*已复核\/验证/);
...
assert.match(dataPageHtml, /90%[\s\S]*待审核/);
...
assert.match(dataPageHtml, /food-additives-official-review-v1/);
...
assert.match(filteredDataPageHtml, /当前筛选 90 \/ 100 条记录/);
...
assert.match(filteredReviewedDataPageHtml, /当前筛选 10 \/ 100 条记录/);
...
assert.match(searchHtmlWithSuggestions, /100 条记录/);
...
assert.equal(nestedENumberFoodAnalysis.matchedCount, 2);
assert.deepEqual(nestedENumberFoodAnalysis.ingredients.map((item) => item.id), ['potassium-sorbate', 'sodium-benzoate']);
assert.deepEqual(nestedENumberFoodAnalysis.unknownItems, ['水']);
