...
后续支持商品条码和数字标签二维码时，用户端仍保持一个拍照入口，由后端自动识别图片对象并分流：

- 商品条码：识别 GTIN 后查询商品及配料数据。
...
条码商品库和数字标签页面都不是官方成分知识库，也不能直接生成 `verified_regulation`、GB 2760 使用范围、限量、ADI 或安全结论。若当前分支无法获得完整配料，只提示用户重新拍摄包装其他区域或手动补充，不自动跳转其他分支，不伪造缺失字段。
