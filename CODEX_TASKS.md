...
→ 本轮刚完成：
  ...
  17. 食品成分官方材料管线加固：`buildPipelineSnapshot` 将抽取失败和解析失败分开记录为 `failed_extract` / `failed_parse`，`pdftotext` 缺失或损坏 PDF 失败时输出可执行原因；新增损坏 PDF 回归测试，确认单个来源失败不会中断其他官方材料 staging 抽取。
