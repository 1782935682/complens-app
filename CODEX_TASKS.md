...
```
首页单一拍包装入口（成分搜索 / 当前关注目标 / 最近扫描为辅助）
  → 拍照 / 上传食品包装图片（包装正面 / 配料表 / 营养成分表 / 生产日期）
  → [客户端] 图片预处理（EXIF 修正 + 压缩 + IndexedDB 或平台文件缓存）
  → OCR 识别 / OCR 文本清洗 / 有效标签提取 / 商品身份识别 / 手动输入
  → 拍照识别结束直接生成结果；失败或信息不足时生成信息不足报告并在报告内引导补拍 / 手动分区补充
  → 本地拆分配料 + 添加剂识别 + 营养字段解析
...
**添加剂识别是核心能力，配料解释是理解过程，营养分析服务于购买/食用判断，食品标签解读报告是最终输出。P0 不做条码识别、商品库优先匹配、商品对比或日化规则；包装正面只提取商品名、食品类型和明确包装声明。成分搜索和商品对比都不是 MVP 主路径。**
...
约束：不要新增三段式首页入口，也不要把条码、二维码、OCR 设计成固定串行流程；后端识别到哪类对象就走对应取数分支。当当前识别分支只获得商品名、品牌、商品码、二维码等身份线索时，可通过 AI 联网搜索公开标签信息；搜到配料/营养则生成带 AI 来源标注的参考报告，搜不到则生成信息不足报告并提示补拍包装其他区域或手动补充，不自动代用户发起其他识别分支。

2026-06-21 门禁状态：食品包装非条码 / 二维码 OCR 500 样本已达 `stable_500`，`494/500`、`98.8%`、Codex 子 agent 与 DeepSeek Review 均通过；条码 / 二维码 / 商品数字码解析级各 500 用例已通过（总计 `1500/1500`，准确率 `100%`）。真实图片级条码 / 二维码扫码解码仍需浏览器 BarcodeDetector、小程序扫码能力或真机样本验证，不得写成已完成。
...
→ 本轮刚完成：
  ...
  12. user-uniapp P0 可用性自测：ARM 环境安装系统 Chromium / chromedriver，Playwright H5 截图覆盖首页、拍食品标签、轻确认、消费建议、我的关注和扫描记录；截图后修正默认日常目标结论过重问题，并继续压缩结果页重点提醒、目标化结论文案和轻确认页空态/按钮；P0 主流程未发现商品正面、条码、商品库、商品对比或日化入口。
  ...
  22. user-uniapp P0 体验修复：首页主拍照按钮放大并减少重复文案，搜索/关注降为弱入口；拍照框视觉重做；搜索结果隐藏搜索后的热门模块并移除“常见用途/需要注意/常见食品”旧字段；我的页关注目标和过敏/忌口改为等宽选择控件；当前主路径恢复为清晰识别直接生成结果，低置信、失败、手动输入或信息不足时保留文本确认 / 补充页。
  23. user-uniapp 小程序产品化收口：拍照/OCR 后清晰可生成结果自动进入报告，低置信、失败、手动输入或信息不足时进入文本确认 / 补充；新增扫描记录页并接入我的关注页和报告页；报告页改为“食品标签解读”，第一屏展示糖、钠、脂肪、能量、添加剂和过敏原重点提醒，补添加剂大白话、收藏、分享入口和识别文字折叠区；新增设置与说明页，覆盖图片/OCR、本机记录、使用边界和本机数据清空。
  ...
  39. user-uniapp 产品主路径重排：首页去掉页面内品牌标题，主按钮改为“拍包装”并直接拉起相机；成分搜索降为首页小入口；OCR 成功后自动生成结果，失败或手动场景才进入输入页；报告页去掉独立分享摘要和大块反馈卡，改为“一句话结论 / 关键原因 / 营养重点图 / 适合谁与不适合谁 / 添加剂解释 / 建议吃法 / 折叠识别详情”；反馈收为小入口；历史页简化筛选和卡片操作；我的页只保留关注偏好，不再并列历史、搜索和设置说明入口。
  ...
  41. user-uniapp 上线前 UI/产品深度评审收口：报告首屏改为“一句话结论 + 你的关注短标签 + 为什么 / 谁少吃 / 怎么吃”决策卡，营养重点图加入低/中/高参考，信息不足报告前置补拍和手动粘贴动作，首页成分搜索进一步弱化为小文字入口，历史筛选改为“有提醒”；新增 `review:chatgpt` / `review:deepseek` 评审脚本，当前 ChatGPT 会话子评审和 DeepSeek 综合评审均为 `pass` 且 `shipBlockers=[]`。
...
| CONSUMER-FS-D | 报告页配料与添加剂条目追溯入口（成分详情跳转） | ✅ 2026-06-16 |
...
4. ✅ `aliyun` / `paddleocr` 属于生产/外部供应商；`aliyun` 已实现阿里云通用文字识别签名调用和结果映射，缺少 Key/Secret 时返回 `503 ocr_not_configured`；`paddleocr` 仍是占位 provider，已配置但适配未实现时返回 `501 ocr_provider_pending`。
...
状态：✅ 已完成 2026-06-20（本机 `rapidocr` 已接入；`aliyun` provider 已实现；生产真云端 smoke 待 Key）。
是否需要人工：否（provider 代码已接入）。生产 Aliyun 真云端验收见 Batch 3-E。
阻塞条件：无（本机 RapidOCR / Aliyun adapter / manual / mock 闭环不阻塞；生产 Aliyun OCR Key 仍属生产验收阻塞）。
验证命令：前端 OCR 协议定向断言 + `cd backend && npm run test -- tests/config.test.ts tests/ocr.test.ts` + `cd backend && npm run typecheck`。
...
状态：🔄 进行中（本机 RapidOCR 已接入；Aliyun OCR provider 已实现；生产真云端 smoke 待 Key）。
...
3. ✅ 生产 provider 可切换到 Aliyun OCR：后端 adapter 已实现，待生产 Key 后做真云端 smoke/验收。
4. 图片后端二次压缩到 1MB 内再发送（生产供应商适配时补）。
5. 真实包装图片测试（≥3 张，识别可用率达标）。

人工操作：
- [x] 本机 OCR 服务目录确认：`/home/downloads/tools/complens-ocr`。
- [ ] 生产 Aliyun OCR 申请 Key → `OCR_PROVIDER=aliyun` / `ALIYUN_ACCESS_KEY_ID` / `ALIYUN_ACCESS_KEY_SECRET`。
- [ ] 标注：`[人工完成 ✅ YYYY-MM-DD]`

是否需要人工：是。
阻塞条件：生产 Aliyun OCR API Key（`blocked_by_user`）。Aliyun adapter、本机 rapidocr/manual/mock 闭环不受阻。
...
2. 固定主路径：拍照/上传食品标签 → OCR / 商品身份识别 → 自动生成报告（足够信息或信息不足）→ 标签类型识别 → 配料/营养解析 → 我的关注项 → 食品标签解读报告 → 历史；文本补充只由用户主动手动输入或从报告触发。
...
2. OCR 失败、低置信或信息不足直接生成信息不足报告，不进入确认页；手动输入或报告内补充不得跳过文本补充。
...
实现内容：
1. 覆盖首页 → 拍照/上传 → OCR / 商品身份识别 → 自动报告（足够信息或信息不足）→ 拆分 → 匹配 → 报告 → 保存历史；手动输入 / 报告内补充路径单独覆盖。
...
2. OCR 后给出标签内容判断；低置信或未知时直接生成信息不足报告，不让用户选择专业类型。
3. 用户可从信息不足报告选择补拍配料表 / 营养成分表或手动补充。

验收标准：
1. 无法判断时不硬凑分析，必须进入信息不足报告并给出补拍 / 手动补充动作。
2. 标签类型判断不展示为权威结论。
3. 用户主动补充后进入对应文本补充路径。
...
### Batch CONSUMER-LABEL-G：单一拍照入口自动识别分流（商品条码 / 数字标签二维码 / 配料表 OCR）[Codex]

目标：保持用户端现有一个拍食品标签 / 包装的拍照入口，不新增三个外露入口；图片提交后由后端自动识别是商品条码、数字标签二维码，还是配料表 / 营养成分表，并按识别结果进入对应数据获取分支。

涉及文件：`user-uniapp/src/pages/capture/index.vue`、`user-uniapp/src/pages/report/index.vue`、`user-uniapp/src/services/recognition/*`、`user-uniapp/src/stores/scanStore.ts`、`user-uniapp/src/types/index.ts`、`backend/src/routes/products.ts`、`backend/src/services/productLookupService.ts`、`backend/src/app.ts`、`backend/tests/productLookup.test.ts`、`docs/product-blueprint/API_CONTRACT.md`、`DATA_SOURCES.md`、`AI_REVIEW.md`。

实现内容：
1. 保持首页和拍照页现有主入口，不增加 `扫商品条码`、`扫数字标签二维码`、`拍配料表 OCR` 三个外露按钮。
2. `imageRecognitionService` 统一编排：先尝试条形码/二维码能力检测，再走 OCR，再按配料表、营养成分表、数字编码、未知图片分类。
3. `barcodeService` 负责 EAN-13 / EAN-8 / UPC-A / Code128 / QR 等内容标准化；H5 支持 `BarcodeDetector` 时可从图片解码，其他平台先退化为 OCR 数字编码识别，后续可替换为后端/云端解码 provider。
4. 商品条码 / 二维码 / 数字编码只作为商品身份信息；若同时识别到配料表或营养表，报告优先使用包装实拍 OCR 内容，并把编码保存为复用线索。
5. `productLookupService` 先查本机识别历史；只有商品名、品牌、编码、二维码等身份线索且缺少配料表和营养表时，通过后端 `POST /api/products/lookup` 触发 DeepSeek 搜索公开标签信息。搜到公开配料/营养则生成带 AI 来源标注的参考报告；缺 Key、AI 关闭、超时或无结果时不崩溃，生成信息不足报告并提示补拍配料/营养表。
6. 报告页新增统一识别信息卡片，展示识别类型、原始内容摘要、内容类型、标准化编码、商品名、品牌、数据来源和识别时间。
7. 若使用 DeepSeek 搜索，报告明显展示“部分商品信息来自 AI 联网搜索，可能存在过期、缺失或不准确；仅作公开标签线索，不作为包装实拍 OCR、成分事实、法规或医疗结论，请以商品包装实物标注为准。”并把 AI 搜索与包装实拍 OCR 分开展示。
8. 识别历史保存 `imageId`、`detectedType`、`rawContent`、`ocrText`、`normalizedCode`、`qrContent`、`productName`、`brand`、`ingredientsText`、`nutritionText`、`source`、`reportSummary`、`usedAiSearch`、`aiNotice`、`createdAt`，同一商品码/二维码后续补拍会合并历史。
...
4. OCR 入口继续遵守图片隐私和不伪造 OCR 的既有红线；拍照识别结束不进入确认页，低置信 / 失败 / 信息不足生成信息不足报告，手动输入和报告内补充进入文本补充；清晰可生成的 OCR 或已标注来源的 AI 公开标签线索可自动生成参考报告。

是否需要人工：可能需要 GTIN 商品数据源选择、数字标签标准/域名白名单、第三方数据授权和隐私/合规确认。

阻塞条件：外部商品库授权、数字标签来源确认、生产域名/网络访问策略、微信真机图片条码解码 provider 仍属于后续增强；不阻塞当前单入口自动分类、OCR 数字编码、合成 PNG 图片级工程门禁、历史复用、DeepSeek 后端补全和统一报告输出。

验证命令：`npm --prefix backend run typecheck`、`npm --prefix backend test -- tests/productLookup.test.ts tests/foodAnalyze.test.ts tests/nutrition.test.ts tests/ocr.test.ts`、`npm --prefix user-uniapp run typecheck`、`npm --prefix user-uniapp run lint`、`npm run user:regression:ocr-report`、`npm run product-code:evaluate`、`npm run product-code:review-report`、`npm run product-code:image-evaluate`、`npm run product-code:image-review-report`，后续补 H5 / mp-weixin / visual smoke / AI review。

状态：🔄 第一版实现中（2026-06-21）：单入口、自动分类服务、识别信息卡、历史合并、DeepSeek 后端补全和报告来源提示已落地；商品码解析级 `1500/1500`、合成 PNG 图片级 `983/1000` 均通过 DeepSeek 与 Codex 子 agent Review；微信真机条码/二维码图片解码 provider、真实包装照片扫码、外部商品库和数字标签规范化解析后续继续。
...
3. 拍照识别结束不进入确认页：清晰可生成的 OCR 或已标注来源的 AI 公开标签线索自动生成参考解读；低置信、失败、生成失败或信息不足生成信息不足报告；手动输入或报告内补充才进入文本补充。
...
5. 小程序报告页新增“一句话结论 / 关键原因 / 适合谁 / 不适合谁 / 添加剂解释 / 建议吃法 / 识别详情”，并接入 `foodAnalysis` 降级数据结构。
6. 新增小麻花真实包装文本回归，要求输出 `不建议常吃｜偶尔解馋可以`、热量/脂肪/碳水/钠提醒、小麦过敏/麸质敏感提醒和半包以内吃法建议。
7. 报告首屏继续减量，关键原因改为前 3 条优先级判断，完整营养数字用“营养重点图”展示，识别详情默认折叠，避免用户在完整 OCR/营养表里自行判断。
...
验证命令：`npm --prefix backend run typecheck`、`npm --prefix backend test -- tests/aiFoodExplanation.test.ts tests/foodAnalyze.test.ts tests/ocr.test.ts`、`npm --prefix user-uniapp run typecheck`、`npm --prefix user-uniapp run lint`、`npm --prefix user-uniapp run audit:product-output`、`npm --prefix user-uniapp run regression:ocr-report`、`npm --prefix user-uniapp run build:mp-weixin`、`npm --prefix user-uniapp run build:h5`、`npm --prefix user-uniapp run visual:smoke`、`git diff --check`。

状态：✅ 已完成（2026-06-20）：规则优先 + AI 解释网关已落地，DeepSeek 真实 smoke 已通过；生产 Provider 监控、成本阈值和正式额度策略仍待上线前配置。
...
4. 结果页去掉“大白话”残留文案，添加剂区统一为“添加剂解释”，避免吓人或过度洗白。
5. 结果页“识别详情”新增营养表行，和商品、类型、生产日期、配料表、包装声明、未确认线索一起靠后展示；详情默认折叠，不抢首屏重点。
...
状态：✅ 已完成（2026-06-20）：设置说明页不再进入小程序页面清单，报告识别详情已覆盖配料表、营养表、生产日期、包装声明和未确认线索，首屏重点已收敛为结论、关键原因和营养重点图。
...
涉及文件：`docs/product-blueprint/AI_UI_REVIEW_PROMPT.md`、`docs/product-blueprint/README.md`、`AI_REVIEW.md`、`user-uniapp/scripts/visual-smoke.mjs`、`user-uniapp/src/pages/index/index.vue`、`user-uniapp/src/pages/capture/index.vue`、`user-uniapp/src/pages/report/index.vue`。
实现内容：新增逐页 AI UI 评审提示词，覆盖首页、拍照/识别、结果页、历史、我的、成分搜索；明确结果页优先、拍照页第二、首页第三，历史/我的/搜索最后；要求 AI 输出普通人能看懂、有价值、可执行的页面优化建议，并禁止恢复“对照包装原文”、重复分享摘要、大块反馈卡和设置说明页。真实 DeepSeek 接通后已将当前页面功能、截图观察和用户目标提交多轮评审，按反馈继续优化报告个人关注、营养重点图、手动页重拍入口、历史页比例和成分搜索来源措辞；最终复评结论为 `pass` 且 `topIssues=[]`。用户要求改用 ChatGPT 评审后，新增 `user-uniapp/scripts/chatgpt-ui-review.mjs` 和 `npm --prefix user-uniapp run review:chatgpt`，读取 visual smoke 截图并调用 OpenAI Responses API；当前机器缺少 `OPENAI_API_KEY`，脚本输出 `missing_openai_api_key`，未伪造 ChatGPT 评审结果。
验收命令：`npm --prefix user-uniapp run visual:smoke`、真实 DeepSeek UI review smoke、`node --check user-uniapp/scripts/chatgpt-ui-review.mjs`、`npm --prefix user-uniapp run review:chatgpt`（无 Key 时预期 `missing_openai_api_key`）、`git diff --check`。
状态：✅ 已完成（2026-06-20）：评审提示词、截图烟测、DeepSeek 真实 AI 闭环和 ChatGPT 评审入口均已落地；真实 ChatGPT API 复评待用户在运行环境配置 `OPENAI_API_KEY`。
---
