...
```
首页扫描主控台（拍商品 / 扫条码 / 手动输入 / 当前关注目标 / 最近扫描）
  → 拍照 / 上传食品标签图片（商品正面 / 条码 / 配料表 / 营养成分表）
  ...
  → 自动识别标签内容（配料表 / 营养成分表 / 包装正面 / 未知，不让用户选择专业类型）
  → 自动拆分配料 + 添加剂识别 + 解析营养字段
  → 匹配食品成分 / 食品添加剂数据库 / 营养字段 / 用户关注项
  → 结合“我的关注目标/忌口”和本地消费建议规则排序展示
  → 生成消费建议页（结论 / 留意点 / 添加剂识别 / 营养快照 / 适合谁 / 怎么选）
  → 保存扫描记录
```

**添加剂识别是核心能力，配料解释是理解过程，营养分析是辅助判断，个性化消费建议是最终输出。成分搜索和商品对比都不是 MVP 主路径。**
